...
.VERSION
  v2 - Fixes single-string PowerShell indexing bug in Resolve-GitRoot.
...
.DESCRIPTION
  ...
  New untracked files are NOT copied unless explicitly named with -ExtraRelativePath.
...
function Resolve-GitRoot {
    ...
    $lines = @(Invoke-Git -Repo $Repo -GitArgs @("rev-parse", "--show-toplevel"))
    if ($lines.Count -lt 1 -or [string]::IsNullOrWhiteSpace($lines[0])) {
        throw "Could not resolve Git root for: $Repo"
    }

    $root = $lines[0].Trim()

    # Git may return forward slashes on Windows. PowerShell accepts them, but normalize.
    $root = $root -replace '/', '\'
...
Write-Host "DotTalk++ dev-to-stage sync v2" -ForegroundColor Cyan
...
foreach ($rel in $allRel) {
    ...
    if (!(Test-Path -LiteralPath $src -PathType Leaf)) {
        if ($isExtra) {
            $missingExtraList.Add([PSCustomObject]@{
                Relative = $rel
                Source = $src
                Destination = $dst
            }) | Out-Null
        } else {
            $missingSourceList.Add([PSCustomObject]@{
                Relative = $rel
                Source = $src
                Destination = $dst
            }) | Out-Null
