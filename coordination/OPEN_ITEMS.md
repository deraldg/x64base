...
| OI-004 | 2026-08-17 | 2026-09-08 | decision | **pydottalk has a write path and no identity.** It exposes `append_blank`, `delete_current`, `set`, `set_field`, `write_current`, and consults NO member, permission or token -- measured 2026-08-17, zero matches for member/identity/permission/auth/token in `module.cpp` outside a comment. LOCKING IS FINE and was nearly mis-stated: writes go through `DbArea`, and `src/xbase/dbarea.cpp:253` calls `xbase::locks::try_lock_record` before writing, unlocking at 275 -- so cooperative cross-process locking is inherited from the engine, same as the CLI and `dottalk_bbsd`. The gap is AUTHORIZATION, and it is an asymmetry: the maintenance console just gained a write-token boundary (`6a931ab3d`), the CLI has `cmd_security` and the AIF-075 permission gate, the BBS refuses unauthenticated sessions -- pydottalk is now the only write surface with none. Decide what it is: a read-only binding, a trusted in-process library where the caller is the boundary, or a surface that must carry identity. NOT urgent -- it is loopback-free, has no listener, and requires someone to already be running Python in the tree. |
