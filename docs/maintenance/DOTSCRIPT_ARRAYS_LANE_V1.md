...
2. **Canonical value model.** The spec proposes one `std::variant<Nil, bool, int64,
   ...
   record how the two value types reconcile. The spec's sec. 31 (one shared array API) and
   sec. 35 (one generalized lvalue) are **required** here, per AIF-037. **Decided
   2026-07-20:** the array *graph mechanics* are proven self-contained in M1a (a
   local sec. 30 `Value` variant), so the mechanics are settled before the value-model wiring;
...
4. **Brace audit (spec sec. 7).** Confirm `{` has no conflicting DotScript meaning today,
...
- **M1a -- Provable array core** (spec Phase 1 mechanics). [x] **DONE + unit-proven
  2026-07-20.** `src/xexpr/array_value.{hpp,cpp}`: the central array API (sec. 31) with the
  single checked-offset helper (sec. 32, the only place that subtracts one), `ArrayValue`/
  ...
  `would_create_cycle` (direct + indirect rejection). The element leaf is the spec sec. 30
...
- **M1b-1 -- Value-model binding.** [x] **DONE, MSVC-green 2026-07-20.** Extended the
...
- **M1b-2a -- Scoped memvar store.** [x] **DONE + g++-proven 2026-07-20**, added to the
  `xexpr` library (`src/xexpr/var_store.{hpp,cpp}`, `dottalk::dotscript::VarStore` +
  `session_vars()`). The Phase-0 sec. 1 prerequisite, built greenfield and self-contained:
...
- **M1b-2b -- Scalar memvars end-to-end.** [x] **DONE, MSVC-green + REPL-proven
...
- **M1b-2c -- Array literals + subscripts.** [x] **DONE, MSVC-green + REPL-proven
...
## Definition of Done (spec sec. 46, kept honest)
