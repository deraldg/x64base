...
| P4.0a | **Differential harness (R24c).** Dev command `EVALDIFF` + typed SANDBOX fixture + predicate corpus. Answers one question: given the same record and the same predicate, do the DbArea-bound and TupleRow-bound evaluators return the same verdict, and fail the same way? Changes no engine behavior -- an observer only | G4.0a (registered regression; a green run recording AGREEMENT is a real result, not a wasted slice) |
| P4.0b | **Evaluator seam + SQLsel migration (R24a/R24b).** One predicate-evaluation interface, two implementations; SQLsel's `WHERE` takes the tuple-bound one with `overlay_table_buffer=false` to preserve R18. Sequenced AFTER the harness so the migration is measured, not assumed | G3 RE-RUN unchanged -- if the SQLite oracle still matches row-for-row, the migration changed nothing |
