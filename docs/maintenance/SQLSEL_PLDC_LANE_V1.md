...
| OQ-10 aggregates over blank | P4.6 | SQL SKIPS NULL in SUM/AVG/COUNT(col); x64base has no NULL and R16 ruled blank IS a value. So does AVG over a numeric column containing blanks treat blank as 0 (drags the mean down) or skip it (matches SQLite)? The two answers differ on real data and BOTH are defensible. No proposed default -- this is an owner ruling, and guessing it silently is the exact defect class this lane closed three times |
