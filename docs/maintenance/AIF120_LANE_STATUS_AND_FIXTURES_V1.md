...
| Rulings **R13 through R81** live in their own files; this ledger is their only index. The charter does not carry them. | -- |
...
| **R76** -- the UIDEF document is a CMake source; extracting the target hit the same silent parent globals AIF-118 recorded for pydottalk | `docs/maintenance/AIF120_CMAKE_TARGET_V1.md` |
| **R77** -- measured against a wx sample the lane did not author: a word for 90% of controls, 4b(b) corroborated from outside, and no word for NEGOTIABLE geometry | `docs/maintenance/AIF120_SAMPLE_MEASUREMENT_V1.md` |
| **R78** -- the round trip carried the whole tree with zero refusals and lost WEIGHT; `FLOW` declares order and containment, not proportion | `docs/maintenance/AIF120_ROUND_TRIP_V1.md` |
| **R79** -- `Weight` and `Fill` added to `PROPS`, implemented in wx and gated; 18/18 fixtures byte-identical | `docs/maintenance/AIF120_LAYOUT_WEIGHT_V1.md` |
| **R80** -- `Weight` needs free space and two of four backends have none; HTML fixed, Tk implemented and reports the lost ratio, character-cell reports the drop | `docs/maintenance/AIF120_WEIGHT_BACKENDS_V1.md` |
| **R81** -- the character-cell backend made space-filling along a row; the remainder rule is stated because a character grid divides a DISCRETE resource | `docs/maintenance/AIF120_CELL_ALLOCATION_V1.md` |
...
| R81 | the three pixel backends hand rounding to a toolkit; a character grid has no toolkit, so the renderer divides a DISCRETE resource and must own the halves itself -- and the rule it chooses has to be WRITTEN DOWN, because a renderer that rounded differently on different runs would make one document mean two things | R80 named the two-pass `draw()` and this is it: along a `row`, measure each child's natural glyph width, divide the slack by the declared weights, widen the glyphs -- **`floor(slack * weight / total_weight)` each, remainder ONE CELL AT A TIME in ORDINAL order**, arbitrary but deterministic and stated; proven on arithmetic chosen not to come out even (3:1:1 of 54 spare cells is 32/10/10 with two left over, so 33/11/10, and the row fills its 94-cell budget EXACTLY), by a script that MEASURES the rendered cells and checks them against the rule computed independently, and that refuses to pass if the case ever stops exercising the remainder; **filling a budget is the first thing this renderer has done that could OVERFLOW one and it immediately did, three times** -- a phantom two-column trailing gap recorded into `maxc` after every row, a `BOX_OVERHEAD` that was 2 where a container GRANTED and 3 where it ASKED so every nesting level cost three columns nobody reserved (and `MAINFRAME` is five deep), and a `stretch()` that would have grown a combo past its own drop arrow -- none of which R79 or R80 could have found, because it took a renderer that USES its budget to expose a renderer that mis-states it; and because R81 honours SOME weights, **every weighted child it will not resize is now named INDIVIDUALLY with its own reason**, since a partial honouring is exactly where a silent drop hides -- 20 weighted children on `MAINFRAME`, 2 honoured, 18 named, against R80's 20 dropped in 18 lumps; 19 of 22 documents byte-identical, and the two that moved without declaring any Weight lost two columns that no element was ever in |
| R80 | a property is not something a backend HAS or LACKS -- `Weight` is a share of FREE SPACE, so a container that sizes to its children cannot carry it however good the toolkit is | R79 shipped a table asserting all four backends carry Weight and Fill natively; every cell was true about the TOOLKIT and two were false about this lane's BACKEND, because `uidef_html.py:317` makes the form `display:inline-block` and `uidef_text.py` only ever DECREMENTS `avail_w` and never divides it -- both content-sized, so correct CSS and correct intent would have been emitted and been INERT; that is asserting from the shape of the thing instead of measuring it, in a ruling one turn old, about the property I was implementing; **HTML fixed** -- a form declaring ORIGIN dimensions becomes a sized flex column so children have something to grow into (`flex-grow:3` / `flex-grow:1` / `align-self:stretch` proven, `Weight = 0` correctly gets nothing), and a form WITHOUT them reports the drop, which measured is every fixture in the corpus; **Tk implemented** and inspected through the real widget tree under Xvfb -- 20 of 33 packed children claiming expand, 22 claiming fill -- **but Tk's `expand` is a BOOLEAN that divides slack EQUALLY, so `Weight=3` against `Weight=1` keeps the intent and loses the RATIO**, reported as DEGRADED per child; **character cell reports the drop** because making it work means a two-pass measure-then-distribute `draw()`, a rewrite of the oldest backend; every one of the three new behaviours is a REPORT rather than a silent success or a silent failure, which is the only reason a half-supported property is safe to ship |
| R79 | "go for gold unless the cost is platinum" has a second half that decides the design: ship the gold now, SHAPED so the platinum upgrade is an extension | the maintainer answered R78's recorded-but-undesigned gap with "gold standard", so `Weight` (share of the FLOW axis, 0 = fixed) and `Fill` (stretches ACROSS it) went into **`PROPS` on the CHILD, not a seventeenth field** -- a column would be tidier and would change the record length, every reader, both importers and every document ever authored, which is the platinum the rule brakes on; **absent means 0 and false, so all 18 fixtures generate BYTE-IDENTICAL output** -- a layout property added to a layout language that provably moves no existing document; child-not-container is the sequencing that makes R77's splitter an EXTENSION rather than a rewrite, because a sash is two weighted children plus a movable boundary and the weights would already be there; measured before implementing that all four backends carry both natively (wx proportion/wxEXPAND, Tk expand/fill, CSS flex-grow/align-self, character cell share/span); proven by rendering ONE document twice -- 0 to 11 `wxEXPAND`, 0 to 8 non-zero proportions, a frame that went from content-in-a-corner to content-using-the-window; and stated plainly that **Weight does NOT describe a sash** -- turning 500px into 3:1 is a judgement the author makes, not a translation; plus R79.1, `manifest()` collected PROPS only for the subsets earlier rulings needed, so a property belonging to ANY child had nowhere to be checked from -- `all_props` is not a new source of truth, just the whole of the existing one |
| R78 | the thing a round trip loses is rarely the exotic control you were watching for -- it is the ordinary property you never thought to name | `author_mainframe.py` describes `src/gui/wx/main_frame.cpp` as a UIDEF document, `uidef_wx.py` generates it, and it BUILDS AND RUNS against wx alone with no engine (R70's opt-in design): **47 records, 45 elements, ZERO refusals**, carrying a `pageset` nested inside a `page` inside a `pageset` that no prior document in this lane has ever nested; what it could not carry was **PROPORTION** -- measured, **33 sizer additions carry an explicit proportion, 20 fixed and 13 saying take the remaining space** (`command_`, `areas_`, `root`, `grid`, ...), plus 27 `wxEXPAND` -- so the render is a recognisable frame in which NOTHING STRETCHES; R12 ruled layout intent is the portable geometry and quarantined absolute coordinates, but **weight is the MOST portable part of layout intent** and all four backends have it (wx proportion, Tk weight, CSS flex-grow, and a character cell renderer needs it to decide which region absorbs the leftover columns), and the design table captured order and containment and dropped the one property every target shares; also lost: the three sash positions 220/500/260, `CreateStatusBar(4)` collapsing to one string, and **R33.4 -- which ruled in this lane that a caption should be a reference `@FORM_STUDENTS_TITLE` resolved through the message catalog, was never implemented in any tool or the contract, and is corroborated by a hand-written GUI that routes EVERY visible string through `gui_text(GuiTextId::X, locale_)`** -- a declaration nothing acted on, for the fourth time this session and the first time belonging to a ruling made long before it; and R78.1, a generator defect the round trip found in one build that eighteen fixtures never could, because not one of them contains a `pageset`: `child_sizer()` was creating a wxBoxSizer for a container that adds its children through `AddPage` and never through a sizer |
| R77 | a language graded only on its own homework will pass -- and the gap an unauthored screen finds is a CONCEPT, not a control | every UIDEF document to date was authored by this lane, so `src/gui/wx/main_frame.cpp` (2,140 hand-written lines, one of the maintainer's template samples, built with no reference to UIDEF) is the first independent case; **56 of 62 control constructions have a word (90%), 42 map cleanly (68%), 6 have none**; the gap I went looking for WAS NOT THERE -- I expected the twelve `wxGrid`s to collide with 4b(b)'s read-only browse and `main_frame.cpp:375` says `grid->EnableEditing(false)`, so a GUI that never read the contract independently arrived at the constraint the contract requires, which is corroboration from outside and better than the gap; what has no word is `wxSplitterWindow` x3, and not because it is a missing widget -- R12 ruled layout intent is the portable geometry and section 8 quarantines absolute coordinates, but **a sash is a boundary the author PROPOSES and the USER moves**, so UIDEF can describe geometry that is decided and geometry that is derived and has no way to describe geometry that is NEGOTIABLE, which `ORIGIN` cannot carry because ORIGIN is advisory input and a sash position is live state; stated and DELIBERATELY not answered, and no kind added, because inventing `splitter` in the document that discovered the need for it is grading your own homework twice in one file |
| R76 | any target extracted from this tree inherits the parent's silent globals -- and that list is already written down, so the second consumer to discover it read nothing | `gui/uidef/CMakeLists.txt` makes the DOCUMENT a build input: `cmake --build` authors FRAMEDEMO.DBF from `author_frame.py`, generates 163 lines of C++ with `uidef_wx.py --stream`, compiles it against the engine and links a running window -- clean configure+build from an empty tree in **62 s, 66 translation units**, four static libraries, a 2.27 MB binary, against the several hundred TUs a root configure would compile (the pydottalk waste AIF-119 measured); the first build failed on **`dottalk/build_vectors.hpp`**, which is item ONE on the four-global list AIF-118 published for pydottalk in August, and `src/help`'s `cli/...` include right behind it -- fixed by running the ROOT'S OWN `config/build_vectors.cmake` and template rather than keeping a second capacity table; the 29 CLI files listed here belong to no library at all (globbed straight into `dottalkpp` at src/CMakeLists.txt:92) so this is their FIRST list, not a second copy, and the right end state is a `dottalk_cli_rt` library both targets link -- R61's boundary with a build consequence; and a hazard named while nobody stands on it: the AIF-043 duplicate-basename shadow guard globs from a HARDCODED five directories, so a future library built from `src/cli` would be invisible to the guard that exists to catch exactly that shadow; owner ruling recorded that `DOTTALK_WITH_WX`/`DOTTALK_WITH_GUI` build the TEMPLATE SAMPLES, so this target attaches to neither |
