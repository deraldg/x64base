...
| `ai.link.agent_assignment_conversation` | Agent Assignment Link | bind each governed assignment to a shared local conversation and typed provider, Portal-context, BBS, language, message-provenance, and UI-observation routes | `docs/contracts/AI_AGENT_ASSIGNMENT_LINK_CONTRACT_V1.md`; `dottalkpp/data/schemas/syschatlnk_v1.schema.json`; `docs/maintenance/AI_PORTAL_BBS_PSEUDO_CHAT_RELATIONAL_SCHEMA_PLAN_V1.md`; `labtalk/registries/agent_assignment_links.yaml` | AI Systems Integration SDLC | AI Systems Integration SDLC | source-defined; disposable v1 X64 create/read proof accepted; normalized design review open; no production catalog | relationship crosswalk, not project or BBS authority |
