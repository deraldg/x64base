...
| P2 | **IMPLEMENTED AND RUNTIME-OBSERVED 2026-09-03.** `sqlsel` is an explicit library target. `SET MODE SQL\|NATIVE` is session-only and prompt-visible; SQL mode routes SELECT/INSERT/UPDATE/DELETE/BEGIN/COMMIT/ROLLBACK, blocks native REL state, and disables native expression fallback. `SQLMODE_SMOKE` pins both directions and refusal behavior | G2 GREEN: 2/2 |
...
| P4.1 | **IMPLEMENTED, RUNTIME-OBSERVED, AND DEFAULT-SUITE PROMOTED 2026-09-03.** INNER JOIN is ad-hoc typed set matching and does not consult REL/SET RELATION. Tables resolve inside the current workspace; aliases, joined WHERE, ORDER BY/LIMIT, COUNT(*), cursor neutrality, row multiplication, and an always-reported access path are machine-gated against SQLite | G4a GREEN: 4/4; mutation RED observed; DEFAULT SUITE |
...
| P4.3 | **IMPLEMENTED AND RUNTIME-OBSERVED 2026-09-03.** LEFT JOIN carries produced absence as `TupleCellKind::ProducedAbsent`, renders through the one `<UNMATCHED>` seam, and evaluates post-extension WHERE with TRUE/FALSE/UNKNOWN. Genuine blanks and stored marker text remain present values. Fence, CDX/scan paths, extension counts, cursor/FLOCK preservation, and SQLite rows are pinned | G4c GREEN: SQLSEL_LEFT_JOIN 7/7 |
| P4.4 | **IMPLEMENTED AND RUNTIME-OBSERVED 2026-09-03.** RIGHT/FULL/CROSS two-table forms share typed ON/WHERE, read fences, cursor guards, and path reports. RIGHT/FULL outer WHERE now uses UNKNOWN correctly; CROSS takes no ON. Family regression includes answer/path/extension/refusal assertions | G4d GREEN: SQLSEL_JOIN_FAMILY 14/14 |
| P4.5 | **IMPLEMENTED AND RUNTIME-OBSERVED 2026-09-03.** DISTINCT, UNION, UNION ALL, INTERSECT, and EXCEPT operate on typed TupleRows. Equal arity and compatible per-column types are required; incompatible operands refuse instead of coercing. INTERSECT precedence and duplicate behavior are oracle-pinned | G4e GREEN: SQLSEL_SET_OPS 7/7; mutation RED observed |
| P4.6 | **IMPLEMENTED AND RUNTIME-OBSERVED 2026-09-03.** GROUP BY/HAVING and COUNT/SUM/AVG/MIN/MAX consume SQLsel-produced typed rows, including joined rows. Per R28 numeric blanks are skipped rather than invented as zero, and the contributing/blank split is reported | G4f GREEN: SQLSEL_AGGREGATES 7/7; mutation RED observed |
| P4.7 | **IMPLEMENTED AND RUNTIME-OBSERVED 2026-09-03.** Scalar, IN/NOT IN, EXISTS/NOT EXISTS, uncorrelated caching, correlated single-table outer binding, and relational division by nested NOT EXISTS. Scalar cardinality/type failures refuse; correlated work reports actual evaluation count. Joined outer correlation remains an explicit refusal | G4g GREEN: SQLSEL_SUBQUERIES 7/7; mutation RED observed |
...
| P4 | **IMPLEMENTED AND RUNTIME-OBSERVED 2026-09-03.** Full R26 algebra is present. Generalized work adds typed expression projection, self/composite joins, multi-column order, and INNER/LEFT/CROSS chains across three or more sources; RIGHT/FULL remain two-table forms. The house TupleRow is the carrier throughout | G4 GREEN: 11 read/evaluator specs plus SQLMODE; 99 compared result sets/cases before DML/workspace |
| P5 | **IMPLEMENTED AND RUNTIME-OBSERVED 2026-09-03.** Typed INSERT/UPDATE/DELETE assemble existing APPEND/REPLACE/DELETE storage semantics through TableBuffer, TBJ1 WAL, owner-aware FLOCK, index maintenance, and COMMIT. Autocommit and SQL-mode BEGIN/COMMIT/ROLLBACK are atomic for one target table; read-your-writes applies to later DML while SELECT remains committed truth. Cross-table atomicity, stored NULL, and memo-field DML refuse | G5 GREEN: SQLSEL_DML 6/6 and SQLSEL_WORKSPACE 4/4; both mutation RED; MSVC and GCC/WSL |
| P6 | **USER MANUAL AND RUNTIME HELP UPDATED 2026-09-03.** The SQLsel chapter now documents the complete implemented algebra, typed TupleRow model, current-workspace resolver, SQL mode, DML, one-table transaction boundary, NULL/memo refusals, and all named gates. Source-owned usage and `sql_ref.hpp` were updated with it. Generated HELP DBFs, LabTalk lessons, evidence gallery, and public-site promotion remain separate deliberate publication work. **SCOPED 2026-09-04, AND NONE OF THE FOUR IS MECHANICAL FROM HERE.** (a) The generated HELP DBFs under `dottalkpp/data/help/` are dated 2026-09-01, BEFORE the 2026-09-03 source-owned usage update, so they are stale by construction; regenerating means `CMDHELP BUILD` against the host runtime, AND the whole `help/*.dbf` and `*.dbt` set is currently CARRYING UNCOMMITTED MODIFICATIONS from other work, so a refresh now would fuse two sessions' changes into one slice -- the thing `AI_SESSION_COORDINATION_PROTOCOL_V1.md` forbids. Refresh only from a clean help set. (b) There are NO LabTalk lessons for SQLsel: `labtalk/` holds proof RUNS only (`20260729_*` through `20260903_*`), which are evidence, not teaching. Lessons are authoring, not generation. (c) No evidence gallery exists in the tree under any name searched. (d) Public-site promotion is a separate repository-role act and never runs from the development worktree. **So G6 stays PARTIAL and the remaining work is four distinct jobs, three of them authoring or publication rather than a build step** | G6 PARTIAL: source/manual current; generated/public surfaces deferred, scoped 2026-09-04 |
...
**SWEPT 2026-09-04 AGAINST THE TREE, and three of the four live rows had drifted.**
This table is written forward as rulings are made and had not been re-read against
the code since. Two rows understated progress (OQ-14 and SET FILTER were both
DONE and still listed as owed) and one understated the problem (the bare `SQL`
verb is still shipping against a ruling that retired it). The direction of drift
is not uniform, so a row here may not be trusted in EITHER direction without
measuring; each corrected row below names the file and line it was measured from.
Only `Product name` is unchanged, because it is an owner's call and no
measurement can move it.

**RE-SWEPT 2026-09-10, AND THAT LAST SENTENCE WAS ITSELF WRONG.** The 2026-09-04
sweep read this table against the TREE and did not read it against the RULINGS
LEDGER three sections above. Both remaining rows were already ruled: OQ-2 by
R16(a) and `Product name` by R19, both on 2026-07-29. The claim that no
measurement could move the `Product name` row was true of the code and false of
this document -- the ruling was six weeks old and sitting in section 2. So the
drift count is five of ten rows, in three different directions: understating
progress, understating a problem, and recording a made decision as an owed one.
**With those two corrected and the bare `SQL` row closed the same day, this table
now has NO open rows.** A table with no open rows is not a table that stopped
needing to be read; it is one whose next entry has to be written by whoever makes
the next ruling, and this section's history says that is exactly the step that
gets skipped.

| Ruling needed | Blocks | Proposed default |
|---|---|---|
| OQ-2 blank-vs-NULL | P1.2 (outer joins UNBLOCKED by R22) | **CLOSED BY R16(a), 2026-07-29 -- the proposed default IS the ruling, and this row has been recording a decision as though it were still owed.** R16(a) reads "OQ-2 CLOSED -- blank-is-a-value; the value model is ENGINE-owned and mode-invariant; no NULL literal in v1", and UNBLOCKS P1.2 in the same clause. What shipped agrees on three independent surfaces: SQLSEL DML REFUSES to store a null rather than inventing one (P5); LEFT JOIN carries produced absence as `TupleCellKind::ProducedAbsent` through the single `<UNMATCHED>` seam (R22, P4.3), so an absent cell is never confused with a genuine blank; and R28 SKIPS numeric blanks in aggregates rather than counting them as zero, reporting the contributing/blank split. NULL semantics may return as its own lane -- that would be a new lane, not this row. |
...
| OQ-11 set-operation operands | **IMPLEMENTED AT P4.5.** | Equal column count and compatible per-column TupleRow types are required. A pairing with no house comparison/equality rule is reported and refused rather than coerced |
| OQ-12 correlated subquery cost | **IMPLEMENTED AT P4.7.** | Correlated subqueries execute correctly per outer row and report the actual evaluation count. Optimization remains evidence-led rather than speculative |
| OQ-13 qualifier namespace DEPTH | **RULED BY AIF-149; current-workspace scope proven 2026-09-03.** | SQLsel qualifiers remain `alias.field`. Logical table names resolve only among areas in the current workspace; there is no dotted workspace namespace or cross-workspace statement |
| SET FILTER stance for SQL statements | **CLOSED -- MEASURED 2026-09-04, both halves hold.** | The proposed default shipped. `src/cli/sqlsel_statement.cpp` never consults a session filter on any read path, and the stance is STATED for the reader at `docs/manuals/user/sqlsel.md:86-91` under "SELECT is cursor-neutral", alongside the record pointer and REL/SET RELATION. Nothing owed |
| Bare `SQL` command disposition | **STILL OPEN, AND THIS ROW IS THE ONLY ONE IN THIS TABLE THAT UNDERSTATES ITS STATE.** P2.4 | Ruling was "retire after P0 demotion". **P0 and P2 are both closed and the retirement NEVER HAPPENED. Measured 2026-09-04: `SQL` is still registered at `src/cli/shell_commands.cpp:452` (`registry().add("SQL", ... cmd_SQL)`), and `src/cli/cmd_help.cpp:588` lists it as a HELP sub-topic while `:689` still routes `SQL`/`SQLHELP`.** The verb is live on the shipped surface against a ruling that retired it. **BUT READING `cmd_sql.cpp` CHANGES THE QUESTION, AND RETIREMENT SHOULD NOT PROCEED ON THIS ROW AS WRITTEN.** Four measurements, 2026-09-04: **(1) `SQL` IS NOT A PHANTOM.** It is a working predicate scanner over the CURRENT WORK AREA -- `SQL [COUNT] [ALL\|DELETED] [FOR <expr> \| <expr>] [VERBOSE]` -- routed through the shared DotTalk evaluator and LRU cache. Retiring it DELETES A FEATURE; it does not remove a stub. **(2) THE CONFUSION THE RETIREMENT EXISTS TO PREVENT IS ALREADY PREVENTED.** `cmd_sql.cpp` carries an AIF-074 guard that intercepts a leading `SELECT` and redirects to SQLSEL with a corrective error naming the right grammar, added precisely because `SQL SELECT ... FROM ...` would otherwise parse as a PREDICATE and report a false zero. The silent-nonsense case is closed in code. **(3) THE P0 HALF IS ALREADY DONE.** The `@dottalk.usage` contract in `cmd_sql.cpp` reads `status: experimental`, so the demotion this row makes retirement conditional on has landed; only the retirement itself is outstanding. **(4) IT IS EXERCISED BY THE DEFAULT SUITE.** `dottalkpp_non_destructive_smoke.dts:138` runs `SQL HELP` and NONDESTRUCTIVE is a default-suite spec, so retirement is a change set that includes a regression edit, not a one-line deletion. **THE RULING OWED IS THEREFORE NOT "RETIRE OR DEPRECATE" BUT "IS THE 2026-07-29 RETIREMENT STILL WANTED AT ALL"**, given that its stated rationale was discharged by the redirect guard and that the verb names a capability SQLSEL does not offer (predicate COUNT over the current area, no statement grammar). **RECOMMENDATION SUPERSEDED THE SAME DAY, ON THE OWNER'S QUESTION "does SQL actually do anything really".** The four findings above are correct and the conclusion drawn from them was not: it treated `SQL` as a capability SQLSEL lacks, without asking whether some OTHER command already had it. **`COUNT` DOES, AND MORE.** `cmd_count.cpp` documents `COUNT`, `COUNT FOR <expr>`, `COUNT WHERE <expr>`, `COUNT <expr>`, `COUNT ALL`, `COUNT DELETED`, `COUNT NOT DELETED`, `COUNT !DELETED`, and normalizes the predicate form before scanning -- a strict superset of `SQL`'s surface except for `VERBOSE` per-row tracing and the bare-predicate listing mode. **AND THE TWO DISAGREE, WHICH IS THE PART THAT MATTERS.** `COUNT` treats persistent `SET FILTER` and `SET DELETED` as part of the LOGICAL ROWSET (AIF-123, 2026-08-24) and selects through the shared `cli::scan::collect_selected_recnos` path that `cmd_delete.cpp` and `cmd_recall.cpp` also use. `cmd_sql.cpp` uses NONE of it: it declares a PRIVATE `DelMode` enum with its own `include_row()` deleted-flag policy and walks the table with a raw `do { ... } while (A.skip(+1) && A.readCurrent())`, and the string `FILTER` does not appear in the file. **So `SQL COUNT FOR <x>` and `COUNT FOR <x>` can return DIFFERENT NUMBERS over the same table whenever a filter is active or `SET DELETED` is off, and neither reports that it differs.** That is a fourth private declaration of "which rows are in scope", the same defect shape this house records for field-name normalizers. **RETIREMENT IS THEREFORE BACK ON, for a stronger reason than the one it was filed under.** The 2026-07-29 rationale (name confusion with SQLSEL) really was discharged by the redirect guard; the live reason is that `SQL` is a SECOND, WEAKER, DIVERGENT IMPLEMENTATION OF `COUNT`. Owner ruling owed on: retire outright, or keep only `VERBOSE`/listing behaviour by folding it into `COUNT` first. Either way `dottalkpp_non_destructive_smoke.dts:138` (`SQL HELP`) moves in the same change set **CLOSED 2026-09-04 BY THE OWNER'S RULING AND THE COMMIT THAT CARRIES IT.** Owner: "fold verbose and listing into count and reserve SQL command name for other use." The fold landed -- `COUNT LIST` and `COUNT VERBOSE` on `cli::scan::collect_selected_recnos` -- and `cmd_sql.cpp` is now a reserved-name responder that answers with direction, accepts any argument, needs no open table and moves no cursor. So this row's own question, "is the 2026-07-29 retirement still wanted at all", is answered YES ON THE STRONGER GROUND THE ROW ITSELF NAMES: not the name confusion, but the second divergent declaration of which rows are in scope. `dottalkpp_non_destructive_smoke.dts:138` (`SQL HELP`) did NOT have to move after all -- the reserved responder takes any argument -- and NONDESTRUCTIVE is green on the landing build. **ONE OF TWO COPIES.** `cmd_sql_select.cpp`'s compat scan keeps its own `DelMode` at :144 and its own raw walk at :572-613 with zero uses of the shared selector, so the divergent-scope defect this row finally named is HALF closed. That half is a separate owner ruling: SQLSEL is a `supported` surface and removing part of it is not a maintenance edit. **CLOSED 2026-09-10 BY THE OWNER'S RULING AND THE COMMIT THAT CARRIES IT.** The second copy is gone. `src/cli/cmd_sql_select.cpp` fell from 618 lines to 225: the private `enum class DelMode`, `static Opts parse_opts`, `extract_field_names`, the `STok`/`stok` tokenizer, `Clause`/`parse_simple_chain`/`eval_clause`/`eval_chain`, `up()`, the `compile_where` forward declaration and the entire raw-walk body were deleted, so the file now holds exactly one path -- usage, then `sqlsel::try_execute_statement`, then a corrective error. SQLSEL therefore has ONE answer to "which rows are in scope", the statement engine's, and the fourth private declaration of scope this row named is now fully closed. **RETIREMENT, NOT FOLD.** `COUNT` already carries the predicate job -- `COUNT FOR <expr>`, `COUNT LIST`, `COUNT VERBOSE` on `cli::scan::collect_selected_recnos` -- so nothing was left to move; the corrective error names COUNT and says why the old form is gone. A symbol census before deletion confirmed every removed name was reachable only from the legacy region, and a tree-wide grep confirmed nothing uses the bare-predicate form: every `SQLSEL` line in the tree carries a `FROM`, is a comment, or is `SQLSEL HELP`. **RUNTIME-PROVEN 2026-09-10** against a fresh `datarun` on the landing build: `SQLSEL LNAME = "SMITH"` printed the five-line retirement message verbatim; `SQLSEL HELP` printed the amended usage notes inside NONDESTRUCTIVE; `SQLSEL_INNER_JOIN` reported PASS (4/4 row sets equal SQLite, cursors 2/2, refusals 3/3, access paths 4/4); NONDESTRUCTIVE green. **TWO CONTRACT FIELDS CHANGED ON THE AUTHOR'S JUDGMENT AND FLAGGED FOR REVIEW:** `noargs` from `scan/report` to `corrective-error`, and `requires_open_table` from `yes except usage` to `no; a statement names tables already open in the workspace` -- both because the behaviour they described is exactly what was retired. `review-needed`. |
| Product name (SQLTalk?) | P6 | **CLOSED BY R19, 2026-07-29.** The owner ruled the product name is `SQLsel` -- capital SQL, lowercase sel; no new name, no "SQLTalk"; the working name IS the product name. `SQLSEL` remains the CLI verb as typed (commands render uppercase by convention); `SQLsel` is the brand form for manuals, HELP prose, website and curriculum. R19 states in its own text that it closes the last open P6 question. R16, written the same day, lists the product name as still open and was superseded by R19 hours later -- which is how this row kept its "owner's call" cell through the 2026-09-04 sweep. |
| OQ-14 verb redundancy: why `SQLSEL SELECT`? | grammar surface + docs-wide | **RULED (owner, 2026-08-08): `SELECT` is OPTIONAL (soft landing) -- implemented in `src/cli/sqlsel_statement.cpp` (dispatch keys off top-level FROM; a leading `SELECT` is consumed if present). USAGE updated. ~~Build + `REGRESSION SQLSEL_SELECT_V1` (add a bare-form case) owed -- sandbox cannot build.~~ **BOTH DISCHARGED; MEASURED 2026-09-04.** The bare form is exercised at `dottalkpp/data/scripts/sqlsel_select_v1_regression.dts:140` (`SQLSEL S.SID,S.LNAME FROM SQLSTU AS S WHERE ... ORDER BY ...`, no inner `SELECT`) and it is not merely executed: `src/cli/cmd_regression.cpp:1451` pairs marker set `SQLSEL-S14` with oracle set `SQLSEL-O14`, so the bare form is machine-compared against SQLite like every other case in that spec. The "sandbox cannot build" clause was ALSO false by the time it mattered -- see `CLAUDE.md`, which corrected that ceiling on 2026-08-25 after four agents re-derived it.** Original rationale: **drop the redundant inner `SELECT`.** `SQLSEL` IS the select verb (the home SQL brand); `SQLSEL SELECT <list> FROM` starts a statement with two select verbs. We never write `foxpro select` or `sqlite select` -- a dialect names its verb once -- so `sqlsel select` is redundant and half-undoes the confusion SQLSEL was coined to remove. Canonical becomes `SQLSEL <select-list> FROM <table> [WHERE][ORDER BY][LIMIT]`. Source: `src/cli/sqlsel_statement.cpp:150` requires the inner `SELECT`, USAGE at `:132-135` documents it. Owner leans clean break (no live scripts depend on it) vs a one-release optional-keyword soft landing. Doc-wide consequence: everywhere, differentiate xBase `SELECT <area>` (`cmd_select.cpp`, work-area switch) from `SQLSEL` (SQL query). Full note: `SQLSEL_VERB_REDUNDANCY_DESIGN_NOTE_V1.md` |
...
- The shared x64base sidecar layer was tightened at the seam this exposed. Lock
  ...
  reclaimed.
- The native `dottalkpp_lock_protocol_test` passed six arms over real X64
  ...
  reverse table-to-record exclusion, same-owner re-entry, and sixteen rounds of
  synchronized two-process atomic publication. The last arm requires exactly
  one winner and verifies that winner's owner token while the sidecar is held.
  A check-then-overwrite mutation made both children report `WIN` in every round
  and turned the test red. AIF-150 records this cross-cutting lock finding apart
  from the SQLsel feature that exposed it.
...
**2026-09-03, P4.3 LEFT JOIN (candidate evidence):**

- `TupleRow` now carries an optional aligned `TupleCellKind` vector. Existing
  producers default to `Present`; new tuple construction records it explicitly.
  `ProducedAbsent` is a distinct carrier state, and `render_tuple_cell()` is the
  single display seam for it. This implements R29's kind-tag and one-renderer
  readiness without claiming stored NULL or three-valued logic.
- LEFT JOIN extends each non-deleted outer row that has no non-deleted ON match.
  The extension uses no fabricated right-side DBF bytes. Projection and ORDER BY
  ask the shared renderer for the cell text, and every LEFT statement prints the
  number of extended rows independently of the marker.
- `REGRESSION SQLSEL_LEFT_JOIN` passed four SQLite oracle pairs: projection and
  COUNT(*) through CDX seek, then the same two through nested-loop scan. The
  fixture includes duplicate keys, an unmatched key, a deleted outer row, a
  deleted-only right match, a genuine blank right value, and a matched right
  value whose stored text is literally `<UNMATCHED>`. SQLite maps NULL with
...
- LEFT JOIN plus WHERE is deliberately refused. The expression AST currently
  returns bool, so treating produced absence as either a DBF blank or the marker
...
**2026-09-03, complete algebra + typed DML integration candidate:**

- SQLsel now builds as its own `sqlsel` library and dogfoods typed TupleRows for
  selection, projection, joins, set operations, aggregates, subqueries, and DML
  evaluation. Types, field width/decimals, produced absence, and source fragments
  stay attached to the row carrier.
- The completed relational surface includes DISTINCT and all four set operators;
  GROUP BY/HAVING with COUNT/SUM/AVG/MIN/MAX; scalar, IN/NOT IN, EXISTS/NOT
  EXISTS and correlated subqueries; expression projection; self/composite joins;
  multi-column ordering; and INNER/LEFT/CROSS chains. RIGHT/FULL remain supported
  two-table forms. The focused family is 14 registered specs and 109 compared
  result sets or exact evaluator cases, all green on the native build.
- SQLsel DML does not invent a storage path. INSERT becomes the existing
  `appendBlank()` at COMMIT; UPDATE shares REPLACE's type-normalization gate;
  DELETE stages the existing delete flag. All three flow through TableBuffer,
  TBJ1 WAL, owner-aware FLOCK, index maintenance, and house COMMIT/ROLLBACK.
- Explicit SQL transactions require SQL mode and are atomic for one target
  table. Later DML reads its own buffered inserts/updates/deletes; SELECT remains
  a committed-truth surface until COMMIT. A second target table, stored NULL,
  and memo-field DML refuse because no cross-store atomicity is claimed.
- SQLsel table names now resolve only among areas in the current workspace. A
  two-workspace fixture opens duplicate table names with different typed values
  and proves SELECT, JOIN, UPDATE, and cursor restoration against SQLite. A
  hidden second process-wide lookup in the tuple projection path was caught by
  the first run and removed by passing the resolved area slot forward.
- `SQLSEL_DML` passed 6/6 and `SQLSEL_WORKSPACE` passed 4/4 under both MSVC and
  GCC/WSL. Independent oracle-row mutations made each validator fail before the
  fixtures were restored. Native CTest passed 22/22.
- The WSL run found that the shared L3 catalog-isolation arm used Windows path
  separators and printed its detector false while the command still returned
  success. Its scratch paths are now portable, and the six-marker read rule is
  executable in `cmd_regression.cpp`. A deliberate false-marker mutation
  produced `L3 CATALOG ISOLATION: FAIL` plus canonical invalid-argument status;
  restoration produced 6/6 PASS and success on Windows and WSL.
- The user manual, runtime usage text, source usage contract, SQL cross-reference,
