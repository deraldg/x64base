...
- **Polish -- `AGGS ALL`** single-pass COUNT/SUM/AVG/MIN/MAX (~4x vs four verbs;
...
engine stays correct and reasonably fast when the data grows ~1,000x -- real
...
### Good news for the boy (2026-07-19/20)

Pinocchio's question so far has been about *performance* at volume. The engine has
now also started *growing structurally* toward real-database shape -- on two fronts,
both development-only and not yet promoted:

- **Record-number addressing is widening to 64-bit (RECNO64, M1-M4).** Positioning,
  navigation, the table-buffer change key, record locks, index hooks, and the CDX
  keyed-seek decoders carry 64-bit record numbers, regression-checked on x32/CNX
  (`CURSOR`) and x64/CDX (`INDEX_X64`). The classic `recno()`/`recLength()` accessors
  still saturate at `INT_MAX`, so addressing a record *past* `2^31` is not yet
  measured -- the decisive "real boy" scale test (M4-5) is still ahead. See
  `RECNO64_END_TO_END_64BIT_ADDRESSING_LANE_V1.md` and
  `SESSION_CLOSEOUT_RECNO64_M1_M4_2026-07-19.md`.
- **The engine's ceilings were raised** (development): work areas 256 -> 512, fields
  128 -> 256, x64 name ceilings 128 -> 256, plus an explicit 16 MiB record-size hard
  ceiling and 64 KiB advisory. These are deliberately round engineering choices,
  centralized in `include/xbase.hpp`, with a future configuration layer (SAP-style
  profile parameters or an `xbase.meta` descriptor) as the natural home. See
  `SESSION_CLOSEOUT_ENGINE_LIMITS_AND_CORRECTIONS_2026-07-19.md`.
- **The binary field types are now real, and custom field types went from planned to
  implemented (FIELDTYPE M1-M5).** Directly relevant to Pinocchio: Phase 1 had to
  switch the key from `SID I` to `N(8)` because `I` was a 4-byte field storing 4
  ASCII chars (max 9999) and truncated the 8-digit key. `I` is now a true 4-byte
  little-endian int32 (holds +/-2.1 B), so the boy can carry a compact `I(4)` key
  again instead of the 8-char `N(8)` workaround. `B`/`Y`/`T` are real IEEE-754 /
  int64-currency / julian+time, proven VFP-compatible both directions by an
  independent reader/writer. And the "planned custom field types" the ecosystem
  advertised are implemented: a custom type registers once and the CREATE/validation
  chain consults it (worked example: `X`=pronoun). See
  `FIELD_TYPE_CODEC_LANE_V1.md` and `SESSION_CLOSEOUT_FIELDTYPE_CODEC_2026-07-19.md`.

So the boy is growing -- more fields, more work areas, larger rows, and wider record
addressing. **Update (2026-07-20):** the record-number *identity* boundary has now
been measured -- a synthetic unit test (`dottalkpp_recno64_boundary_test`) resolves
distinct records at/past `2^31` and the legacy 32-bit accessors return `-1` at
overflow (M4-5). And the real step: a sparse-file end-to-end test
(`dottalkpp_recno64_sparse_e2e_test`) builds an x64 table with `record_count = 2^31+2`
and the engine reads distinct records at recno `2^31+1`/`+2` **off disk** (the ~18 GiB
gap is a filesystem hole). So the boy has genuinely walked past two billion -- 64-bit
record addressing is proven end-to-end. What remains is *volume*, not addressing: a
*fully populated* multi-billion-row table has not been materialized, and that is
exactly Pinocchio's scale question (Phase 1 proved 1M/5.5M; a billion-row run is the
open stretch). Real boy on addressing; still growing on sheer volume.
...
| --- | ---: | ---: | ---: |
| ENROLL 5,501,358 -- `TOP SID` (paired nav run) | 66.09 s | 0.0013 s | 50,838x |
| ENROLL 5,501,358 -- `BOTTOM SID` | 66.51 s | 0.0010 s | 66,510x |
| ENROLL 5,501,358 -- first `SKIP` | 65.9 s | 0.0015 s | 43,933x |
| ENROLL 5,501,358 -- `SKIP 1000000` | 87.1 s | 0.021 s | 4,148x |
| ENROLL 5,501,358 -- `SKIP -1000000000` to top | 529 s | 0.11 s | 4,809x |
| ENROLL 5,501,358 -- `TOP SID` (Phase 1 cross-run) | 72.169 s | 0.0013 s | 55,515x |
| STUDENTS 1,000,000 -- `TOP SID` | 19.478 s | 0.001-0.002 s | 9,739-19,478x |
| STUDENTS 1,000,000 -- `TOP LNAME` | 23.231 s | 0.001-0.002 s | 11,616-23,231x |
...
| --- | --- | --- |
| FLAT -- O(1) / O(log n) | `COUNT` (no filter, header `recCount`), `GO n` / `GO TOP` / `GO BOTTOM`, `TOP`, `BOTTOM`, `SKIP +/-N` (unfiltered CDX cursor) | Must not grow ~5.5x STUDENTS->ENROLL |
...
2. **Destructive profile (Phase 1.3b) -- DONE.** `pinocchio_scale_destructive.dts`
   ...
   surcharge quantified (linear-in-deletes, ~2.5 ms/row -- not O(n^2)).
...
5. **Polish -- `AGGS ALL` (2026-07-19) -- DONE (one confirm re-run open).**
   `AGGS ALL <expr>` computes COUNT/SUM/AVG/MIN/MAX in one scan (~4x vs the four
