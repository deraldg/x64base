# DotTalk++ / x64base Developer Manual -- Candidate Publication Rebuild

> Generated by MDO-347E at 2026-06-05T10:24:20+00:00 for validation only. Not an active publication.



<!-- MDO-347E-CANDIDATE-SECTION ordinal=1 title=Navigation, Browsing, and Search -->
...
<!-- MDO-347E-CANDIDATE-SECTION ordinal=2 title=Indexing, Tags, Relations, and Views -->
...
<!-- MDO-347E-CANDIDATE-SECTION ordinal=3 title=Expressions, Querying, and Aggregates -->
...
<!-- MDO-347E-CANDIDATE-SECTION ordinal=4 title=Messages, Errors, and Diagnostics -->
...
<!-- MDO-347E-CANDIDATE-SECTION ordinal=5 title=Command Surface, Dispatch, and Entry Variants -->
...
<!-- MDO-347E-CANDIDATE-SECTION ordinal=6 title=Command Reference Assembly, Aliases, and Generated Page Hygiene -->
...
<!-- MDO-347E-CANDIDATE-SECTION ordinal=7 title=HELP, Metadata, CMDHELPCHK, and Manualgen Alignment -->
...
<!-- MDO-347E-CANDIDATE-SECTION ordinal=8 title=Runtime Evidence, Source Verification, and Canary Closure -->
...
<!-- MDO-347E-CANDIDATE-SECTION ordinal=9 title=Promoted Draft Review, Header Normalization, and Publication Readiness -->
...
<!-- MDO-347E-CANDIDATE-SECTION ordinal=10 title=Documentation, Modeling, and Project Notes -->
...
<!-- MDO-347E-CANDIDATE-SECTION ordinal=11 title=Educational and Demo Commands -->
...
<!-- MDO-347E-CANDIDATE-SECTION ordinal=12 title=Functions and Expression Helpers -->
...
<!-- MDO-347E-CANDIDATE-SECTION ordinal=13 title=Getting Started and Session Basics -->
...
<!-- MDO-347E-CANDIDATE-SECTION ordinal=14 title=Runtime Operation, Invocation, and Automation -->
# Runtime Operation, Invocation, and Automation

## Purpose

Runtime invocation is a core Developer Manual topic for DotTalk++ / x64base. It is not a small usage note. It is part of the development, maintenance, smoke-test, and evidence contract for the system.

DotTalk++ can be run interactively, through scripts, through pipes/stdin, and through automation harnesses. Those modes are not interchangeable unless the working directory, executable source, input text, environment, transcript, and return code are all captured. A red result in one launch shape must not be generalized into a command failure or subsystem failure until invocation context is proven.
...
```text
Developer Manual:
  primary home for runtime invocation, automation, and maintenance doctrine

User Manual:
  later stable user-facing excerpts only

Student Manual:
  optional teaching/lab note later

Shared Evidence / Process:
  cross-manual runtime-smoke evidence rules
```

The Developer Manual is the central development and maintenance manual. It should define how developers launch, test, diagnose, and automate the runtime.

## Why invocation context matters

A recent manualgen/MDO lane produced a false-red diagnostic path:

```text
Python subprocess input=... from repo root:
  red historical launch shape

Manual/PowerShell tests from dottalkpp\data:
  --script mode green
  pipe/stdin QUIT green
  pipe/stdin upper + QUIT green
  Get-Content pipe with QUIT green
  interactive DO script green
```

The conclusion is not that stdin is generally broken. The conclusion is that launch context matters.

Automation harnesses must not diagnose CDX, command dispatch, DO scripts, or stdin broadly until the launch context is documented and reproduced.

## Required working directory and path context

Runtime smoke tests must record and intentionally set the working directory.

The current preferred context for automation parity testing is:

```text
D:\code\ccode\dottalkpp\data
```

or the equivalent repository-local `dottalkpp/data` path.

This matters because DotTalk++ initializes data, DBF, index, LMDB, workspace, schema, script, test, help, log, and temp roots relative to the runtime configuration and launch context.

## Executable path and freshness

Runtime smoke tests must record:

```text
executable path
executable source
copy/deploy step
build timestamp when visible
environment variables
```

A manual launch sequence may refresh the runtime executable before testing, for example by copying the Release build into the runtime bin folder. That is material evidence and must be captured.

## Invocation modes

### Interactive launcher / open runtime

Interactive runtime remains the first sanity check for command text.

Use it to answer:

```text
Does the command text itself work when typed or pasted into DotTalk++?
Does DO <script> work in an open runtime?
Does QUIT shut down cleanly?
```

Interactive success does not prove an automation harness is green, but it prevents misclassifying valid command text as bad.

### `dottalkpp.exe --script <file.dts>`

`--script` is the preferred automation candidate for non-interactive script execution when available and proven in the current runtime.

A stable smoke shape is:

```powershell
Set-Location D:\code\ccode\dottalkpp\data

& "D:\code\ccode\dottalkpp\bin\dottalkpp.exe" --script "D:\path\to\script.dts"
```

Use script files rather than large stdin blobs when the goal is repeatable automation evidence.

### PowerShell pipe / stdin

PowerShell pipe mode can be valid when the launch context is controlled.
...
"QUIT" | & "D:\code\ccode\dottalkpp\bin\dottalkpp.exe"

"upper stdin_smoke`nQUIT`n" | & "D:\code\ccode\dottalkpp\bin\dottalkpp.exe"
```

Pipe success from the correct working directory supersedes a broad claim that stdin itself is broken.

### Python subprocess stdin

Python `subprocess.run(..., input=...)` is not rejected permanently, but it is not the preferred automation path until its launch context is corrected and smoked.

A Python harness must set:

```text
cwd = dottalkpp/data
executable = freshly deployed runtime exe
input text = captured verbatim
environment = captured
stdout/stderr = captured
return code = captured
```

A Python red from repo root is a harness finding, not a command-system finding.

### File redirection / `Get-Content` pipe

File-to-pipe tests are useful smoke evidence, especially for tiny inputs such as `QUIT`.

They should be treated as a pipe/stdin variant and require the same evidence capture.

## DO scripts and QUIT behavior

Script and outer-invocation shutdown must be handled deliberately.

Document whether a script contains `QUIT`, whether the outer harness also sends `QUIT`, and whether the runtime receives EOF after QUIT.

Do not assume that these forms are equivalent:

```text
DO script_without_quit
QUIT
...
Each form should have its own smoke evidence if used by automation.

## Runtime smoke evidence requirements

Every runtime smoke must record:

```text
executable path
executable source/freshness
working directory
invocation mode
stdin text or script file path
script contents or content hash
environment variables
stdout/stderr transcript
return code
artifact paths touched
active/candidate boundary
DBF/CDX/LMDB paths if applicable
manual/MAN*/HELP/source mutation flags
```

A smoke report without these fields is incomplete.

## False-red diagnosis rules

Do not blame the command, CDX, LMDB, HELP, MAN*, or stdin subsystem until the invocation contract is known.

A valid false-red checklist is:

```text
Was the executable fresh?
Was the working directory correct?
Was the same command text proven in open runtime?
Was --script tested?
Was PowerShell pipe tested?
Was Python subprocess cwd/env captured?
Did the script contain QUIT?
Did the outer harness also send QUIT?
Were active artifacts protected?
```

## Current known status

The current runtime invocation lesson is:

```text
interactive/open runtime:
  green manual evidence

--script from dottalkpp/data:
  green manual evidence

PowerShell pipe from dottalkpp/data:
  green manual evidence

Get-Content pipe from dottalkpp/data:
  green manual evidence

Python subprocess stdin from repo root:
  red historical harness shape
```

The current CDX lesson is separate:

```text
candidate CDX files should be preserved
generated CDX ADDTAG syntax still needs correction/proof
LMDB should remain deferred until CDX tag/readback is proven
```

## Manual family placement

This contract belongs first in the Developer Manual. It may later produce stable excerpts for the User Manual and optional teaching notes for the Student Manual. It should also feed Shared Evidence / Process material so future manualgen and runtime smoke lanes do not repeat the same diagnostic drift.

## Boundary doctrine

Runtime invocation documentation does not authorize source edits, HELP edits, MAN* catalog refresh, LMDB generation, or manual publication rebuild by itself.

Each mutation lane remains explicitly gated.


<!-- MDO-347E-CANDIDATE-SECTION ordinal=15 title=HELP, Metadata, and SelfDoc -->
...
<!-- MDO-347E-CANDIDATE-SECTION ordinal=16 title=Import, Export, and Storage Bridges -->
...
<!-- MDO-347E-CANDIDATE-SECTION ordinal=17 title=Indexing, Order, and Relations -->
...
<!-- MDO-347E-CANDIDATE-SECTION ordinal=18 title=Legacy and Compatibility Surfaces -->
...
<!-- MDO-347E-CANDIDATE-SECTION ordinal=19 title=Relations and Tuple Views -->
...
<!-- MDO-347E-CANDIDATE-SECTION ordinal=20 title=Scripting and Control Flow -->
...
<!-- MDO-347E-CANDIDATE-SECTION ordinal=21 title=System, Shell, and Files -->
...
<!-- MDO-347E-CANDIDATE-SECTION ordinal=22 title=Tables, Records, and Data Editing -->
...
<!-- MDO-347E-CANDIDATE-SECTION ordinal=23 title=Tables, Records, and Data Model -->
...
<!-- MDO-347E-CANDIDATE-SECTION ordinal=24 title=Transactions, Locking, and Buffering -->
...
<!-- MDO-347E-CANDIDATE-SECTION ordinal=25 title=Workspaces, Areas, and Session State -->
...
<!-- MDO-347E-CANDIDATE-SECTION ordinal=26 title=AI Portal and Pseudo-Chat -->
<!-- DOCFLUSH-20260722-001 (AIF-068): added to mirror the primary reader body's ai_portal_and_pseudo_chat section. -->
# AI Portal and Pseudo-Chat

Status: SECTION_SKELETON_DRAFT_REPAIRED / REVIEW_REQUIRED

Purpose:
Controlled-runtime mirror of the primary reader body's "Working with AI: the Portal & Pseudo-Chat" appendix -- how AI collaboration on the project is governed, published, and answered.

## The AI Portal

An outside AI partner cannot read the maintainer's private development tree, and the GitHub governance copy moves only on full engine snapshots, so between snapshots an external partner's picture of lane state can go stale. The doc-only live portal -- *AI Agent Sync -- Live Current State* at `/docs/labtalk/agent-sync` -- carries the current governance surface (working agreement, doctrine, accepted decisions, active-track state, open questions), dated and refreshed at each maintainer-session closeout.

Two coordination surfaces exist: a partner with development-tree access reads the in-repo governance directly and exchanges work as files; a web-only partner reads the live Agent Sync page and hands work back as standalone, compile-only packets. Precedence is explicit -- the live page is fresher than the snapshot between snapshots, but the maintainer's reconciliation against the development tree is authoritative over both.

## Pseudo-Chat -- the return lane

Pseudo-Chat is the return lane that closes the loop: the page's Open questions are a tracked table, and an external partner's answer is transcribed back onto the page at closeout, flipping the matching question's status so question and answer live together on one durable page. It is called *pseudo* precisely because it is not real-time -- it moves at closeout cadence, one turn at a time, like a bulletin board that is read-by-visit and write-by-relay.

## Boundary

- controlled-runtime mirror stub, review required
- not final published manual prose
- authoritative coordination doctrine remains the primary reader appendix
- no HELP mutation
- no META mutation
- no CMDHELPCHK mutation
- no catalog apply
...
<!-- MDO-347E-CANDIDATE-SECTION ordinal=27 title=Identity, Authentication, RBAC, and Security -->
<!-- DOCFLUSH-20260722-001 (AIF-068): added to mirror the primary reader body's identity_authentication_rbac_and_security section. -->
# Identity, Authentication, RBAC, and Security

Status: SECTION_SKELETON_DRAFT_REPAIRED / REVIEW_REQUIRED

Purpose:
Controlled-runtime mirror of the primary reader body's Identity, Authentication, RBAC, and Security section -- how DotTalk++ separates identity, roles, permissions, authorization grants, session state, and runtime security policy.

## The security model

DotTalk++ separates identity, participation, roles, permissions, authorization grants, session state, and the runtime security policy. Permission establishes eligibility; a current authorization permits the specific action; the runtime security policy may still deny it. Deny takes precedence, and grants can expire, be revoked, and be audited.

## Session and authentication

The console boots unauthenticated as `member.public`. A session carries both the authenticated principal and the effective acting member. Human members authenticate with `USER LOGIN` and a locally stored salted password hash; AI and service members authenticate with owner-issued, rotatable tokens. Owner-only mutations require an authenticated owner session, and acting as another member is an explicit session operation.

## Command map

- USER: identity, authentication, RBAC, authorization, and session administration (see the USER command page).
- Inspection surfaces: `USER WHOAMI`, `USER STORE`, `USER ROLES`, `USER PERMS`, `USER CAN <permission.key> [FOR <member.key>]`.
- Administrative flow: `USER REQUEST`, `USER APPROVE`, `USER DENY`.

## Boundary

- controlled-runtime mirror stub, review required
- not final published manual prose
- complete syntax remains the USER command page
- no HELP mutation
- no META mutation
- no CMDHELPCHK mutation
- no catalog apply
...
<!-- MDO-347E-CANDIDATE-SECTION ordinal=28 title=VDISK, RAM DBF, and Transient Storage -->
<!-- DOCFLUSH-20260722-001 (AIF-068): added to mirror the primary reader body's vdisk_ram_dbf_and_transient_storage section. -->
# VDISK, RAM DBF, and Transient Storage

Status: SECTION_SKELETON_DRAFT_REPAIRED / REVIEW_REQUIRED

Purpose:
Controlled-runtime mirror of the primary reader body's VDISK, RAM DBF, and Transient Storage section -- the in-process RAM virtual disk and its ephemeral-data contract.

## What VDISK is

`VDISK` is the engine-facing control surface for DotTalk++'s in-process RAM virtual disk. Once mounted, the relocatable RAM slot can hold X64 DBF tables, native CDX-V64 indexes, and related transient storage. Ordinary database commands still operate on named files; the difference is that the file bytes live in the process-local RAM filesystem instead of a persistent host volume. A RAM DBF created under the mounted slot is the active table, not a cache over an authoritative disk copy.

## Commands and observability

- `VDISK MOUNT` / `VDISK ON` activates the RAM slot; `VDISK STATUS` reports root, mount state, bytes used, and resident files; `VDISK CONFIG` reports the optional configuration extension; `VDISK UNMOUNT` / `VDISK OFF` / `VDISK CLEAR` discard all resident RAM files.
- The default RAM root is `<DATA>/ram`; `SET PATH RAM <path>` relocates it before mounting. `CREATE X64`, `USE`, `CDX`, `SET ORDER`, and `REINDEX` use the same contracts they use on persistent storage.

## Ephemeral-data contract

The RAM filesystem is process-local and ephemeral. Unmounting, clearing, or terminating the process destroys its resident DBF, memo, CDX, and related files. That behavior is intentional and must be treated as a destructive storage operation. Do not present a RAM table as durable merely because it uses the normal DBF command surface.

## Boundary

- controlled-runtime mirror stub, review required
- not final published manual prose
- current command contract remains the VDISK command page
- no HELP mutation
- no META mutation
- no CMDHELPCHK mutation
- no catalog apply
