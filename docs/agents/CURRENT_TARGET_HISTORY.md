# Current Target -- history
    ...
    status      : historical strata, retained as evidence
    superseded  : the live pointer is docs/agents/CURRENT_TARGET.md
    split_utc   : 2026-07-31T18:50:00Z
    lane        : AIF-082 (6.5a)

Dated development-focus entries and closed objectives, newest first, moved out of
`CURRENT_TARGET.md` on 2026-07-31 so the pointer stops carrying 96 percent
history. **Nothing was deleted.** These sections are accurate records of what was
true when written; they are simply not the current target and must not be read as
such.

The repository-role statements below are superseded by
`docs/contracts/REPOSITORY_ROLE_AND_PROMOTION_CONTRACT_V1.md` and by the live
pointer. Where an old entry calls `C:\x64base` a backup or mirror, that
description is wrong and was corrected on 2026-07-14.
