...
Evidencefix revision: separates runtime/source roots, prefers system/user diagram lanes, and uses line-context matching to reduce broad prose false positives.
...
METADATA_EXPORT_JSON = "cmdhelpchk_metadata_rows.json"
...
DIAGRAM_PATTERNS = (
    "docs/generated/diagrams/**/*.drawio",
    "docs/generated/diagrams/**/*.xml",
    "tools/diagram/**/*.meta",
    "tools/diagram/**/*.drawio",
    "data/**/*.drawio",
    "user/*/diagrams/**/*.drawio",
)
...
@dataclass
class ScanResult:
    # runtime_root is the DotTalk++ runtime/dev tree: data/, docs/, help/, bin/, etc.
    # source_root is the C++ source tree: src/cli/shell_commands.cpp, include/, CMakeLists.txt.
    # They may be the same tree, but in Derald's working layout they are often siblings.
    runtime_root: Path
    source_root: Path
    generated_at: str
    ...
    def repo_root(self) -> Path:
        # Backward-compatible name used by non-source scan lanes.
...
def has_shell_registry(root: Path) -> bool:
    return (root / "src" / "cli" / "shell_commands.cpp").is_file()


def discover_source_root(runtime_root: Path) -> Path | None:
    """Find a sibling/parent C++ source tree without confusing it with runtime data root.

    This prevents the common DotTalk++ layout mistake where the scanner is run from
    dottalkpp/ while the C++ source lives one level up or in a sibling ccode/.
    """
    runtime_root = runtime_root.resolve()
    candidates: list[Path] = [runtime_root]

    for base in (runtime_root.parent, runtime_root.parent.parent):
        if base and base not in candidates:
            candidates.append(base)
        for name in ("ccode", "src", "source", "dottalkpp", "x64base"):
            c = base / name
            if c not in candidates:
                candidates.append(c)

    # Shallow sibling scan: safe and fast, but catches D:\code\ccode\ccode style trees.
    try:
    ...
    matches = [c.resolve() for c in candidates if has_shell_registry(c)]
    if not matches:
        return None
    ...
    def score(path: Path) -> tuple[int, str]:
        ...
        if (path / "include").is_dir():
            s += 5
        if (path / "src" / "cli").is_dir():
            s += 5
        if path == runtime_root:
            s += 3
        return (-s, str(path).lower())

    return sorted(dict.fromkeys(matches), key=score)[0]


def source_relpath(path: Path, result: ScanResult) -> str:
    try:
        return path.resolve().relative_to(result.source_root.resolve()).as_posix()
    except ValueError:
        return path.as_posix()
...
def scan_registry(result: ScanResult, specs: Sequence[CommandSpec]) -> None:
    ...
    if not text:
        result.warnings.append(f"Could not read source registry: {registry_path}; registry checks remain unverified.")
    for spec in specs:
        hits: list[FileHit] = []
        for key in spec.registry_keys:
            ...
            m = rx.search(text)
            if m:
                hits.append(FileHit(registry_path, source_relpath(registry_path, result), key, line_summary(text, m.start()), is_probably_legacy(registry_path)))
        hit = first_hit(hits)
        status = "OK"
        note = ""
    ...
    for spec in specs:
        ...
        for p in files:
            ...
            for handler in spec.handlers:
                ...
                m = rx.search(text)
                if m:
                    handler_hits.append(FileHit(p, source_relpath(p, result), handler, line_summary(text, m.start()), is_probably_legacy(p)))
                    break
            ...
            for block_start in [m.start() for m in re.finditer(r"@dottalk\.usage\s+v1", text, re.IGNORECASE)]:
                ...
                for alias in spec.aliases:
                    ...
                    if exact_word_regex(alias).search(normalized) and ("usage:" in normalized.lower() or "summary:" in normalized.lower()):
                        alias_match = True
                if alias_match:
                    usage_hits.append(FileHit(p, source_relpath(p, result), "@dottalk.usage v1", "command-local usage metadata block", is_probably_legacy(p)))
                    break
        ...
        for p in files:
            ...
            for m in re.finditer(r"\b(?:static\s+)?void\s+[A-Za-z0-9_]*usage[A-Za-z0-9_]*\s*\(|Usage:\\n|DotTalk\+\+ Help System", text, re.IGNORECASE):
                ...
                if any(exact_word_regex(alias).search(window) for alias in spec.aliases) or spec.command == "HELP" and "DotTalk++ Help System" in window:
                    print_hits.append(FileHit(p, source_relpath(p, result), m.group(0).replace("\n", " ")[:60], line_summary(text, m.start()), is_probably_legacy(p)))
                    break
...
def matrix_evidence(result: ScanResult, command: str, layer: str) -> Evidence | None:
    ...
    if not matches:
        return None
    priority = {"OK": 0, "NOT_APPLICABLE": 0, "PARTIAL": 1, "ROW_PRESENT_INCOMPLETE": 1, "NEEDS_REVIEW": 2, "MISS": 2, "TABLE_MISSING": 2, "KEY_MISMATCH": 2, "EXPORT_ERROR": 2, "PROBE_FAILED": 2, "TREE_PRESENT_UNREAD": 3, "UNVERIFIED": 4}
...
def exact_token_in_line(line: str, token: str, ignore_case: bool = False) -> bool:
    flags = re.IGNORECASE if ignore_case else 0
    return re.search(r"(?<![A-Za-z0-9_])" + re.escape(token) + r"(?![A-Za-z0-9_])", line, flags) is not None


def line_is_commentish(line: str) -> bool:
    stripped = line.strip()
    return stripped.startswith(("*", "//", "#"))


def command_invocation_line(line: str, alias: str) -> bool:
    stripped = line.strip()
    if not stripped or line_is_commentish(stripped):
        return False
    # DTS/log prompts often use "." or ">" before a command.
    stripped = re.sub(r"^(?:\.|>)\s*", "", stripped)
    parts = alias.split()
    if len(parts) == 1:
        return re.match(r"^" + re.escape(parts[0]) + r"(?:\s|$|[;,])", stripped, re.IGNORECASE) is not None
    pattern = r"^" + r"\s+".join(re.escape(part) for part in parts) + r"(?:\s|$|[;,])"
    return re.match(pattern, stripped, re.IGNORECASE) is not None


def help_output_line(line: str, alias: str) -> bool:
    stripped = line.strip()
    if alias == "HELP" and re.search(r"DotTalk\+\+\s+type\s+HELP\b", stripped, re.IGNORECASE):
        return True
    return re.match(r"^" + re.escape(alias) + r"\s+-\s+", stripped, re.IGNORECASE) is not None


def doc_reference_line(line: str, spec: CommandSpec, alias: str) -> bool:
    stripped = line.strip()
    if command_invocation_line(stripped, alias):
        return True
    if exact_token_in_line(stripped, spec.canonical_id, ignore_case=False):
        return True
    if " " in alias:
        # Multi-word command names are intentionally accepted in prose.
        return exact_token_in_line(stripped, alias, ignore_case=True)
    if not alias.isupper():
        return False
    if not exact_token_in_line(stripped, alias, ignore_case=False):
        return False
    # Avoid false positives such as lowercase prose "field list" or "workspace".
    context = stripped.upper()
    return (
        re.search(r"^\d+\s+DOT\s+", context) is not None
        or " CMD" in context
...
def diagram_reference_line(line: str, spec: CommandSpec, alias: str) -> bool:
    stripped = line.strip()
    if exact_token_in_line(stripped, spec.canonical_id, ignore_case=False):
        return True
    if " " in alias:
        return exact_token_in_line(stripped, alias, ignore_case=True)
    # Diagrams usually carry command labels/ids in uppercase XML values.
    return alias.isupper() and exact_token_in_line(stripped, alias, ignore_case=False)


def script_reference_line(line: str, spec: CommandSpec, alias: str) -> bool:
    return command_invocation_line(line, alias)


def runtime_reference_line(line: str, spec: CommandSpec, alias: str) -> bool:
    return command_invocation_line(line, alias) or help_output_line(line, alias)


def search_files_with_matcher(root: Path, files: Sequence[Path], spec: CommandSpec, matcher) -> list[FileHit]:
    hits: list[FileHit] = []
    for path in files:
        text = read_text_limited(path)
        if not text:
        ...
        for raw_line in text.splitlines():
            ...
            if not line:
                continue
            for alias in spec.aliases + (spec.canonical_id,):
                if matcher(line, spec, alias):
                    if len(line) > 190:
                        line = line[:187] + "..."
                    hits.append(FileHit(path, relpath(path, root), alias, line, is_probably_legacy(path)))
                    break
            if hits and hits[-1].path == path:
                break
    return hits


def first_hit_for_layer(hits: Sequence[FileHit], layer: str) -> FileHit | None:
    if not hits:
        return None
    def priority(hit: FileHit) -> tuple[int, str]:
        rel = hit.rel_path.lower().replace("\\", "/")
        score = 0
        if hit.legacy:
            score += 100
        if layer == "DIAGRAM":
            if rel.startswith("docs/generated/diagrams/"):
                score -= 30
            elif rel.startswith("user/") and "/diagrams/" in rel:
                score -= 15
            elif rel.startswith("data/scripts/scripts/"):
                score += 20
        elif layer == "DOCS":
            if rel.startswith("docs/generated/reports/"):
                score += 30
            elif rel.startswith("docs/"):
                score -= 10
        elif layer in {"TESTS", "RUNTIME_HELP"}:
...
def help_giant_command_hit(root: Path, spec: CommandSpec) -> FileHit | None:
    """Prefer the docs authority command catalog row when present.

    Generic docs scanning is intentionally conservative to avoid broad prose false
    positives. The command authority catalog, however, has a structured row form:

        [.]<n> DOT <COMMAND> <...>

    Search it directly so newly appended authority rows such as TUPLEDELTA are
    detected even if generic docs heuristics would miss them. Leading-dot numbered rows such as .397 DOT TUPLEDELTA are accepted.
    """
    candidates = [
        root / "docs" / "cases" / ". help giant.txt",
        root / "docs" / "cases" / "help_giant.txt",
    ]
    aliases = list(spec.aliases) + [spec.canonical_id]
    for path in candidates:
        ...
        for raw_line in text.splitlines():
            ...
            for alias in aliases:
                # Canonical ids may appear in generated docs, but . help giant rows
                # normally use command tokens, so prefer DOT <alias> row form.
                if re.search(r"^\.?\d+\s+DOT\s+" + re.escape(alias) + r"(?:\s|$)", line, re.IGNORECASE):
                    if len(line) > 190:
                        line = line[:187] + "..."
                    return FileHit(path, relpath(path, root), alias, line, is_probably_legacy(path))
                # Also allow unnumbered authority rows, but only with DOT prefix.
                if re.search(r"^DOT\s+" + re.escape(alias) + r"(?:\s|$)", line, re.IGNORECASE):
                    if len(line) > 190:
                        line = line[:187] + "..."
                    return FileHit(path, relpath(path, root), alias, line, is_probably_legacy(path))
    return None
...
def scan_docs_diagrams_tests_runtime(result: ScanResult, specs: Sequence[CommandSpec]) -> None:
    ...
    add_layer_presence(result, "DIAGRAM", root / "docs" / "generated" / "diagrams", "system generated diagrams directory", (root / "docs" / "generated" / "diagrams").exists())
    add_layer_presence(result, "DIAGRAM", root / "user", "user/student diagrams root", (root / "user").exists())
    add_layer_presence(result, "DIAGRAM", root / "tools" / "diagram", "diagram tool directory", (root / "tools" / "diagram").exists())
    add_layer_presence(result, "DIAGRAM", root / "data", "legacy/data diagram search directory", (root / "data").exists())
    ...
    for spec in specs:
        doc_hit = help_giant_command_hit(root, spec) or first_hit_for_layer(search_files_with_matcher(root, docs, spec, doc_reference_line), "DOCS")
        diagram_hit = first_hit_for_layer(search_files_with_matcher(root, diagrams, spec, diagram_reference_line), "DIAGRAM")
        test_hit = first_hit_for_layer(search_files_with_matcher(root, tests, spec, script_reference_line), "TESTS")
        runtime_hit = first_hit_for_layer(search_files_with_matcher(root, runtime_files, spec, runtime_reference_line), "RUNTIME_HELP")
...
METADATA_EXPORT_LAYERS: tuple[str, ...] = ("SYSCMD", "SYSHELP", "SYSARGS", "SYSENTVAR", "SYSSUBCMD", "DATA_HELP")


def normalize_export_status(value, row_found=None) -> str:
    """Normalize runtime-exported metadata evidence status.

    The exporter may emit either explicit evidence_status/status values or a
    boolean row_found. Keep the vocabulary small and report-oriented.
    """
    ...
    if raw in aliases:
        return aliases[raw]
    if raw in {"OK", "MISS", "PARTIAL", "NOT_APPLICABLE", "TABLE_MISSING", "PROBE_FAILED", "EXPORT_ERROR", "ROW_PRESENT_INCOMPLETE", "KEY_MISMATCH"}:
        return raw
...
def command_lookup_for_export(specs: Sequence[CommandSpec]) -> dict[str, CommandSpec]:
    lookup: dict[str, CommandSpec] = {}
    for spec in specs:
        keys = {spec.command, spec.canonical_id, *spec.aliases, *spec.registry_keys}
        # Also allow SETINDEX/SETORDER style keys for multi-word specs.
        keys.add(spec.command.replace(" ", ""))
        for key in keys:
            lookup[str(key).strip().upper()] = spec
    return lookup


def value_from_any(d: dict, names: Sequence[str], default=""):
    for name in names:
...
def add_export_evidence_row(result: ScanResult, spec: CommandSpec, export_path: Path, row: dict, default_layer: str | None = None) -> None:
    layer = str(value_from_any(row, ("metadata_layer", "layer", "table", "table_name"), default_layer or "")).strip().upper().replace("SYSTEM_", "SYS")
    layer_aliases = {
        "COMMANDS": "SYSCMD",
        "HELP": "SYSHELP",
        "ARGUMENTS": "SYSARGS",
        "ENTRY_VARIANTS": "SYSENTVAR",
        "SUBCOMMANDS": "SYSSUBCMD",
        "DATAHELP": "DATA_HELP",
        "COMMANDS_DBF": "DATA_HELP",
        "CMD_ARGS": "DATA_HELP",
    }
    layer = layer_aliases.get(layer, layer)
    if layer not in METADATA_EXPORT_LAYERS:
        return
    status = normalize_export_status(value_from_any(row, ("evidence_status", "status"), ""), row.get("row_found"))
    key_field = str(value_from_any(row, ("key_field", "key"), ""))
    key_value = str(value_from_any(row, ("key_value", "value", "surface_form", "command_name"), ""))
    summary_field = str(value_from_any(row, ("summary_field",), ""))
    summary_value = str(value_from_any(row, ("summary_value", "summary", "text", "caption"), ""))
    table_name = str(value_from_any(row, ("table_name", "table"), layer))
    notes = str(value_from_any(row, ("notes", "note"), ""))
    source_path = str(value_from_any(row, ("source_path", "path"), relpath(export_path, result.repo_root)))
    evidence_key = key_value or key_field or table_name
    evidence_summary = f"{table_name}: {status}"
    if key_field or key_value:
        evidence_summary += f" ({key_field}={key_value})" if key_field else f" ({key_value})"
    if summary_value:
        clean_summary = re.sub(r"\s+", " ", summary_value).strip()
        if len(clean_summary) > 140:
            clean_summary = clean_summary[:137] + "..."
        evidence_summary += f" - {clean_summary}"
    result.evidence.append(Evidence(spec.command, spec.canonical_id, layer, "runtime-mediated metadata row evidence", status, "runtime-export", source_path, evidence_key, evidence_summary, notes))


def scan_metadata_export(result: ScanResult, specs: Sequence[CommandSpec], export_path: Path | None = None) -> None:
    """Consume runtime-mediated metadata evidence, never DBF/DBT/CDX/LMDB.

    This is Phase 2 bridge behavior. The Python scanner only reads the neutral
    JSON exported by DotTalk++ runtime/DotScript/C++ helpers.
    """
    root = result.repo_root
    if export_path is None:
        export_path = root / "docs" / "generated" / "reports" / METADATA_EXPORT_JSON
    add_layer_presence(result, "METADATA_EXPORT", export_path, "runtime-mediated metadata evidence JSON", export_path.exists(), "scanner consumes this file when present; it never parses DBF rows directly")
    if not export_path.exists():
        return
    try:
        payload = json.loads(export_path.read_text(encoding="utf-8"))
    except Exception as ex:
        result.warnings.append(f"Metadata evidence export could not be parsed: {export_path}: {ex}")
        return

    lookup = command_lookup_for_export(specs)

    # Flat export rows: evidence_rows[]
    flat_rows = payload.get("evidence_rows") if isinstance(payload, dict) else None
    if isinstance(flat_rows, list):
        for row in flat_rows:
            if not isinstance(row, dict):
                continue
            key = str(value_from_any(row, ("canonical_id", "command_id", "command_name", "surface_form", "registry_key"), "")).strip().upper()
            spec = lookup.get(key)
            if spec:
                add_export_evidence_row(result, spec, export_path, row)

    # Nested command export: commands[].metadata.<layer>
    commands = payload.get("commands") if isinstance(payload, dict) else None
    if isinstance(commands, list):
        for cmd in commands:
            if not isinstance(cmd, dict):
                continue
            key = str(value_from_any(cmd, ("canonical_id", "command_id", "command_name", "surface_form", "registry_key"), "")).strip().upper()
            spec = lookup.get(key)
            if not spec:
                continue
            ...
            if not isinstance(metadata, dict):
                continue
            for layer, row in metadata.items():
                if isinstance(row, dict):
                    add_export_evidence_row(result, spec, export_path, row, default_layer=layer)

def scan_repo(runtime_root: Path, source_root: Path | None = None, tree_file: Path | None = None) -> ScanResult:
    tree_text = ""
    ...
    if source_root is None:
        discovered = discover_source_root(runtime_root)
        source_root = discovered or runtime_root
    source_root = source_root.resolve()

    result = ScanResult(runtime_root, source_root, datetime.now(timezone.utc).replace(microsecond=0).isoformat(), tree_file.resolve() if tree_file else None, tree_text)
    specs = build_command_specs()

    if discovered and discovered != runtime_root:
        result.warnings.append(f"Auto-discovered separate source root: {discovered}")
    if not has_shell_registry(source_root):
        result.warnings.append(f"Source root does not contain src/cli/shell_commands.cpp: {source_root}")
    if not (runtime_root / "data").exists():
        result.warnings.append(f"Runtime root does not contain data/: {runtime_root}")

    scan_registry(result, specs)
    scan_handlers_and_usage(result, specs)
    scan_metadata(result, specs)
    scan_data_help(result, specs)
    scan_metadata_export(result, specs)
    ...
    if not (runtime_root / "data").exists() and tree_text:
        result.warnings.append("Runtime root appears to be source-only; uploaded tree lists data/docs assets, but their content was not available for row/doc verification.")
    return result
...
def compact_symbol(ev: Evidence | None) -> str:
    ...
    if ev.status == "NOT_APPLICABLE":
        return "N/A"
    if ev.status == "PARTIAL" or ev.status == "ROW_PRESENT_INCOMPLETE":
        return "PART"
    if ev.status == "TREE_PRESENT_UNREAD":
        return "TREE"
    if ev.status == "UNVERIFIED":
        return "?"
    if ev.status in {"EXPORT_ERROR", "PROBE_FAILED"}:
        return "ERR"
    if ev.status in {"TABLE_MISSING", "KEY_MISMATCH"}:
        return "MISS"
    if ev.status.startswith("MISSING") or ev.status.endswith("MISSING") or ev.status in {"DOCS_MISSING", "DIAGRAM_MISSING", "TEST_COVERAGE_MISSING", "MISS"}:
        return "MISS"
...
def write_markdown(result: ScanResult, output_path: Path, csv_path: Path) -> None:
    ...
    lines.append(f"- Runtime/data/docs root scanned: `{result.runtime_root}`")
    lines.append(f"- Source root scanned: `{result.source_root}`")
    if result.tree_file:
    ...
    lines.append(f"- Metadata evidence export consumed when present: `{result.runtime_root / 'docs' / 'generated' / 'reports' / METADATA_EXPORT_JSON}`")
    ...
    lines.append("")
    lines.append("Legend: `OK` exact static evidence; `MISS` expected item missing in an available layer; `N/A` not applicable; `ERR` export/probe error; `TREE` listed in uploaded tree but unread; `?` unverified; `SCR` script/test evidence; `MAN` manual/session evidence; `PART` partial/review.")
    lines.append("")
...
def parse_args(argv: Sequence[str]) -> argparse.Namespace:
    ...
    parser.add_argument("--runtime-root", type=Path, default=None, help="DotTalk++ runtime/dev root containing data/, docs/, help/, bin/. Defaults to current working directory.")
    parser.add_argument("--source-root", type=Path, default=None, help="C++ source root containing src/cli/shell_commands.cpp. Auto-discovered when omitted.")
    parser.add_argument("--repo", type=Path, default=None, help="Backward-compatible alias for --runtime-root.")
    ...
    parser.add_argument("--out-dir", type=Path, default=None, help="Directory for generated report files; defaults to <runtime-root>/docs/generated/reports.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    runtime_root = (args.runtime_root or args.repo or Path.cwd()).resolve()
    source_root = args.source_root.resolve() if args.source_root else None
    if not runtime_root.exists() or not runtime_root.is_dir():
        print(f"ERROR: runtime root does not exist or is not a directory: {runtime_root}", file=sys.stderr)
        return 2
    if source_root and (not source_root.exists() or not source_root.is_dir()):
    ...
    if args.tree_file and not args.tree_file.exists():
        ...
        return 2
    out_dir = args.out_dir.resolve() if args.out_dir else runtime_root / "docs" / "generated" / "reports"
    md_path = out_dir / REPORT_MD
    csv_path = out_dir / REPORT_CSV
    result = scan_repo(runtime_root, source_root, args.tree_file)
