#!/usr/bin/env python3
"""Review PHASE23LS COUNT + SMARTLIST n + TUPLE CMDHELP locale readback transcript."""
...
GREEN = "PHASE23LS_CMDHELP_LOCALE_COUNT_SMARTLIST_TUPLE_READBACK_PROOF_GREEN"
REVIEW_REQUIRED = "PHASE23LS_CMDHELP_LOCALE_COUNT_SMARTLIST_TUPLE_READBACK_REVIEW_REQUIRED"
...
def main() -> int:
    ...
    transcript = candidate_dir / "transcripts" / "phase23ls_cmdhelp_locale_count_smartlist_tuple_probe_transcript.txt"
    dts = candidate_dir / "runtime" / "phase23ls_cmdhelp_locale_count_smartlist_tuple_probe.dts"
    ...
    transcript_markers_ok = int("PHASE23LS_DOTSCRIPT_START" in text and "PHASE23LS_DOTSCRIPT_END" in text)
    contract_marker_ok = int("PHASE23LS_COUNT_SMARTLIST_N_TUPLE_CONTRACT" in text)
    no_list_contract_marker_ok = int("PHASE23LS_NO_LIST_NO_SMARTLIST_ALL" in text)
    dts_extension_ok = int(dts.exists() and dts.suffix.lower() == ".dts")
    path_reset_ok = int(
        "PHASE23LS_PATH_RESET_TO_DEFAULT_DATA_ROOTS" in text
        and re.search(r"SETPATH:\s*LMDB\s*=\s*.*dottalkpp\\data\\lmdb", text, re.IGNORECASE) is not None
    )

    table_markers_ok = int(all(f"PHASE23LS_READBACK_{t}" in text for t in TABLES))
    tuple_markers_ok = int(all(f"PHASE23LS_TUPLE_{t}_TOP_VALUES_ONLY" in text for t in TABLES))
    compact_tuple_markers_ok = int(all(f"PHASE23LS_TUPLE_{t}_COMPACT" in text for t in TABLES))
    ...
    # TUPLE * --VALUES-ONLY deliberately emits a compact concatenated value row, not a pipe-delimited row.
    # Count the actual deterministic values-only rows instead of requiring pipe separators.
    tuple_values_row_count = count(r"(?m)^PHASE23J-", text, flags=0)
    compact_tuple_row_count = count(r"(?m)^DOT\|(?:ABOUT|AREA).*(?:SOURCE_CANONICAL|DRAFT_PLACEHOLDER)", text, flags=0)
    ...
    next_gate = "HOLD_OR_AUTHORIZE_PHASE23M_HELP_LOCALE_ACTIVE_PROMOTION_PLAN" if status == GREEN else "FIX_OR_RERUN_PHASE23LS_DOTSCRIPT"
    ...
    (report_dir / "phase23ls_review_result.json").write_text(json.dumps(result, indent=2), encoding="utf-8")
