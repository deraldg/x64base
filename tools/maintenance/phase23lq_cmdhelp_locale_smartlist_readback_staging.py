#!/usr/bin/env python3
"""Stage PHASE23LQ CMDHELP locale SMARTLIST readback repair artifacts.

PHASE23LQ repairs PHASE23L's proof contract by replacing LIST/LIST ALL with
SMARTLIST ALL. The phase remains read-only: it opens the PHASE23K candidate HELP
locale companion tables after PHASE23KR repaired their CDX/LMDB tags, proves
TOPICKEY ordered readback through LMDB, and does not mutate active HELP,
CMDHELP, CMDHELPCHK, source, MAINT, or BBOX.
"""
...
STATUS = "PHASE23LQ_CMDHELP_LOCALE_SMARTLIST_READBACK_STAGING_GREEN_MANUAL_DOTSCRIPT_REQUIRED"
...
def main() -> int:
    ...
    runtime_dir = q_dir / "runtime"
    transcript_dir = q_dir / "transcripts"
    ...
    if missing:
        print("PHASE23LQ_CMDHELP_LOCALE_SMARTLIST_READBACK_STAGING_BLOCKED_MISSING_CANDIDATE_ARTIFACTS")
    ...
    dts = runtime_dir / "phase23lq_cmdhelp_locale_smartlist_readback_probe.dts"
    transcript = transcript_dir / "phase23lq_cmdhelp_locale_smartlist_readback_probe_transcript.txt"

    lines: list[str] = []
    lines.extend([
        "ECHO ON",
        "SET PAGING OFF",
        "ECHO PHASE23LQ_DOTSCRIPT_START",
        "ECHO PHASE23LQ_SCOPE_READONLY_CANDIDATE_TABLES_ONLY",
        "ECHO PHASE23LQ_SMARTLIST_CONTRACT",
        f"ECHO PHASE23LQ_SAMPLE_LOCALE_{SAMPLE_LOCALE}",
    ...
    for topic in SAMPLE_TOPICS:
        ...
        lines.append(f"ECHO PHASE23LQ_SAMPLE_TOPIC_{safe}_{SAMPLE_LOCALE}")
    for table in TABLES:
        lines.extend([
            f"ECHO PHASE23LQ_READBACK_{table}",
            f"SELECT {table}",
            "AREA",
    ...
    manifest = {
        ...
        "readback_command": "SMARTLIST ALL",
        "lookup_contract": "TOPICKEY + LOCALE_ID rows visible through candidate CDX/LMDB ordered SMARTLIST readback",
        ...
        "next_gate": "HOLD_OR_RUN_PHASE23LQ_DOTSCRIPT_AND_REVIEW_TRANSCRIPT",
    }
    (reports_dir / "phase23lq_staging_manifest.json").write_text(json.dumps(manifest, indent=2), encoding="utf-8")
    ...
    print(f"readback_command: SMARTLIST ALL")
    ...
    print("next_gate: HOLD_OR_RUN_PHASE23LQ_DOTSCRIPT_AND_REVIEW_TRANSCRIPT")
    return 0
