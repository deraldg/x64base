...
def _run_id_from_path(path: Path) -> str | None:
    for part in path.parts:
        if re.fullmatch(r"DOCFLUSH-\d{8}-\d{3}", part):
            return part
    return None
...
def build(
    repo: Path,
    *,
    run_id: str = "DOCFLUSH-20260716-001",
    authority_path: Path | None = None,
    reference_inventory: Path | None = None,
    sysfunc_candidate: Path | None = None,
    sysargs_candidate: Path | None = None,
) -> tuple[list[dict[str, str]], list[dict[str, str]], dict[str, object]]:
    authority_path = authority_path or repo / "selfdoc/reference_identity_authority_v1.json"
    authority_path = _resolve(repo, authority_path)
    authority = json.loads(authority_path.read_text(encoding="utf-8"))
    inputs = {row["evidence_id"]: repo / row["path"] for row in authority["evidence_inputs"]}
    if reference_inventory is not None:
        inputs["joined_reference_inventory"] = _resolve(repo, reference_inventory)
    if sysfunc_candidate is not None:
        inputs["sysfunc_candidate"] = _resolve(repo, sysfunc_candidate)
    if sysargs_candidate is not None:
        inputs["sysargs_candidate"] = _resolve(repo, sysargs_candidate)
    ...
    manifest = {
        "schema_version": "reference_authority_crosswalk_v1",
        "run_id": run_id,
        "report_only": True,
        "contract": contract,
        "authority_map": str(authority_path.relative_to(repo)).replace("\\", "/"),
        "authority_map_sha256": map_hash,
        "input_paths": {
        ...
        "validation_counts": dict(sorted(Counter(row["validation_state"] for row in rows).items())),
        "excluded_reference_topics": len(excluded),
...
def main() -> int:
    ...
    parser.add_argument("--authority-map", type=Path)
    parser.add_argument("--reference-inventory", type=Path)
    parser.add_argument("--sysfunc-candidate", type=Path)
    parser.add_argument("--sysargs-candidate", type=Path)
    ...
    run_id = args.run_id or _run_id_from_path(out)
    if not run_id:
        parser.error("--run-id is required when it cannot be inferred from --output-dir")
    rows, excluded, manifest = build(
        repo,
        run_id=run_id,
        authority_path=args.authority_map,
        reference_inventory=args.reference_inventory,
        sysfunc_candidate=args.sysfunc_candidate,
        sysargs_candidate=args.sysargs_candidate,
