...
class PinocchioBenchmarkValidatorTests(unittest.TestCase):
    ...
    def test_attested_binding_requires_evidence_and_honesty_note(self) -> None:
        profile = {
            "machine_type": "lab-a",
            "cpu": {"name": "test cpu", "threads_total": 8},
            "memory": {"os_visible_gib": 16},
            "os": "test os",
            "historical_run_binding": "MAINTAINER_ATTESTED",
        }
        findings = MOD.validate_profile(profile)
        self.assertIn("PROFILE_MAINTAINER_ATTESTATION_MISSING", findings)
        self.assertIn(
            "PROFILE_MAINTAINER_ATTESTATION_HONESTY_NOTE_MISSING",
            findings,
        )
