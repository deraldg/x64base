param(
    [Parameter(Mandatory=$true)]
    [string]$RepoRoot,

    [Parameter(Mandatory=$false)]
    [string]$PythonExe = "",

    [switch]$AllowActiveCatalogMutation,

    [switch]$AllowAlreadyPromotedNoop
)

Set-StrictMode -Version Latest
$ErrorActionPreference = "Stop"

if (-not $AllowActiveCatalogMutation) {
    throw "Refusing Phase 22AE active catalog replacement without -AllowActiveCatalogMutation"
}

if (-not $PythonExe) {
    if ($env:PYTHON_EXE) { $PythonExe = $env:PYTHON_EXE }
    elseif (Get-Command py -ErrorAction SilentlyContinue) { $PythonExe = "py" }
    elseif (Get-Command python -ErrorAction SilentlyContinue) { $PythonExe = "python" }
    else { throw "No Python runtime found. Pass -PythonExe `$py12 or set PYTHON_EXE." }
}

$script = Join-Path $RepoRoot "tools\messaging\execute_message_catalog_phase22ae_active_catalog_replacement.py"
$argsList = @("--repo-root", $RepoRoot, "--allow-active-catalog-mutation")
if ($AllowAlreadyPromotedNoop) {
    $argsList += "--allow-already-promoted-noop"
}

if ($PythonExe -eq "py") {
    & py -3.12 $script @argsList
} else {
    & $PythonExe $script @argsList
}
exit $LASTEXITCODE
