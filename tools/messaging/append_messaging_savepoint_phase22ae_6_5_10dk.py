...
SUMMARY_REL='docs/messaging/apply/phase22ae_6_5_10dk_runtime_proof_evidence_decision_package_v1/phase22ae_6_5_10dk_summary_v1.json'
...
def main():
    ...
    sp=repo/SUMMARY_REL
    if not sp.exists(): raise SystemExit(f'10DK summary not found: {sp}')
    summary=json.loads(sp.read_text(encoding='utf-8'))
    if summary.get('status') != STATUS: raise SystemExit(f'10DK summary is not green: {summary.get("status")}')
    ts=now()
    body='\n'.join([f'## {PHASE}','',f'- timestamp_utc: {ts}',f'- status: {STATUS}',f'- summary: {SUMMARY_REL}',f'- next_gate: {NEXT_GATE}','- boundary: transcript-only evidence decision staged; access-violation crash held; runtime proof/reuse/apply not accepted; protected-system mutation 0',''])
