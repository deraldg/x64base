...
$script = Join-Path $RepoRoot "tools\messaging\review_message_catalog_phase22ae_6_5_10r_rollback_and_failure.py"
$argsList = @("--repo-root", $RepoRoot)
if ($RollbackRuntimeLog) { $argsList += @("--rollback-runtime-log", $RollbackRuntimeLog) }
