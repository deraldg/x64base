...
def main():
    ...
    ts=now_iso()
    entry=f"\n## {PHASE} - {ts}\n\n- status: {STATUS}\n- next_gate: {NEXT_GATE}\n- boundary: source-held/report-only package; shutdown-isolation probe scripts staged; runtime proof/reuse/apply/source mutations remain blocked.\n"
    ...
    with index.open('a',newline='',encoding='utf-8') as f:
        fields=['phase','status','next_gate','timestamp_utc','entry_sha256']; w=csv.DictWriter(f,fieldnames=fields)
