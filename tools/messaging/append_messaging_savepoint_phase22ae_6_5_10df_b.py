...
SAVEPOINT = "MSG-022AE.6.5.10DF-B"
...
def summary(repo: Path) -> dict:
    path = repo / "docs/messaging/reports/message_catalog_phase22ae_6_5_10df_b_status_summary_v1.csv"
    try:
...
def main() -> int:
    ...
    if not args.accept_side_branch_savepoint:
        print("[MSG-022AE.6.5.10DF-B] Refusing without --accept-side-branch-savepoint.")
        return 1
    if status != GREEN:
        print(f"[MSG-022AE.6.5.10DF-B] Status is not green: {status}")
        return 1

    journal = repo / "docs/messaging/MESSAGING_SAVEPOINT_JOURNAL.md"
    old = read_text(journal)
    if SAVEPOINT in old:
        print("[MSG-022AE.6.5.10DF-B] Side-branch savepoint already appears in journal; refusing duplicate append.")
        return 0
...
- Target selection plan created: {row.get('TARGET_SELECTION_PLAN_CREATED','')}
- Ranked review candidates: {row.get('RANKED_REVIEW_CANDIDATES','')}
- HELP DATA review candidates: {row.get('HELP_DATA_REVIEW_CANDIDATES','')}
- CMDHELPCHK review candidates: {row.get('CMDHELPCHK_REVIEW_CANDIDATES','')}
...
- Latest pointer changed by DF-B: {row.get('LATEST_POINTER_CHANGED_BY_DF_B','')}
- Next gate: {row.get('NEXT_GATE','')}
"""
    write_text(journal, old.rstrip() + "\n\n" + entry.lstrip())
    print("[MSG-022AE.6.5.10DF-B] Side-branch savepoint appended.")
