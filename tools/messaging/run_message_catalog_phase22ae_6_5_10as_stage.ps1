param(
    ...
    [switch]$ReplaceExistingScript,

    [switch]$ReplaceExistingWorkspaceProof
)
...
$script = Join-Path $RepoRoot "tools\messaging\stage_message_catalog_phase22ae_6_5_10as_workspace_profile.py"
$argsList = @("--repo-root", $RepoRoot)
if ($ReplaceExistingScript) { $argsList += "--replace-existing-script" }
if ($ReplaceExistingWorkspaceProof) { $argsList += "--replace-existing-workspace-proof" }
