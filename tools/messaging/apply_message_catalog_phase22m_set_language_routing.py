...
STATUS_GREEN = "MESSAGE_CATALOG_PHASE22M_LOW_RISK_SET_LANGUAGE_RUNTIME_ROUTING_PATCH_APPLIED"
STATUS_BLOCKED = "MESSAGE_CATALOG_PHASE22M_LOW_RISK_SET_LANGUAGE_RUNTIME_ROUTING_PATCH_BLOCKED"
NEXT_GATE = "BUILD_AND_RUN_PHASE22M_SET_LANGUAGE_ROUTING_SMOKE_THEN_VALIDATE"
REPORT_DIR = Path("docs/messaging/reports")
BACKUP_ROOT_BASE = Path("docs/messaging/backups")

ROUTE_BEGIN = "    // MSG-022M BEGIN SET LANGUAGE active provider routing"
ROUTE_END = "    // MSG-022M END SET LANGUAGE active provider routing"
...
def patch_cmd_set(text: str):
    ...
    if replaced is not None:
        actions.append({
            ...
            "DETAIL": "refreshed SET LANGUAGE active-provider routing block",
        })
    ...
    for anchor in anchors:
        patched = insert_after_anchor(text, anchor, ROUTE_BLOCK)
        if patched is not None:
            actions.append({
                "ACTION": "INSERT_ROUTE_BLOCK",
                "DETAIL": f"inserted after anchor: {anchor.strip()}",
            })
            return patched, actions

    raise RuntimeError("SET LANGUAGE locale assignment anchor not found: S.message_locale = normalized;")
...
def main():
    ...
    p22l = first_row(reports / "message_catalog_phase22l_status_summary_v1.csv")
    messages = p22l.get("MESSAGES", "12")
    text_rows = p22l.get("TEXT_ROWS", "60")
    locales = p22l.get("LOCALES", "de;en-US;es;fr;it")
    ...
    gate("PHASE22L_ROUTING_PLAN_GREEN",
         p22l.get("STATUS") == "MESSAGE_CATALOG_PHASE22L_LOW_RISK_RUNTIME_MESSAGE_ROUTING_PLAN_GREEN_SOURCE_HELD",
         p22l.get("STATUS", ""))
    gate("SELECTED_SYMBOL_MESSAGE_LOCALE_SET",
         p22l.get("SELECTED_SYMBOL") == "MESSAGE_LOCALE_SET",
         p22l.get("SELECTED_SYMBOL", ""))
    ...
    if failures == 0:
        try:
            backup_root = repo / BACKUP_ROOT_BASE / f"MSG-022M_SET_LANGUAGE_ROUTING_BACKUP_{datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')}"
            ...
            mutation_rows.append({
                ...
                "DETAIL": "routed SET LANGUAGE/SET LOCALE status output through active provider symbol MESSAGE_LOCALE_SET with compiled fallback preserved below block",
            })
            status = STATUS_GREEN
    ...
    write_csv(reports / "message_catalog_phase22m_status_summary_v1.csv", [{
        "STATUS": status,
        "MESSAGES": messages,
        "TEXT_ROWS": text_rows,
        "LOCALES": locales,
        ...
        "SET_LANGUAGE_ROUTING_PATCH_APPLIED": 1 if status == STATUS_GREEN else 0,
        "BUILD_EXECUTED": 0,
        ...
        "REPORT_TIMESTAMP_UTC": datetime.now(timezone.utc).replace(microsecond=0).isoformat().replace("+00:00", "Z"),
         ...
         "ROUTED_SYMBOL", "SET_LANGUAGE_ROUTING_PATCH_APPLIED", "BUILD_EXECUTED",
         "RUNTIME_SMOKE_EXECUTED", "ERRORS", "NEXT_GATE", "REPORT_TIMESTAMP_UTC"])

    write_csv(reports / "message_catalog_phase22m_gate_check_v1.csv",
              gates,
              ["GATE", "STATUS", "DETAIL"])
    write_csv(reports / "message_catalog_phase22m_patch_actions_v1.csv",
              action_rows,
              ["TARGET_PATH", "ACTION", "DETAIL"])
    write_csv(reports / "message_catalog_phase22m_source_mutation_inventory_v1.csv",
              mutation_rows,
              ["TARGET_PATH", "ACTION", "BYTES", "SHA256", "DETAIL"])
    write_csv(reports / "message_catalog_phase22m_source_backup_inventory_v1.csv",
              backup_rows,
              ["TARGET_PATH", "BACKUP_PATH", "BYTES", "SHA256", "ROLE"])

    boundary = [
        {"PROTECTED_SYSTEM": "SOURCE_CODE", "MUTATION_ALLOWED": 1, "OBSERVED_MUTATION": len(mutation_rows), "DETAIL": "Authorized source mutation limited to src/cli/cmd_set.cpp SET LANGUAGE/SET LOCALE status routing."},
    ...
    write_csv(reports / "message_catalog_phase22m_boundary_ledger_v1.csv",
              boundary,
              ["PROTECTED_SYSTEM", "MUTATION_ALLOWED", "OBSERVED_MUTATION", "DETAIL"])

    smoke = repo / "docs/messaging/scripts/MESSAGE_CATALOG_PHASE22M_SET_LANGUAGE_ROUTING_SMOKE.dts"
    smoke.parent.mkdir(parents=True, exist_ok=True)
    smoke.write_text("\n".join([
        "* MESSAGE_CATALOG_PHASE22M_SET_LANGUAGE_ROUTING_SMOKE.dts",
        "* First real low-risk runtime routing smoke for SET LANGUAGE status.",
        "SET LANGUAGE es",
