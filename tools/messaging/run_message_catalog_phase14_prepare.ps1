...
if (-not $AllowInactiveCandidateCdxExecution) {
    throw "Refusing Phase 14 staging without -AllowInactiveCandidateCdxExecution"
}
...
$script = Join-Path $RepoRoot "tools\messaging\prepare_message_catalog_phase14_cdx_execution.py"
if ($PythonExe -eq "py") {
