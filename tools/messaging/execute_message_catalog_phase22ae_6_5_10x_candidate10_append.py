...
PACKAGE_ROOT = Path("docs/messaging/apply/phase22ae_6_5_10x_candidate10_text_append_micro_proof_v1")
SCRIPT_PATH = Path("docs/messaging/scripts/MESSAGE_CATALOG_PHASE22AE_6_5_10X_CANDIDATE10_TEXT_APPEND_MICRO_PROOF.dts")
RUNLOG_PATH = Path("docs/messaging/runlog/MSG-022AE_6_5_10X_CANDIDATE10_TEXT_APPEND_MICRO_PROOF.md")

PLAN_ROOT = Path("docs/messaging/apply/phase22ae_6_5_10w_candidate10_text_extension_micro_proof_plan_v1")
CANDIDATE10_CSV = PLAN_ROOT / "import/system_message_text_candidate10_append_only.csv"
FULL70_REFERENCE_CSV = PLAN_ROOT / "import/system_message_text_baseline60_plus_candidate10_reference.csv"
...
STATUS_PREPARED = "MESSAGE_CATALOG_PHASE22AE_6_5_10X_CANDIDATE10_TEXT_APPEND_MICRO_PROOF_PREPARED"
STATUS_GREEN = "MESSAGE_CATALOG_PHASE22AE_6_5_10X_CANDIDATE10_TEXT_APPEND_MICRO_PROOF_GREEN_RESTORE_REQUIRED"
STATUS_BLOCKED = "MESSAGE_CATALOG_PHASE22AE_6_5_10X_CANDIDATE10_TEXT_APPEND_MICRO_PROOF_BLOCKED_RESTORE_REQUIRED"
STATUS_RESTORED_GREEN = "MESSAGE_CATALOG_PHASE22AE_6_5_10X_CANDIDATE10_TEXT_APPEND_MICRO_PROOF_PROVEN_AND_RESTORED"
STATUS_RESTORED_BLOCKED = "MESSAGE_CATALOG_PHASE22AE_6_5_10X_CANDIDATE10_TEXT_APPEND_MICRO_PROOF_FAILED_BUT_RESTORED"

NEXT_PREPARED = "RUN_PHASE22AE_6_5_10X_CANDIDATE10_TEXT_APPEND_MICRO_PROOF_THEN_FINALIZE"
NEXT_FINALIZED = "RESTORE_PHASE22AE_6_5_10X_ACTIVE_TEXT_BACKUP_BEFORE_SAVEPOINT"
NEXT_RESTORED_GREEN = "HOLD_OR_AUTHORIZE_PHASE22AE_6_5_10Y_CANDIDATE10_RESULT_CLASSIFICATION"
NEXT_RESTORED_BLOCKED = "HOLD_AND_REVIEW_PHASE22AE_6_5_10X_CANDIDATE10_APPEND_FAILURE"
...
def prepare(repo: Path, args):
    ...
    w = first_row(reports / "message_catalog_phase22ae_6_5_10w_status_summary_v1.csv")
    sp, latest = savepoint_present(repo, "MSG-022AE.6.5.10W")
    ...
    gate("ALLOW_CANDIDATE10_APPEND_FLAG", args.allow_candidate10_append_micro_proof, args.allow_candidate10_append_micro_proof)
    gate("PHASE22AE_6_5_10W_GREEN", w.get("STATUS") == "MESSAGE_CATALOG_PHASE22AE_6_5_10W_CANDIDATE10_TEXT_EXTENSION_MICRO_PROOF_PLAN_GREEN_SOURCE_HELD", w.get("STATUS", "missing"))
    gate("MSG_022AE_6_5_10W_SAVEPOINT_PRESENT", sp, latest)
    ...
    gate("CANDIDATE10_CSV_HAS_10_ROWS", len(read_csv(repo / CANDIDATE10_CSV)) == 10, len(read_csv(repo / CANDIDATE10_CSV)))
    ...
    write_csv(reports / "message_catalog_phase22ae_6_5_10x_active_text_fingerprint_before_v1.csv", before, ["ROLE","PATH","EXISTS","KIND","BYTES","SHA256","FILES"])

    status = "MESSAGE_CATALOG_PHASE22AE_6_5_10X_CANDIDATE10_TEXT_APPEND_MICRO_PROOF_PREPARE_BLOCKED"
    backup_root_rel = ""
    script_rel = ""
    backup_rows = []
    ...
    if failures == 0:
        ...
        shutil.copy2(repo / CANDIDATE10_CSV, pkg_root / "import/system_message_text_candidate10_append_only.csv")
        if (repo / FULL70_REFERENCE_CSV).exists():
            shutil.copy2(repo / FULL70_REFERENCE_CSV, pkg_root / "import/system_message_text_expected_full70_reference.csv")

        stamp = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
        backup_root = repo / "docs/messaging/backups" / f"MSG-022AE_6_5_10X_CANDIDATE10_TEXT_APPEND_BACKUP_{stamp}"
        ...
        script.write_text("\n".join([
            "* MESSAGE_CATALOG_PHASE22AE_6_5_10X_CANDIDATE10_TEXT_APPEND_MICRO_PROOF.dts",
            "* ACTIVE TEXT-ONLY CANDIDATE10 APPEND MICRO PROOF - diagnostic only.",
            "* No QUIT here; quit manually in interactive runs.",
            f"USE {(repo / ACTIVE_TEXT_DBF).resolve().as_posix()}",
            "COUNT",
            f"IMPORT {(pkg_root / 'import/system_message_text_candidate10_append_only.csv').resolve().as_posix()}",
            f"USE {(repo / ACTIVE_TEXT_DBF).resolve().as_posix()}",
            "COUNT",
            "LIST ALL",
            "",
    ...
    write_csv(reports / "message_catalog_phase22ae_6_5_10x_prepare_gate_check_v1.csv", gates, ["GATE","STATUS","DETAIL"])
    write_csv(reports / "message_catalog_phase22ae_6_5_10x_backup_manifest_v1.csv", backup_rows, ["ROLE","ORIGINAL_PATH","BACKUP_PATH","EXISTS_AT_BACKUP","KIND","BYTES","SHA256","RESTORE_POLICY"])
    write_csv(reports / "message_catalog_phase22ae_6_5_10x_prepare_boundary_ledger_v1.csv", [
    ...
    write_csv(reports / "message_catalog_phase22ae_6_5_10x_prepare_status_summary_v1.csv", [{
        "STATUS": status,
        "VALIDATION_ISSUES": val,
        "PHASE22AE_6_5_10W_STATUS": w.get("STATUS",""),
        "MSG_022AE_6_5_10W_SAVEPOINT_PRESENT": 1 if sp else 0,
        "DOTTALKPP_PROCESS_RUNNING": 1 if running else 0,
        "ALLOW_CANDIDATE10_APPEND_MICRO_PROOF": 1 if args.allow_candidate10_append_micro_proof else 0,
        ...
        "CANDIDATE10_ROWS": len(read_csv(pkg_root / "import/system_message_text_candidate10_append_only.csv")) if pkg_root.exists() else "",
    ...
    }], ["STATUS","VALIDATION_ISSUES","PHASE22AE_6_5_10W_STATUS","MSG_022AE_6_5_10W_SAVEPOINT_PRESENT","DOTTALKPP_PROCESS_RUNNING","ALLOW_CANDIDATE10_APPEND_MICRO_PROOF","PACKAGE_ROOT","SCRIPT_PATH","RUNLOG_PATH","BACKUP_ROOT","BACKUP_ROWS","CANDIDATE10_ROWS","ACTIVE_TEXT_HEADER_COUNT_BEFORE","SHOULD_EXECUTE_RUNTIME","ACTIVE_TEXT_MUTATION_OBSERVED","SOURCE_FILES_MUTATED","NEXT_GATE","REPORT_TIMESTAMP_UTC"])

    print(status)
    print(f"  validation issues: {val}")
    print(f"  Phase 22AE.6.5.10W green: {1 if w.get('STATUS') == 'MESSAGE_CATALOG_PHASE22AE_6_5_10W_CANDIDATE10_TEXT_EXTENSION_MICRO_PROOF_PLAN_GREEN_SOURCE_HELD' else 0}")
    print(f"  MSG-022AE.6.5.10W savepoint present: {1 if sp else 0}")
    print(f"  dottalkpp process running: {1 if running else 0}")
    print(f"  allow candidate10 append micro proof: {1 if args.allow_candidate10_append_micro_proof else 0}")
    ...
    print(f"  candidate10 rows: {len(read_csv(repo / CANDIDATE10_CSV))}")
...
def finalize(repo: Path, args):
    reports = repo / REPORT_DIR
    prep = first_row(reports / "message_catalog_phase22ae_6_5_10x_prepare_status_summary_v1.csv")
    ...
    imported10 = "IMPORTED 10 RECORDS" in up
    ...
    gate("IMPORTED_10_SIGNAL", imported10, "candidate10 import")
    ...
    before = read_csv(reports / "message_catalog_phase22ae_6_5_10x_active_text_fingerprint_before_v1.csv")
    ...
    write_csv(reports / "message_catalog_phase22ae_6_5_10x_finalize_gate_check_v1.csv", gates, ["GATE","STATUS","DETAIL"])
    write_csv(reports / "message_catalog_phase22ae_6_5_10x_active_text_fingerprint_after_append_v1.csv", after, ["ROLE","PATH","EXISTS","KIND","BYTES","SHA256","FILES"])
    write_csv(reports / "message_catalog_phase22ae_6_5_10x_active_text_fingerprint_delta_append_v1.csv", delta, ["ROLE","PATH","CHANGE","BEFORE_SHA256","AFTER_SHA256","BEFORE_BYTES","AFTER_BYTES"])
    write_csv(reports / "message_catalog_phase22ae_6_5_10x_runtime_observations_v1.csv", [
        {"OBSERVATION":"runtime_log_exists","VALUE":1 if runtime.exists() else 0,"DETAIL":rel(runtime, repo)},
        {"OBSERVATION":"imported_10_records_signal","VALUE":1 if imported10 else 0,"DETAIL":"candidate10 append"},
        ...
        {"OBSERVATION":"active_text_header_count_after_append","VALUE":header_count,"DETAIL":"raw header evidence only"},
    ], ["OBSERVATION","VALUE","DETAIL"])

    write_csv(reports / "message_catalog_phase22ae_6_5_10x_finalize_status_summary_v1.csv", [{
        "STATUS": status,
        "VALIDATION_ISSUES": val,
        "RUNTIME_LOG": rel(runtime, repo),
        "RUNTIME_IMPORTED_10": 1 if imported10 else 0,
        ...
        "ACTIVE_TEXT_HEADER_COUNT_AFTER_APPEND": header_count,
        "ACTIVE_TEXT_FINGERPRINT_DELTA_ROWS": len(delta),
        "RESTORE_REQUIRED": 1,
        "SOURCE_FILES_MUTATED": 0,
        "NEXT_GATE": NEXT_FINALIZED,
        "REPORT_TIMESTAMP_UTC": datetime.now(timezone.utc).replace(microsecond=0).isoformat().replace("+00:00","Z"),
    }], ["STATUS","VALIDATION_ISSUES","RUNTIME_LOG","RUNTIME_IMPORTED_10","RUNTIME_COUNT_70","RUNTIME_LISTED_70","PROOF_SYMBOLS_VISIBLE","PROOF_LOCALES_VISIBLE","ACTIVE_TEXT_HEADER_COUNT_AFTER_APPEND","ACTIVE_TEXT_FINGERPRINT_DELTA_ROWS","RESTORE_REQUIRED","SOURCE_FILES_MUTATED","NEXT_GATE","REPORT_TIMESTAMP_UTC"])

    print(status)
    print(f"  validation issues: {val}")
    print(f"  runtime imported 10: {1 if imported10 else 0}")
    ...
    print(f"  active text header count after append: {header_count}")
...
def restore(repo: Path, args):
    reports = repo / REPORT_DIR
    final = first_row(reports / "message_catalog_phase22ae_6_5_10x_finalize_status_summary_v1.csv")
    rows = read_csv(reports / "message_catalog_phase22ae_6_5_10x_backup_manifest_v1.csv")
    ...
    if final.get("STATUS") == STATUS_GREEN and restored_exact:
        ...
        status = "MESSAGE_CATALOG_PHASE22AE_6_5_10X_RESTORE_BLOCKED"
        next_gate = "HOLD_AND_REPAIR_PHASE22AE_6_5_10X_RESTORE_FAILURE"
        fails += max(1, len(delta))

    write_csv(reports / "message_catalog_phase22ae_6_5_10x_restore_rows_v1.csv", restore_rows, ["ROLE","ORIGINAL_PATH","BACKUP_PATH","KIND","ACTION","OK","DETAIL"])
    write_csv(reports / "message_catalog_phase22ae_6_5_10x_active_text_fingerprint_after_restore_v1.csv", after_restore, ["ROLE","PATH","EXISTS","KIND","BYTES","SHA256","FILES"])
    write_csv(reports / "message_catalog_phase22ae_6_5_10x_active_text_fingerprint_delta_after_restore_v1.csv", delta, ["ROLE","PATH","CHANGE","BEFORE_SHA256","AFTER_SHA256","BEFORE_BYTES","AFTER_BYTES"])

    val = "0" if status in (STATUS_RESTORED_GREEN, STATUS_RESTORED_BLOCKED) else str(fails)
    write_csv(reports / "message_catalog_phase22ae_6_5_10x_restore_status_summary_v1.csv", [{
        "STATUS": status,
        "VALIDATION_ISSUES": val,
...
def main():
    ...
    ap.add_argument("--allow-candidate10-append-micro-proof", action="store_true")
