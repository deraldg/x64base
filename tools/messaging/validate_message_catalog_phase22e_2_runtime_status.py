...
def main() -> int:
    ...
    # Phase 22E.1 is the repaired source-hook apply phase. The original Phase 22E
    # source-hook package failed before writing the old status file.
    ...
    gate("PHASE22E1_SOURCE_HOOK_APPLIED",
         p22e1.get("STATUS") == "MESSAGE_CATALOG_PHASE22E_1_RUNTIME_PROVIDER_STATUS_SOURCE_PATCH_APPLIED",
         p22e1.get("STATUS", ""))
    gate("RUNLOG_PRESENT", runlog.exists(), str(runlog))
    gate("PROVIDER_STATUS_OUTPUT_PRESENT", "MESSAGE CATALOG PROVIDER STATUS" in upper,
         "output should show provider status heading")
    gate("COMPILED_FALLBACK_MODE_PRESENT", "MODE: COMPILED_FALLBACK" in upper or "COMPILED FALLBACK" in upper,
         "compiled fallback mode/status should be visible")
    gate("ACTIVE_CATALOG_PRESENT_YES", "ACTIVE CATALOG PRESENT: YES" in upper,
         "active catalog artifacts should be detected")
    gate("ACTIVE_CATALOG_LOADED_NO", "ACTIVE CATALOG LOADED: NO" in upper,
         "row loading is intentionally not yet active")
    gate("NO_RUNTIME_WRITEBACK_BOUNDARY", "NO DBF/CDX/LMDB MUTATION" in upper and "NO RUNTIME WRITEBACK" in upper,
         "read-only boundary should be visible")
    review("ACTIVE_DBF_PATH_VISIBLE",
           "DOTTALKPP/DATA/MESSAGING" in upper or "DOTTALKPP\\DATA\\MESSAGING" in upper or "ACTIVE DBF DIR:" in upper,
           "active DBF path should be visible")
    ...
    write_csv(reports / "message_catalog_phase22e_runtime_gate_check_v1.csv", gates,
              ["GATE", "STATUS", "DETAIL"])
    ...
    write_csv(reports / "message_catalog_phase22e_runtime_boundary_ledger_v1.csv", boundary,
