...
STATUS_GREEN = "MESSAGE_CATALOG_PHASE22AE_6_5_6_1_WORK_AREA_SELECT_IMPORT_PROOF_GREEN_NATIVE_14_70_KEYS_MEMO_REVIEW"
STATUS_BLOCKED = "MESSAGE_CATALOG_PHASE22AE_6_5_6_1_WORK_AREA_SELECT_IMPORT_PROOF_BLOCKED"
...
def count_command_hint(text: str, n: int) -> bool:
    # Accept several likely COUNT output shapes without depending on exact wording.
    patterns = [
        rf"(?im)^\s*{n}\s*$",
        rf"(?im)\b{n}\s+record\(s\)\b",
        rf"(?im)\b{n}\s+records?\b",
        rf"(?im)\bcount\s*[:=]\s*{n}\b",
    ]
...
def main() -> int:
    ...
    gate("STAGE_GREEN", stage.get("STATUS") == "MESSAGE_CATALOG_PHASE22AE_6_5_6_1_WORK_AREA_SELECT_IMPORT_PROOF_STAGED_SOURCE_HELD", stage.get("STATUS", "missing"))
    ...
    already_open_warning = "ALREADY OPEN" in runtime_upper
    select_missing_warning = "SELECT:" in runtime_upper and ("NOT FOUND" in runtime_upper or "UNKNOWN" in runtime_upper)
    runtime_msg_12_initial = any("MSG6561_MESSAGES_NATIVE_IMPORT" in r["OPENED_NAME"].upper() and r["COUNT"] == 12 for r in open_counts)
    runtime_txt_60_initial = any("MSG6561_TEXT_NATIVE_IMPORT" in r["OPENED_NAME"].upper() and r["COUNT"] == 60 for r in open_counts)
    count_14_hint = count_command_hint(runtime_text, 14)
    count_70_hint = count_command_hint(runtime_text, 70)

    keys_ok = len(msg_matches) == 2 and len(txt_matches) == 10
    counts_ok = msg_count == 14 and text_count == 70

    if failures > 0 or not boundary_clean or not imported_2 or not imported_10 or already_open_warning or select_missing_warning or not counts_ok or not keys_ok:
        status = STATUS_BLOCKED
        validation_issues = str(max(1, failures, len(fp_delta)))
        next_gate = "HOLD_AND_FIX_PHASE22AE_6_5_6_1_WORK_AREA_SELECT_IMPORT_PROOF"
    else:
        status = STATUS_GREEN
        validation_issues = "0"
    ...
    result = [{
        ...
        "RUNTIME_INITIAL_MESSAGE_12": 1 if runtime_msg_12_initial else 0,
        "RUNTIME_INITIAL_TEXT_60": 1 if runtime_txt_60_initial else 0,
        "COUNT_14_HINT_SEEN": 1 if count_14_hint else 0,
        "COUNT_70_HINT_SEEN": 1 if count_70_hint else 0,
        "IMPORTED_2_RECORDS": 1 if imported_2 else 0,
        "IMPORTED_10_RECORDS": 1 if imported_10 else 0,
        "ALREADY_OPEN_WARNING": 1 if already_open_warning else 0,
        "SELECT_MISSING_WARNING": 1 if select_missing_warning else 0,
        ...
        "WORK_AREA_DISCIPLINE_INCLUDED": stage.get("WORK_AREA_DISCIPLINE_INCLUDED", ""),
        "SELECT_TABLE_PROOF_INCLUDED": stage.get("SELECT_TABLE_PROOF_INCLUDED", ""),
        "ABSOLUTE_PATH_NAMES_SUPPORTED_BY_PROOF": stage.get("ABSOLUTE_PATH_NAMES_SUPPORTED_BY_PROOF", ""),
        "BOUNDARY_CLEAN": 1 if boundary_clean else 0,
        "PROTECTED_FINGERPRINT_CHANGES": len(fp_delta),
    }]

    write_csv(reports / "message_catalog_phase22ae_6_5_6_1_validate_gate_check_v1.csv",
              gates, ["GATE", "STATUS", "DETAIL"])
    write_csv(reports / "message_catalog_phase22ae_6_5_6_1_native_import_result_v1.csv",
              result, ["MESSAGE_ROWS_AFTER", "TEXT_ROWS_AFTER", "COUNTS_14_70",
                       "RUNTIME_INITIAL_MESSAGE_12", "RUNTIME_INITIAL_TEXT_60",
                       "COUNT_14_HINT_SEEN", "COUNT_70_HINT_SEEN",
                       ...
                       "WORK_AREA_DISCIPLINE_INCLUDED", "SELECT_TABLE_PROOF_INCLUDED",
                       "ABSOLUTE_PATH_NAMES_SUPPORTED_BY_PROOF",
                       "BOUNDARY_CLEAN", "PROTECTED_FINGERPRINT_CHANGES"])
    write_csv(reports / "message_catalog_phase22ae_6_5_6_1_runtime_open_counts_v1.csv",
              open_counts, ["OPENED_NAME", "COUNT"])
    write_csv(reports / "message_catalog_phase22ae_6_5_6_1_found_message_keys_v1.csv",
              msg_matches, ["SYMBOL", "MATCHES", "RECNO"])
    write_csv(reports / "message_catalog_phase22ae_6_5_6_1_found_text_keys_v1.csv",
              txt_matches, ["SYMBOL", "LOCALE", "MATCHES", "RECNO"])
    write_csv(reports / "message_catalog_phase22ae_6_5_6_1_active_fingerprint_after_v1.csv",
              after_fp, ["ROLE", "PATH", "EXISTS", "KIND", "BYTES", "SHA256", "FILES"])
    write_csv(reports / "message_catalog_phase22ae_6_5_6_1_active_fingerprint_delta_v1.csv",
    ...
    write_csv(reports / "message_catalog_phase22ae_6_5_6_1_validate_boundary_ledger_v1.csv",
              boundary, ["PROTECTED_SYSTEM", "MUTATION_ALLOWED", "OBSERVED_MUTATION", "DETAIL"])

    write_csv(reports / "message_catalog_phase22ae_6_5_6_1_validate_status_summary_v1.csv", [{
        "STATUS": status,
        "VALIDATION_ISSUES": validation_issues,
        "STAGE_GREEN": 1 if stage.get("STATUS") == "MESSAGE_CATALOG_PHASE22AE_6_5_6_1_WORK_AREA_SELECT_IMPORT_PROOF_STAGED_SOURCE_HELD" else 0,
        "MESSAGE_ROWS_AFTER": msg_count,
        "TEXT_ROWS_AFTER": text_count,
        "COUNTS_14_70": 1 if counts_ok else 0,
        "IMPORTED_2_RECORDS": 1 if imported_2 else 0,
        ...
        "REPORT_TIMESTAMP_UTC": datetime.now(timezone.utc).replace(microsecond=0).isoformat().replace("+00:00", "Z"),
         ...
         "COUNT_14_HINT_SEEN", "COUNT_70_HINT_SEEN",
         "FOUND_MESSAGE_KEYS", "FOUND_TEXT_KEYS", "KEYS_PROVEN",
         "MEMO_PAYLOAD_REVIEW_ROWS", "WORK_AREA_DISCIPLINE_INCLUDED",
         "SELECT_TABLE_PROOF_INCLUDED", "ABSOLUTE_PATH_NAMES_SUPPORTED_BY_PROOF",
         "BOUNDARY_CLEAN", "PROTECTED_FINGERPRINT_CHANGES",
    ...
    print(f"  stage green: {1 if stage.get('STATUS') == 'MESSAGE_CATALOG_PHASE22AE_6_5_6_1_WORK_AREA_SELECT_IMPORT_PROOF_STAGED_SOURCE_HELD' else 0}")
    ...
    print(f"  already-open warning: {1 if already_open_warning else 0}")
    print(f"  select missing warning: {1 if select_missing_warning else 0}")
    print(f"  count 14/70 hint seen: {1 if count_14_hint else 0}/{1 if count_70_hint else 0}")
    ...
    print(f"  work-area discipline included: {stage.get('WORK_AREA_DISCIPLINE_INCLUDED','')}")
    print(f"  select table proof included: {stage.get('SELECT_TABLE_PROOF_INCLUDED','')}")
    print(f"  absolute path names supported by proof: {stage.get('ABSOLUTE_PATH_NAMES_SUPPORTED_BY_PROOF','')}")
