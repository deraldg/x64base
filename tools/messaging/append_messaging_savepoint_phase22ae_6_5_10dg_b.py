...
SAVEPOINT = "MSG-022AE.6.5.10DG-B"
...
def summary(repo: Path) -> dict:
    path = repo / "docs/messaging/reports/message_catalog_phase22ae_6_5_10dg_b_status_summary_v1.csv"
    try:
...
def main() -> int:
    ...
    if not args.accept_side_branch_savepoint:
        print("[MSG-022AE.6.5.10DG-B] Refusing without --accept-side-branch-savepoint.")
        return 1
    if status != GREEN:
        print(f"[MSG-022AE.6.5.10DG-B] Status is not green: {status}")
        return 1

    journal = repo / "docs/messaging/MESSAGING_SAVEPOINT_JOURNAL.md"
    old = read_text(journal)
    if SAVEPOINT in old:
        print("[MSG-022AE.6.5.10DG-B] Side-branch savepoint already appears in journal; refusing duplicate append.")
        return 0
...
- Target selection plan reviewed: {row.get('TARGET_SELECTION_PLAN_REVIEWED','')}
- Ranked candidates reviewed: {row.get('RANKED_CANDIDATES_REVIEWED','')}
- Focus review queue rows: {row.get('FOCUS_REVIEW_QUEUE_ROWS','')}
...
- Latest pointer changed by DG-B: {row.get('LATEST_POINTER_CHANGED_BY_DG_B','')}
- Next gate: {row.get('NEXT_GATE','')}
"""
    write_text(journal, old.rstrip() + "\n\n" + entry.lstrip())
    print("[MSG-022AE.6.5.10DG-B] Side-branch savepoint appended.")
