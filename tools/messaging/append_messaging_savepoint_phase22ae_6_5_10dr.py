...
PHASE = '22AE.6.5.10DR'
STATUS = 'MESSAGE_CATALOG_PHASE22AE_6_5_10DR_SHUTDOWN_ISOLATION_PROOF_REVIEW_GREEN_GENERAL_SHUTDOWN_EXIT_CRASH_CONFIRMED_SOURCE_HELD'
...
def main():
    ...
    summary_path=root/'phase22ae_6_5_10dr_summary_v1.json'
    if not summary_path.exists():
        print(f'MSG-022AE.6.5.10DR summary not found: {summary_path}')
        return 1
    summary=json.loads(summary_path.read_text(encoding='utf-8'))
    if summary.get('status') != STATUS:
        print(f'MSG-022AE.6.5.10DR summary status is not green: {summary.get("status")}')
        return 1
