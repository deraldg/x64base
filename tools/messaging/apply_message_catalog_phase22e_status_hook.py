...
STATUS_GREEN = "MESSAGE_CATALOG_PHASE22E_RUNTIME_PROVIDER_STATUS_SOURCE_PATCH_APPLIED"
STATUS_BLOCKED = "MESSAGE_CATALOG_PHASE22E_RUNTIME_PROVIDER_STATUS_SOURCE_PATCH_BLOCKED"
...
INCLUDE_LINE = '#include "message_catalog.hpp"'
...
def patch_cmd_set(text: str) -> tuple[str, list[dict[str, str]]]:
    ...
    if INCLUDE_LINE not in new_text:
        ...
        if anchor not in new_text:
            raise RuntimeError("cmd_set.cpp include anchor not found: cli/table_state.hpp")
    ...
    if "SET MESSAGE CATALOG CHECK" not in new_text:
        usage_anchor = '        << "  SET PATH <slot> <path>\\\\n"'
        if usage_anchor in new_text:
            new_text = new_text.replace(usage_anchor, usage_anchor + "\\n" + USAGE_LINE, 1)
    ...
    if HELPER_MARKER_BEGIN not in new_text:
        ...
        if anchor not in new_text:
            raise RuntimeError("cmd_set.cpp helper anchor not found: static void print_set_usage()")
        new_text = new_text.replace(anchor, HELPER_BLOCK + "\\n" + anchor, 1)
        actions.append({"ACTION": "INSERT_HELPER", "DETAIL": "print_message_catalog_provider_status"})
    else:
    ...
    if BRANCH_MARKER_BEGIN not in new_text:
        anchor = "    // ─────────────────────────────────────────────────────────────\\n    // SET TABLE BUFFER"
        if anchor not in new_text:
            raise RuntimeError("cmd_set.cpp branch anchor not found: SET TABLE BUFFER block")
        new_text = new_text.replace(anchor, BRANCH_BLOCK + anchor, 1)
        actions.append({"ACTION": "INSERT_BRANCH", "DETAIL": "SET MESSAGE CATALOG CHECK"})
    else:
...
def main() -> int:
    ...
    if failures == 0:
        ...
        backup_root = repo / BACKUP_ROOT_BASE / f"MSG-022E_STATUS_COMMAND_PATCH_BACKUP_{timestamp}"
        ...
        mutation_rows.append({
            ...
            "DETAIL": "added SET MESSAGE CATALOG CHECK provider status hook",
        })
        status = STATUS_GREEN

    validation_issues = "0" if status == STATUS_GREEN else str(failures)

    write_csv(reports / "message_catalog_phase22e_status_summary_v1.csv", [{
        "STATUS": status,
        "MESSAGES": messages,
        "TEXT_ROWS": text_rows,
        "LOCALES": locales,
    ...
    write_csv(reports / "message_catalog_phase22e_gate_check_v1.csv", gates, ["GATE", "STATUS", "DETAIL"])
    write_csv(reports / "message_catalog_phase22e_patch_actions_v1.csv", patch_action_rows, ["TARGET_PATH", "ACTION", "DETAIL"])
    write_csv(reports / "message_catalog_phase22e_source_mutation_inventory_v1.csv", mutation_rows, ["TARGET_PATH", "ACTION", "BYTES", "SHA256", "DETAIL"])
    write_csv(reports / "message_catalog_phase22e_source_backup_inventory_v1.csv", backup_rows, ["TARGET_PATH", "BACKUP_PATH", "BYTES", "SHA256", "ROLE"])
    ...
    write_csv(reports / "message_catalog_phase22e_boundary_ledger_v1.csv", boundary, ["PROTECTED_SYSTEM", "MUTATION_ALLOWED", "OBSERVED_MUTATION", "DETAIL"])
