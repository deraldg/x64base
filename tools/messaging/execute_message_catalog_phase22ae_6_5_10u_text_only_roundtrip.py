...
PACKAGE_ROOT = Path("docs/messaging/apply/phase22ae_6_5_10u_text_only_active_baseline_roundtrip_v1")
SCRIPT_PATH = Path("docs/messaging/scripts/MESSAGE_CATALOG_PHASE22AE_6_5_10U_TEXT_ONLY_ACTIVE_BASELINE_ROUNDTRIP.dts")
RUNLOG_PATH = Path("docs/messaging/runlog/MSG-022AE_6_5_10U_TEXT_ONLY_ACTIVE_BASELINE_ROUNDTRIP.md")

PLAN_ROOT = Path("docs/messaging/apply/phase22ae_6_5_10t_text_only_active_import_micro_proof_plan_v1")
BASELINE60_CSV = PLAN_ROOT / "import/system_message_text_baseline60_roundtrip.csv"
...
STATUS_PREPARED = "MESSAGE_CATALOG_PHASE22AE_6_5_10U_TEXT_ONLY_ACTIVE_BASELINE_ROUNDTRIP_PREPARED"
STATUS_GREEN = "MESSAGE_CATALOG_PHASE22AE_6_5_10U_TEXT_ONLY_ACTIVE_BASELINE_ROUNDTRIP_GREEN_RESTORE_REQUIRED"
STATUS_BLOCKED = "MESSAGE_CATALOG_PHASE22AE_6_5_10U_TEXT_ONLY_ACTIVE_BASELINE_ROUNDTRIP_BLOCKED_RESTORE_REQUIRED"
STATUS_RESTORED_GREEN = "MESSAGE_CATALOG_PHASE22AE_6_5_10U_TEXT_ONLY_ACTIVE_BASELINE_ROUNDTRIP_PROVEN_AND_RESTORED"
STATUS_RESTORED_BLOCKED = "MESSAGE_CATALOG_PHASE22AE_6_5_10U_TEXT_ONLY_ACTIVE_BASELINE_ROUNDTRIP_FAILED_BUT_RESTORED"

NEXT_PREPARED = "RUN_PHASE22AE_6_5_10U_TEXT_ONLY_ACTIVE_BASELINE_ROUNDTRIP_THEN_FINALIZE"
NEXT_FINALIZED = "RESTORE_PHASE22AE_6_5_10U_ACTIVE_TEXT_BACKUP_BEFORE_SAVEPOINT"
NEXT_RESTORED_GREEN = "HOLD_OR_AUTHORIZE_PHASE22AE_6_5_10V_TEXT_ACTIVE_PATH_CLASSIFICATION"
NEXT_RESTORED_BLOCKED = "HOLD_AND_REVIEW_PHASE22AE_6_5_10U_TEXT_ROUNDTRIP_FAILURE"
...
def prepare(repo: Path, args):
    ...
    t = first_row(reports / "message_catalog_phase22ae_6_5_10t_status_summary_v1.csv")
    sp, latest = savepoint_present(repo, "MSG-022AE.6.5.10T")
    ...
    gate("ALLOW_ACTIVE_TEXT_MICRO_PROOF_FLAG", args.allow_active_text_micro_proof, args.allow_active_text_micro_proof)
    gate("PHASE22AE_6_5_10T_GREEN", t.get("STATUS") == "MESSAGE_CATALOG_PHASE22AE_6_5_10T_TEXT_ONLY_ACTIVE_IMPORT_MICRO_PROOF_PLAN_GREEN_SOURCE_HELD", t.get("STATUS", "missing"))
    gate("MSG_022AE_6_5_10T_SAVEPOINT_PRESENT", sp, latest)
    ...
    gate("BASELINE60_CSV_HAS_60_ROWS", len(read_csv(repo / BASELINE60_CSV)) == 60, len(read_csv(repo / BASELINE60_CSV)))
    ...
    write_csv(reports / "message_catalog_phase22ae_6_5_10u_active_text_fingerprint_before_v1.csv", before, ["ROLE","PATH","EXISTS","KIND","BYTES","SHA256","FILES"])

    status = "MESSAGE_CATALOG_PHASE22AE_6_5_10U_TEXT_ONLY_ACTIVE_BASELINE_ROUNDTRIP_PREPARE_BLOCKED"
    backup_root_rel = ""
    script_rel = ""
    backup_rows = []
    ...
    if failures == 0:
        ...
        shutil.copy2(repo / BASELINE60_CSV, pkg_root / "import/system_message_text_baseline60_roundtrip.csv")
        stamp = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
        backup_root = repo / "docs/messaging/backups" / f"MSG-022AE_6_5_10U_TEXT_ONLY_BASELINE_ROUNDTRIP_BACKUP_{stamp}"
        ...
        script.write_text("\n".join([
            "* MESSAGE_CATALOG_PHASE22AE_6_5_10U_TEXT_ONLY_ACTIVE_BASELINE_ROUNDTRIP.dts",
            "* ACTIVE TEXT-ONLY MICRO PROOF - diagnostic only.",
            "* No QUIT here; quit manually in interactive runs.",
            f"USE {(repo / ACTIVE_TEXT_DBF).resolve().as_posix()}",
            "ZAP",
            f"USE {(repo / ACTIVE_TEXT_DBF).resolve().as_posix()}",
            f"IMPORT {(pkg_root / 'import/system_message_text_baseline60_roundtrip.csv').resolve().as_posix()}",
            f"USE {(repo / ACTIVE_TEXT_DBF).resolve().as_posix()}",
            "COUNT",
            "LIST ALL",
            "",
    ...
    write_csv(reports / "message_catalog_phase22ae_6_5_10u_prepare_gate_check_v1.csv", gates, ["GATE","STATUS","DETAIL"])
    write_csv(reports / "message_catalog_phase22ae_6_5_10u_backup_manifest_v1.csv", backup_rows, ["ROLE","ORIGINAL_PATH","BACKUP_PATH","EXISTS_AT_BACKUP","KIND","BYTES","SHA256","RESTORE_POLICY"])
    boundary = [
    ...
    write_csv(reports / "message_catalog_phase22ae_6_5_10u_prepare_boundary_ledger_v1.csv", boundary, ["PROTECTED_SYSTEM","MUTATION_ALLOWED","OBSERVED_MUTATION","DETAIL"])

    val = "0" if status == STATUS_PREPARED else str(failures)
    write_csv(reports / "message_catalog_phase22ae_6_5_10u_prepare_status_summary_v1.csv", [{
        "STATUS": status,
        "VALIDATION_ISSUES": val,
        "PHASE22AE_6_5_10T_STATUS": t.get("STATUS",""),
        "MSG_022AE_6_5_10T_SAVEPOINT_PRESENT": 1 if sp else 0,
        "DOTTALKPP_PROCESS_RUNNING": 1 if running else 0,
        "ALLOW_ACTIVE_TEXT_MICRO_PROOF": 1 if args.allow_active_text_micro_proof else 0,
        ...
        "BASELINE60_ROWS": len(read_csv(pkg_root / "import/system_message_text_baseline60_roundtrip.csv")) if pkg_root.exists() else "",
    ...
    }], ["STATUS","VALIDATION_ISSUES","PHASE22AE_6_5_10T_STATUS","MSG_022AE_6_5_10T_SAVEPOINT_PRESENT","DOTTALKPP_PROCESS_RUNNING","ALLOW_ACTIVE_TEXT_MICRO_PROOF","PACKAGE_ROOT","SCRIPT_PATH","RUNLOG_PATH","BACKUP_ROOT","BACKUP_ROWS","BASELINE60_ROWS","SHOULD_EXECUTE_RUNTIME","ACTIVE_TEXT_MUTATION_OBSERVED","SOURCE_FILES_MUTATED","NEXT_GATE","REPORT_TIMESTAMP_UTC"])

    print(status)
    print(f"  validation issues: {val}")
    print(f"  Phase 22AE.6.5.10T green: {1 if t.get('STATUS') == 'MESSAGE_CATALOG_PHASE22AE_6_5_10T_TEXT_ONLY_ACTIVE_IMPORT_MICRO_PROOF_PLAN_GREEN_SOURCE_HELD' else 0}")
    print(f"  MSG-022AE.6.5.10T savepoint present: {1 if sp else 0}")
    print(f"  dottalkpp process running: {1 if running else 0}")
    print(f"  allow active text micro proof: {1 if args.allow_active_text_micro_proof else 0}")
    print(f"  script path: {script_rel}")
    print(f"  backup root: {backup_root_rel}")
    print(f"  baseline60 rows: {len(read_csv(repo / BASELINE60_CSV))}")
...
def finalize(repo: Path, args):
    reports = repo / REPORT_DIR
    prep = first_row(reports / "message_catalog_phase22ae_6_5_10u_prepare_status_summary_v1.csv")
    ...
    gate("IMPORTED_60_SIGNAL", "IMPORTED 60 RECORDS" in up, "baseline60 import")
    gate("ZAP_COMPLETE_SIGNAL", "ZAP COMPLETE" in up, up.count("ZAP COMPLETE"))
    gate("COUNT_60_SIGNAL", "\n60\n" in log.replace("\r", "\n") or "COUNT 60" in " ".join(log.split()).upper(), "COUNT should show 60")
    gate("LIST_60_SIGNAL", "60 RECORD(S) LISTED" in up, "LIST ALL should list 60")
    ...
    before = read_csv(reports / "message_catalog_phase22ae_6_5_10u_active_text_fingerprint_before_v1.csv")
    ...
    write_csv(reports / "message_catalog_phase22ae_6_5_10u_finalize_gate_check_v1.csv", gates, ["GATE","STATUS","DETAIL"])
    write_csv(reports / "message_catalog_phase22ae_6_5_10u_active_text_fingerprint_after_roundtrip_v1.csv", after, ["ROLE","PATH","EXISTS","KIND","BYTES","SHA256","FILES"])
    write_csv(reports / "message_catalog_phase22ae_6_5_10u_active_text_fingerprint_delta_roundtrip_v1.csv", delta, ["ROLE","PATH","CHANGE","BEFORE_SHA256","AFTER_SHA256","BEFORE_BYTES","AFTER_BYTES"])
    write_csv(reports / "message_catalog_phase22ae_6_5_10u_runtime_observations_v1.csv", [
        {"OBSERVATION":"runtime_log_exists","VALUE":1 if runtime.exists() else 0,"DETAIL":rel(runtime, repo)},
        {"OBSERVATION":"imported_60_records_signal","VALUE":1 if "IMPORTED 60 RECORDS" in up else 0,"DETAIL":"baseline60 roundtrip"},
        {"OBSERVATION":"zap_complete_signal","VALUE":1 if "ZAP COMPLETE" in up else 0,"DETAIL":"active text ZAP"},
        {"OBSERVATION":"list_60_signal","VALUE":1 if "60 RECORD(S) LISTED" in up else 0,"DETAIL":"LIST ALL"},
        {"OBSERVATION":"active_text_header_count_after_roundtrip","VALUE":header_count,"DETAIL":"raw header evidence only"},
    ], ["OBSERVATION","VALUE","DETAIL"])

    write_csv(reports / "message_catalog_phase22ae_6_5_10u_finalize_status_summary_v1.csv", [{
        "STATUS": status,
        "VALIDATION_ISSUES": val,
        "RUNTIME_LOG": rel(runtime, repo),
        "RUNTIME_IMPORTED_60": 1 if "IMPORTED 60 RECORDS" in up else 0,
        "RUNTIME_LISTED_60": 1 if "60 RECORD(S) LISTED" in up else 0,
        "ACTIVE_TEXT_HEADER_COUNT_AFTER_ROUNDTRIP": header_count,
        "ACTIVE_TEXT_FINGERPRINT_DELTA_ROWS": len(delta),
        "RESTORE_REQUIRED": 1,
        "SOURCE_FILES_MUTATED": 0,
        "NEXT_GATE": NEXT_FINALIZED,
        "REPORT_TIMESTAMP_UTC": datetime.now(timezone.utc).replace(microsecond=0).isoformat().replace("+00:00","Z"),
    }], ["STATUS","VALIDATION_ISSUES","RUNTIME_LOG","RUNTIME_IMPORTED_60","RUNTIME_LISTED_60","ACTIVE_TEXT_HEADER_COUNT_AFTER_ROUNDTRIP","ACTIVE_TEXT_FINGERPRINT_DELTA_ROWS","RESTORE_REQUIRED","SOURCE_FILES_MUTATED","NEXT_GATE","REPORT_TIMESTAMP_UTC"])

    print(status)
    print(f"  validation issues: {val}")
    print(f"  runtime imported 60: {1 if 'IMPORTED 60 RECORDS' in up else 0}")
    print(f"  runtime listed 60: {1 if '60 RECORD(S) LISTED' in up else 0}")
    print(f"  active text header count after roundtrip: {header_count}")
...
def restore(repo: Path, args):
    reports = repo / REPORT_DIR
    prep = first_row(reports / "message_catalog_phase22ae_6_5_10u_prepare_status_summary_v1.csv")
    final = first_row(reports / "message_catalog_phase22ae_6_5_10u_finalize_status_summary_v1.csv")
    rows = read_csv(reports / "message_catalog_phase22ae_6_5_10u_backup_manifest_v1.csv")
    ...
    if final.get("STATUS") == STATUS_GREEN and restored_exact:
        ...
        status = "MESSAGE_CATALOG_PHASE22AE_6_5_10U_RESTORE_BLOCKED"
        next_gate = "HOLD_AND_REPAIR_PHASE22AE_6_5_10U_RESTORE_FAILURE"
        fails += max(1, len(delta))

    write_csv(reports / "message_catalog_phase22ae_6_5_10u_restore_rows_v1.csv", restore_rows, ["ROLE","ORIGINAL_PATH","BACKUP_PATH","KIND","ACTION","OK","DETAIL"])
    write_csv(reports / "message_catalog_phase22ae_6_5_10u_active_text_fingerprint_after_restore_v1.csv", after_restore, ["ROLE","PATH","EXISTS","KIND","BYTES","SHA256","FILES"])
    write_csv(reports / "message_catalog_phase22ae_6_5_10u_active_text_fingerprint_delta_after_restore_v1.csv", delta, ["ROLE","PATH","CHANGE","BEFORE_SHA256","AFTER_SHA256","BEFORE_BYTES","AFTER_BYTES"])

    val = "0" if status in (STATUS_RESTORED_GREEN, STATUS_RESTORED_BLOCKED) else str(fails)
    write_csv(reports / "message_catalog_phase22ae_6_5_10u_restore_status_summary_v1.csv", [{
        "STATUS": status,
        "VALIDATION_ISSUES": val,
...
def main():
    ...
    ap.add_argument("--allow-active-text-micro-proof", action="store_true")
