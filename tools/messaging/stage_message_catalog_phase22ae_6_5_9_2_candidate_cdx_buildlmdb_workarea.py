...
OK="MESSAGE_CATALOG_PHASE22AE_6_5_9_2_CANDIDATE_CDX_BUILDLMDB_WORKAREA_PROOF_STAGED_SOURCE_HELD"
BAD="MESSAGE_CATALOG_PHASE22AE_6_5_9_2_CANDIDATE_CDX_BUILDLMDB_WORKAREA_PROOF_BLOCKED"
NEXT="RUN_PHASE22AE_6_5_9_2_CANDIDATE_CDX_BUILDLMDB_RUNTIME_THEN_VALIDATE"
    ...
    for prefix,root in roots:
        for name in ["SYSTEM_MESSAGES","SYSTEM_MESSAGE_TEXT"]:
            for p in sorted(root.glob(name+"*")) if root.exists() else []:
                if p.is_dir():
                    h,c,b=dir_sha(p); out.append({"ROLE":prefix+"_"+p.name,"PATH":rel(p,repo),"KIND":"dir","EXISTS":1,"BYTES":b,"SHA256":h,"FILES":c})
                elif p.is_file():
...
def main():
    ...
    ap.add_argument("--allow-candidate-cdx-lmdb-repair", action="store_true")
    ap.add_argument("--replace-existing-candidate-indexes", action="store_true")
    ap.add_argument("--replace-existing-candidate-lmdb", action="store_true")
    args=ap.parse_args()
    ...
    gate("CANDIDATE_CDX_LMDB_REPAIR_EXPLICITLY_AUTHORIZED",args.allow_candidate_cdx_lmdb_repair,args.allow_candidate_cdx_lmdb_repair)
    ...
    gate("CANDIDATE_LMDB_ROOT_ABSENT_OR_REPLACE_AUTHORIZED",(not lmdb.exists()) or args.replace_existing_candidate_lmdb,rel(lmdb,repo))
    before=fp(repo)
    wcsv(reports/"message_catalog_phase22ae_6_5_9_2_active_fingerprint_before_v1.csv",before,["ROLE","PATH","KIND","EXISTS","BYTES","SHA256","FILES"])
    script_rel=""; err=""
    if fail==0:
        try:
            if indexes.exists() and args.replace_existing_candidate_indexes:
                shutil.rmtree(indexes)
            if lmdb.exists() and args.replace_existing_candidate_lmdb:
                shutil.rmtree(lmdb)
            indexes.mkdir(parents=True, exist_ok=True)
            lmdb.mkdir(parents=True, exist_ok=True)
            script=repo/"docs/messaging/scripts/MESSAGE_CATALOG_PHASE22AE_6_5_9_2_CANDIDATE_CDX_BUILDLMDB_WORKAREA_PROOF.dts"
            script.parent.mkdir(parents=True, exist_ok=True)
            script.write_text("\n".join([
                "* MESSAGE_CATALOG_PHASE22AE_6_5_9_2_CANDIDATE_CDX_BUILDLMDB_WORKAREA_PROOF.dts",
                "* Candidate-only CDX + BUILDLMDB usable-index proof.",
                "* Uses explicit work areas SELECT 1 and SELECT 2. Keeps both tables open for WORKSPACE/RDBMS visibility.",
                "* Do not use SET CDX. Do not use SET INDEXES TO. Do not use SELECT 0 for this two-table proof.",
                f"SETPATH DBF {dbf.resolve().as_posix()}",
                f"SETPATH INDEXES {indexes.resolve().as_posix()}",
                f"SETPATH LMDB {lmdb.resolve().as_posix()}",
                "SET PATH",
                "SELECT 1",
                "USE SYSTEM_MESSAGES",
                "CDX CREATE",
    ...
    inv=[
        ...
        {"ROLE":"candidate_lmdb_root","PATH":rel(lmdb,repo),"EXISTS":1 if lmdb.exists() else 0,"BYTES":"","SHA256":""},
    ...
    boundary=[
        {"PROTECTED_SYSTEM":"ACTIVE_MESSAGING_DBF_CATALOG","MUTATION_ALLOWED":0,"OBSERVED_MUTATION":0,"DETAIL":"no active DBF mutation in 6.5.9.2"},
        ...
        {"PROTECTED_SYSTEM":"ACTIVE_AND_DEFAULT_LMDB_ROOTS","MUTATION_ALLOWED":0,"OBSERVED_MUTATION":0,"DETAIL":"repair script targets candidate LMDB slot using SETPATH"},
    ...
    wcsv(reports/"message_catalog_phase22ae_6_5_9_2_stage_gate_check_v1.csv",gates,["GATE","STATUS","DETAIL"])
    wcsv(reports/"message_catalog_phase22ae_6_5_9_2_candidate_inventory_before_v1.csv",inv,["ROLE","PATH","EXISTS","BYTES","SHA256"])
    ...
    wcsv(reports/"message_catalog_phase22ae_6_5_9_2_stage_status_summary_v1.csv",[{
        ...
        "CANDIDATE_DBF_ROOT":rel(dbf,repo),"CANDIDATE_INDEX_ROOT":rel(indexes,repo),"CANDIDATE_LMDB_ROOT":rel(lmdb,repo),
        "SCRIPT_PATH":script_rel,
        "CANDIDATE_CDX_LMDB_REPAIR_AUTHORIZED":1 if args.allow_candidate_cdx_lmdb_repair else 0,
        ...
        "ERRORS":err,"NEXT_GATE":NEXT if status==OK else "HOLD_AND_FIX_PHASE22AE_6_5_9_2_STAGE",
        "REPORT_TIMESTAMP_UTC":datetime.now(timezone.utc).replace(microsecond=0).isoformat().replace("+00:00","Z")
    }],["STATUS","VALIDATION_ISSUES","PHASE22AE_6_5_8_GREEN","PHASE22AE_6_5_9_BLOCKED_INCIDENT_CAPTURED","CANDIDATE_DBF_ROOT","CANDIDATE_INDEX_ROOT","CANDIDATE_LMDB_ROOT","SCRIPT_PATH","CANDIDATE_CDX_LMDB_REPAIR_AUTHORIZED","ACTIVE_PROMOTION_AUTHORIZED","ACTIVE_CATALOG_MUTATION_OBSERVED","SOURCE_FILES_MUTATED","HELP_DATA_MUTATION_OBSERVED","CMDHELPCHK_MUTATION_OBSERVED","ERRORS","NEXT_GATE","REPORT_TIMESTAMP_UTC"])
    print(status)
    ...
    print(f"  candidate LMDB root: {rel(lmdb,repo)}")
    print(f"  script path: {script_rel}")
    print(f"  candidate CDX+LMDB repair authorized: {1 if args.allow_candidate_cdx_lmdb_repair else 0}")
    print("  active promotion authorized: 0")
    print("  active catalog mutation observed: 0")
    print("  source files mutated: 0")
    print(f"  next gate: {NEXT if status==OK else 'HOLD_AND_FIX_PHASE22AE_6_5_9_2_STAGE'}")
