...
SAVEPOINT = "MSG-022AE.6.5.10DQ-B"
...
def summary(repo: Path) -> dict:
    path = repo / "docs/messaging/reports/message_catalog_phase22ae_6_5_10dq_b_status_summary_v1.csv"
    try:
...
def main() -> int:
    ...
    if not args.accept_side_branch_savepoint:
        print("[MSG-022AE.6.5.10DQ-B] Refusing without --accept-side-branch-savepoint.")
        return 1
    if status != GREEN:
        print(f"[MSG-022AE.6.5.10DQ-B] Status is not green: {status}")
        return 1
...
- Localized variants held for future overlay: {row.get('LOCALIZED_VARIANTS_HELD_FOR_FUTURE_OVERLAY','')}
- CMDHELPCHK policy resolved as report-only for this lane: {row.get('CMDHELPCHK_POLICY_RESOLVED_AS_REPORT_ONLY_FOR_THIS_LANE','')}
- Dry-run authorized now: {row.get('DRY_RUN_AUTHORIZED_NOW','')}
- Dry-run still held: {row.get('DRY_RUN_STILL_HELD','')}
- Apply execution authorized now: {row.get('APPLY_EXECUTION_AUTHORIZED_NOW','')}
- Apply still held: {row.get('APPLY_STILL_HELD','')}
- HELP DATA apply executed: {row.get('HELP_DATA_APPLY_EXECUTED','')}
- CMDHELPCHK apply executed: {row.get('CMDHELPCHK_APPLY_EXECUTED','')}
- Latest pointer changed by DQ-B: {row.get('LATEST_POINTER_CHANGED_BY_DQ_B','')}
- Next gate: {row.get('NEXT_GATE','')}
"""
    write_text(journal, old.rstrip() + "\n\n" + entry.lstrip())
    print("[MSG-022AE.6.5.10DQ-B] Side-branch savepoint appended.")
