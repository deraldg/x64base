...
STATUS_GREEN = "MESSAGE_CATALOG_PHASE22Q_UNSUPPORTED_LOCALE_RUNTIME_ROUTING_PATCH_APPLIED"
STATUS_BLOCKED = "MESSAGE_CATALOG_PHASE22Q_UNSUPPORTED_LOCALE_RUNTIME_ROUTING_PATCH_BLOCKED"
...
def main():
    ...
    if failures == 0:
        try:
            backup_root = repo / BACKUP_ROOT_BASE / f"MSG-022Q_UNSUPPORTED_LOCALE_ROUTING_BACKUP_{datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')}"
            ...
            mutation_rows.append({
                ...
                "DETAIL": "routed SET LANGUAGE unsupported-locale rejection through active provider symbol UNSUPPORTED_MESSAGE_LOCALE with compiled fallback preserved",
            })
            status = STATUS_GREEN
    ...
    write_csv(reports / "message_catalog_phase22q_status_summary_v1.csv", [{
        "STATUS": status,
        "MESSAGES": messages,
        "TEXT_ROWS": text_rows,
        "LOCALES": locales,
        ...
        "UNSUPPORTED_LOCALE_ROUTING_PATCH_APPLIED": 1 if status == STATUS_GREEN else 0,
        "BUILD_EXECUTED": 0,
        ...
        "REPORT_TIMESTAMP_UTC": datetime.now(timezone.utc).replace(microsecond=0).isoformat().replace("+00:00", "Z"),
         ...
         "ROUTED_SYMBOL", "UNSUPPORTED_LOCALE_ROUTING_PATCH_APPLIED", "BUILD_EXECUTED",
         "RUNTIME_SMOKE_EXECUTED", "ERRORS", "NEXT_GATE", "REPORT_TIMESTAMP_UTC"])

    write_csv(reports / "message_catalog_phase22q_gate_check_v1.csv",
              gates,
              ["GATE", "STATUS", "DETAIL"])
    write_csv(reports / "message_catalog_phase22q_patch_actions_v1.csv",
              action_rows,
              ["TARGET_PATH", "ACTION", "DETAIL"])
    write_csv(reports / "message_catalog_phase22q_source_mutation_inventory_v1.csv",
              mutation_rows,
              ["TARGET_PATH", "ACTION", "BYTES", "SHA256", "DETAIL"])
    write_csv(reports / "message_catalog_phase22q_source_backup_inventory_v1.csv",
              backup_rows,
              ["TARGET_PATH", "BACKUP_PATH", "BYTES", "SHA256", "ROLE"])

    boundary = [
        {"PROTECTED_SYSTEM": "SOURCE_CODE", "MUTATION_ALLOWED": 1, "OBSERVED_MUTATION": len(mutation_rows), "DETAIL": "Authorized source mutation limited to src/cli/cmd_set.cpp unsupported-locale routing."},
    ...
    write_csv(reports / "message_catalog_phase22q_boundary_ledger_v1.csv",
              boundary,
