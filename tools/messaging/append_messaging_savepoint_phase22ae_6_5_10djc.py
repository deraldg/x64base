...
SUMMARY_REL='docs/messaging/apply/phase22ae_6_5_10djc_clean_runtime_proof_transcript_review_v1/phase22ae_6_5_10djc_summary_v1.json'
...
def main():
    ...
    sp=repo/SUMMARY_REL
    if not sp.exists(): raise SystemExit(f'10DJC summary not found: {sp}')
    summary=json.loads(sp.read_text(encoding='utf-8'))
    if summary.get('status') != STATUS: raise SystemExit(f'10DJC summary is not green: {summary.get("status")}')
    ts=now(); body='\n'.join([f'## {PHASE}','',f'- timestamp_utc: {ts}',f'- status: {STATUS}',f'- summary: {SUMMARY_REL}',f'- next_gate: {NEXT_GATE}','- boundary: clean no-REM transcript evidence captured; clean exit not proven; runtime exit crash held; source/HELP/CMDHELPCHK/DBF/CDX/LMDB/workspace mutation 0',''])
