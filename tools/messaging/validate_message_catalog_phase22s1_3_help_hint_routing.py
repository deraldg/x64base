...
def main():
    ...
    p = first_row(reports / "message_catalog_phase22s1_3_status_summary_v1.csv")
    messages = p.get("MESSAGES", "12")
    text_rows = p.get("TEXT_ROWS", "60")
    locales = p.get("LOCALES", "de;en-US;es;fr;it")
    ...
    foxhelp_count = upper.count("TRY FOXHELP")
    ...
    gate("PHASE22S1_3_REPAIR_APPLIED", p.get("STATUS") == "MESSAGE_CATALOG_PHASE22S1_3_HELP_HINT_ROUTE_RELOCATION_AND_LITERAL_REPAIR_APPLIED", p.get("STATUS", ""))
    ...
    gate("FOXHELP_FALLBACK_BYPASSED_FOR_ACTIVE_HINT", foxhelp_count == 0, f"Try FOXHELP count={foxhelp_count}")
    ...
    write_csv(reports / "message_catalog_phase22s1_runtime_status_summary_v1.csv", [{
        ...
        "HELP_HINT_ROUTING_PROOF": 1 if proof_line_count == 1 and spanish_hint_count >= 2 and token_count >= 2 and foxhelp_count == 0 else 0,
        ...
        "REPORT_TIMESTAMP_UTC": datetime.now(timezone.utc).replace(microsecond=0).isoformat().replace("+00:00", "Z"),
         ...
         "HELP_HINT_LITERAL_PLACEHOLDER_COUNT", "FOXHELP_FALLBACK_COUNT",
         "HELP_HINT_ROUTING_PROOF", "PROOF_LANE_GATED", "PROVIDER_ACTIVE_DBF",
         "ACTIVE_CATALOG_LOADED", "SOURCE_MUTATION_OBSERVED",
    ...
    print(f"  FOXHELP fallback count: {foxhelp_count}")
    print(f"  HELP_HINT routing proof: {1 if proof_line_count == 1 and spanish_hint_count >= 2 and token_count >= 2 and foxhelp_count == 0 else 0}")
