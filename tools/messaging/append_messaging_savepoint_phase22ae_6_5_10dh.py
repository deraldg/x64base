#!/usr/bin/env python3
"""Append MSG-022AE.6.5.10DH messaging savepoint with duplicate guard."""
...
SUMMARY_REL = "docs/messaging/apply/phase22ae_6_5_10dh_native_writer_runtime_proof_staging_package_v1/phase22ae_6_5_10dh_summary_v1.json"
...
def load_summary(repo: Path) -> Dict[str, object]:
    p = repo / SUMMARY_REL
    if not p.exists():
        raise SystemExit(f"10DH summary not found: {p}")
    data = json.loads(p.read_text(encoding="utf-8"))
    if data.get("status") != STATUS:
        raise SystemExit(f"10DH summary is not green: {data.get('status')}")
    return data
