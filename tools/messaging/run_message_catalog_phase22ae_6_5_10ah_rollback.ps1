...
$script = Join-Path $RepoRoot "tools\messaging\execute_message_catalog_phase22ae_6_5_10ah_guarded_final_promotion.py"

if ($PythonExe -eq "py") { & py -3.12 $script --repo-root $RepoRoot --mode rollback } else { & $PythonExe $script --repo-root $RepoRoot --mode rollback }
exit $LASTEXITCODE
