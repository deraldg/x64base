...
$script = Join-Path $RepoRoot "tools\messaging\validate_message_catalog_phase22s1_1_help_hint_routing.py"
if ($PythonExe -eq "py") { & py -3.12 $script --repo-root $RepoRoot }
else { & $PythonExe $script --repo-root $RepoRoot }
exit $LASTEXITCODE
