...
STATUS_GREEN = "MESSAGE_CATALOG_PHASE22AE_6_5_3_1_V64_RECORD_LAYOUT_REPAIR_APPLIED"
STATUS_BLOCKED = "MESSAGE_CATALOG_PHASE22AE_6_5_3_1_V64_RECORD_LAYOUT_REPAIR_BLOCKED"
NEXT_GATE = "RERUN_PHASE22AE_6_5_3_STAGE_WITH_REPLACE_EXISTING_SANDBOX"

REPORT_DIR = Path("docs/messaging/reports")
BACKUP_DIR = Path("docs/messaging/backups/MSG-022AE_6_5_3_1_V64_RECORD_LAYOUT_REPAIR_TOOL_BACKUP")
...
def parse_dbf(path: Path) -> DbfInfo:
    ...
    while pos + 32 <= len(data):
        ...
        # The previous parser treated a marker such as "<" as a real field and
        # advanced the record offset, producing 550-byte synthetic records for a
        # 403-byte SYSTEM_MESSAGE_TEXT record. For rebuild proof, only ordinary
        # field identifiers that fit inside the declared record length are safe.
...
def append_records_to_dbf(path: Path, rows: list[dict]) -> int:
    ...
    for row in rows:
        # Build by absolute offsets into the declared record length instead of
        # concatenating parsed fields. This prevents any parser/layout drift from
        # creating oversized records and keeps the sandbox DBF structurally sane.
...
def patch_file(path: Path, repo: Path, backup_root: Path, role: str):
    ...
    backup = backup_root / path.as_posix()
...
def main():
    ...
    failures = 0
    for r in rows:
        if r.get("PARSE_PATCHED") != 1 or r.get("CHANGED") != 1:
            failures += 1
    ...
    write_csv(reports / "message_catalog_phase22ae_6_5_3_1_tool_repair_rows_v1.csv",
              rows, ["FILE", "ROLE", "BACKUP", "PARSE_PATCHED", "APPEND_PATCHED", "CHANGED", "BEFORE_SHA256", "AFTER_SHA256"])
    write_csv(reports / "message_catalog_phase22ae_6_5_3_1_tool_repair_issues_v1.csv",
    ...
    write_csv(reports / "message_catalog_phase22ae_6_5_3_1_boundary_ledger_v1.csv",
              boundary, ["PROTECTED_SYSTEM", "MUTATION_ALLOWED", "OBSERVED_MUTATION", "DETAIL"])

    write_csv(reports / "message_catalog_phase22ae_6_5_3_1_status_summary_v1.csv", [{
        "STATUS": status,
