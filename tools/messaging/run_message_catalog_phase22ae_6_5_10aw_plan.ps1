param(
    ...
    [switch]$ReplaceExistingCandidate
)
...
$script = Join-Path $RepoRoot "tools\messaging\plan_message_catalog_phase22ae_6_5_10aw_help_candidate.py"
$argsList = @("--repo-root", $RepoRoot)
if ($ReplaceExistingCandidate) { $argsList += "--replace-existing-candidate" }
