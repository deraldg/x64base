...
SUMMARY_REL='docs/messaging/apply/phase22ae_6_5_10dl_runtime_proof_evidence_decision_review_v1/phase22ae_6_5_10dl_summary_v1.json'
...
def main():
    ...
    sp=repo/SUMMARY_REL
    if not sp.exists(): raise SystemExit(f'10DL summary not found: {sp}')
    summary=json.loads(sp.read_text(encoding='utf-8'))
    if summary.get('status') != STATUS: raise SystemExit(f'10DL summary is not green: {summary.get("status")}')
    ts=now()
    body='\n'.join([f'## {PHASE}','',f'- timestamp_utc: {ts}',f'- status: {STATUS}',f'- summary: {SUMMARY_REL}',f'- next_gate: {NEXT_GATE}','- boundary: runtime proof evidence decision reviewed; transcript-only evidence accepted for accounting; access-violation crash triage required; runtime proof/reuse/apply not accepted; protected-system mutation 0',''])
    sha=hashlib.sha256(body.encode()).hexdigest()
    journal=repo/'docs/messaging/MESSAGING_SAVEPOINT_JOURNAL.md'; journal.parent.mkdir(parents=True, exist_ok=True)
