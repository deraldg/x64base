param(
    ...
    [switch]$ReplaceExistingDiscovery
)
...
$script = Join-Path $RepoRoot "tools\messaging\discover_message_catalog_phase22ae_6_5_10cl_targeted_native_writers.py"
$argsList = @("--repo-root", $RepoRoot)
if ($ReplaceExistingDiscovery) { $argsList += "--replace-existing-discovery" }
