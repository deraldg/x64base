...
SAVEPOINT = "MSG-022AE.6.5.10DL-B"
...
def summary(repo: Path) -> dict:
    path = repo / "docs/messaging/reports/message_catalog_phase22ae_6_5_10dl_b_status_summary_v1.csv"
    try:
...
def main() -> int:
    ...
    if not args.accept_side_branch_savepoint:
        print("[MSG-022AE.6.5.10DL-B] Refusing without --accept-side-branch-savepoint.")
        return 1
    if status != GREEN:
        print(f"[MSG-022AE.6.5.10DL-B] Status is not green: {status}")
        return 1
    journal = repo / "docs/messaging/MESSAGING_SAVEPOINT_JOURNAL.md"
    old = read_text(journal)
    if SAVEPOINT in old:
        print("[MSG-022AE.6.5.10DL-B] Side-branch savepoint already appears in journal; refusing duplicate append.")
        return 0
    entry = f"""
## {SAVEPOINT} — {status}

- Timestamp UTC: {datetime.now(timezone.utc).replace(microsecond=0).isoformat().replace('+00:00','Z')}
- Classified rows: {row.get('CLASSIFIED_ROWS','')}
- Active runtime target candidate rows, unverified: {row.get('ACTIVE_RUNTIME_TARGET_CANDIDATE_ROWS_UNVERIFIED','')}
- Generated candidate/report rows: {row.get('GENERATED_CANDIDATE_OR_REPORT_ROWS','')}
- Source/doc/tooling evidence rows: {row.get('SOURCE_DOC_TOOLING_EVIDENCE_ROWS','')}
...
- Latest pointer changed by DL-B: {row.get('LATEST_POINTER_CHANGED_BY_DL_B','')}
- Next gate: {row.get('NEXT_GATE','')}
"""
    write_text(journal, old.rstrip() + "\n\n" + entry.lstrip())
    print("[MSG-022AE.6.5.10DL-B] Side-branch savepoint appended.")
