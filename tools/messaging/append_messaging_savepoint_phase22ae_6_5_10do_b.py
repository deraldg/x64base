...
SAVEPOINT = "MSG-022AE.6.5.10DO-B"
...
def summary(repo: Path) -> dict:
    path = repo / "docs/messaging/reports/message_catalog_phase22ae_6_5_10do_b_status_summary_v1.csv"
    try:
...
def main() -> int:
    ...
    if not args.accept_side_branch_savepoint:
        print("[MSG-022AE.6.5.10DO-B] Refusing without --accept-side-branch-savepoint.")
        return 1
    if status != GREEN:
        print(f"[MSG-022AE.6.5.10DO-B] Status is not green: {status}")
        return 1
    journal = repo / "docs/messaging/MESSAGING_SAVEPOINT_JOURNAL.md"
    old = read_text(journal)
    if SAVEPOINT in old:
        print("[MSG-022AE.6.5.10DO-B] Side-branch savepoint already appears in journal; refusing duplicate append.")
        return 0
    entry = f"""
## {SAVEPOINT} -- {status}

- Timestamp UTC: {datetime.now(timezone.utc).replace(microsecond=0).isoformat().replace('+00:00','Z')}
- Active HELP catalog mapping plan created: {row.get('ACTIVE_HELP_CATALOG_MAPPING_PLAN_CREATED','')}
- Mapping hypothesis rows: {row.get('MAPPING_HYPOTHESIS_ROWS','')}
- Mapping gap rows: {row.get('MAPPING_GAP_ROWS','')}
- LOCALE_ID gap present: {row.get('LOCALE_ID_GAP_PRESENT','')}
- CMDHELPCHK semantic gap present: {row.get('CMDHELPCHK_SEMANTIC_GAP_PRESENT','')}
...
- Latest pointer changed by DO-B: {row.get('LATEST_POINTER_CHANGED_BY_DO_B','')}
- Next gate: {row.get('NEXT_GATE','')}
"""
    write_text(journal, old.rstrip() + "\n\n" + entry.lstrip())
    print("[MSG-022AE.6.5.10DO-B] Side-branch savepoint appended.")
