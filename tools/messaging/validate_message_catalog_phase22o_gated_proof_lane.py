...
STATUS_GREEN = "MESSAGE_CATALOG_PHASE22O_GATED_ROUTING_PROOF_LANE_SMOKE_GREEN"
STATUS_BLOCKED = "MESSAGE_CATALOG_PHASE22O_GATED_ROUTING_PROOF_LANE_SMOKE_BLOCKED"
NEXT_GATE = "HOLD_OR_AUTHORIZE_PHASE22P_NEXT_LOW_RISK_ROUTING_SEAM_PLAN"
RUNLOG = Path("docs/messaging/runlog/MSG-022O_GATED_ROUTING_PROOF_SMOKE.md")

PROOF_LINE = "MESSAGE ROUTING PROOF: ACTIVE_DBF MESSAGE_LOCALE_SET"
...
def main():
    ...
    p22o = first_row(reports / "message_catalog_phase22o_status_summary_v1.csv")
    ...
    gate("PHASE22O_PATCH_APPLIED",
         p22o.get("STATUS") == "MESSAGE_CATALOG_PHASE22O_GATED_ROUTING_PROOF_LANE_PATCH_APPLIED",
         p22o.get("STATUS", ""))
    ...
    gate("PROOF_LINE_VISIBLE_ONLY_WHEN_ENABLED",
         proof_line_count == 1,
         f"proof line count should be exactly 1 in ON section; observed={proof_line_count}")
    gate("SET_LANGUAGE_ES_VISIBLE",
         "IDIOMA DE MENSAJES: ES" in upper,
         "SET LANGUAGE es output should remain visible")
    ...
    write_csv(reports / "message_catalog_phase22o_runtime_status_summary_v1.csv", [{
        "STATUS": status,
        "MESSAGES": messages,
        "TEXT_ROWS": text_rows,
        "LOCALES": locales,
        ...
        "REPORT_TIMESTAMP_UTC": datetime.now(timezone.utc).replace(microsecond=0).isoformat().replace("+00:00", "Z"),
         ...
         "PROOF_MODE_ON_COUNT", "PROOF_MODE_OFF_COUNT", "PROOF_LINE_COUNT",
    ...
    write_csv(reports / "message_catalog_phase22o_runtime_gate_check_v1.csv",
              gates,
              ["GATE", "STATUS", "DETAIL"])

    boundary = [
        {"PROTECTED_SYSTEM": "SOURCE_CODE", "MUTATION_ALLOWED": 0, "OBSERVED_MUTATION": 0, "DETAIL": "Phase 22O runtime validation only; no source mutation."},
    ...
    write_csv(reports / "message_catalog_phase22o_runtime_boundary_ledger_v1.csv",
              boundary,
    ...
    print(f"  proof line count: {proof_line_count}")
