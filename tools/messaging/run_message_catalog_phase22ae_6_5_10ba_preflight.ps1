param(
    ...
    [switch]$ReplaceExistingPreflight
)
...
$script = Join-Path $RepoRoot "tools\messaging\preflight_message_catalog_phase22ae_6_5_10ba_apply_execution.py"
$argsList = @("--repo-root", $RepoRoot)
if ($ReplaceExistingPreflight) { $argsList += "--replace-existing-preflight" }
