...
def main():
    ...
    cmd = [
        ...
        "--lane", "MESSAGING",
        "--status", status,
        "--phase", "Phase 22S1 HELP_HINT_COMMAND runtime routing",
        "--summary", "HELP hint routing smoke green: narrow HELP fallback hint routes through active DBF provider symbol HELP_HINT_COMMAND, command placeholder substitutes, proof lane is shared/gated, and HELP DATA/CMDHELPCHK remain untouched.",
        "--next-gate", row.get("NEXT_GATE", "HOLD_OR_AUTHORIZE_PHASE22T_RUNTIME_ROUTING_CLOSEOUT_OR_NEXT_SEAM"),
        "--source-reports", "docs/messaging/reports/message_catalog_phase22s1_status_summary_v1.csv;docs/messaging/reports/message_catalog_phase22s1_source_mutation_inventory_v1.csv;docs/messaging/reports/message_catalog_phase22s1_runtime_status_summary_v1.csv;docs/messaging/reports/message_catalog_phase22s1_runtime_gate_check_v1.csv;docs/messaging/runlog/MSG-022S1_HELP_HINT_ROUTING_SMOKE.md",
        "--messages", row.get("MESSAGES", "12"),
        "--text-rows", row.get("TEXT_ROWS", "60"),
        "--locales", row.get("LOCALES", "de;en-US;es;fr;it"),
        "--validation-issues", row.get("VALIDATION_ISSUES", "0"),
        "--allowed-candidate-mutations", "none; Phase 22S1 runtime validation after authorized source patch",
