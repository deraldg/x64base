#!/usr/bin/env python3
"""Append Phase 18.2 Messaging savepoint for active promotion with DTX memo-sidecar repair."""
...
def main() -> int:
    ...
    if not args.accept_messaging_savepoint:
        print("[MSG-018.2] Refusing without --accept-messaging-savepoint", file=sys.stderr)
        return 2
    ...
    if status != STATUS_EXPECTED:
        print(f"[MSG-018.2] Refusing savepoint: expected {STATUS_EXPECTED}, got {status}", file=sys.stderr)
        return 2

    generic = repo / "tools/messaging/append_messaging_savepoint.py"
    summary_text = (
        "Active Messaging catalog promotion green with DTX memo-sidecar repair: proven x64 DBF/CDX/LMDB candidate promoted to active messaging data layout. "
    ...
    cmd = [
        ...
        "--repo-root", str(repo),
        "--savepoint-id", "MSG-018.2",
        "--lane", "MESSAGING",
        "--status", "MESSAGE_CATALOG_PHASE18_2_ACTIVE_PROMOTION_GREEN",
        "--phase", "Phase 18.2 active Messaging catalog promotion DTX sidecar repair",
        ...
        "--boundary-summary", "authorized active Messaging catalog promotion: active messaging DBF/CDX/LMDB artifacts were replaced from the proven x64 candidate; .dtx memo sidecar discovered and promoted; no HELP DATA, CMDHELPCHK, source-mining, source, manualgen, datadict, or unrelated active catalog mutation",
        "--allowed-candidate-mutations", "none; Phase 18.2 consumes candidate artifacts",
