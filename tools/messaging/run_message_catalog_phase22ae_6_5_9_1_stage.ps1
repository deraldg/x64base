param(
  ...
  [string]$PythonExe = "",
  [switch]$AllowCandidateCdxRepair,
...
$script = Join-Path $RepoRoot "tools\messaging\stage_message_catalog_phase22ae_6_5_9_1_candidate_cdx_setpath_repair.py"
$argsList = @("--repo-root", $RepoRoot)
if ($AllowCandidateCdxRepair) { $argsList += "--allow-candidate-cdx-repair" }
