...
SAVEPOINT = "MSG-022AE.6.5.10DD-B"
...
def summary(repo: Path) -> dict:
    path = repo / "docs/messaging/reports/message_catalog_phase22ae_6_5_10dd_b_status_summary_v1.csv"
    try:
...
def main() -> int:
    ...
    if not args.accept_side_branch_savepoint:
        print("[MSG-022AE.6.5.10DD-B] Refusing without --accept-side-branch-savepoint.")
        return 1
    if status != GREEN:
        print(f"[MSG-022AE.6.5.10DD-B] Status is not green: {status}")
        return 1

    journal = repo / "docs/messaging/MESSAGING_SAVEPOINT_JOURNAL.md"
    old = read_text(journal)
    if SAVEPOINT in old:
        print("[MSG-022AE.6.5.10DD-B] Side-branch savepoint already appears in journal; refusing duplicate append.")
        return 0
...
- Files scanned: {row.get('FILES_SCANNED','')}
- Target candidate rows: {row.get('TARGET_CANDIDATE_ROWS','')}
- Likely target review rows: {row.get('LIKELY_TARGET_REVIEW_ROWS','')}
...
- Target discovery review required: {row.get('TARGET_DISCOVERY_REVIEW_REQUIRED','')}
...
- Latest pointer changed by DD-B: {row.get('LATEST_POINTER_CHANGED_BY_DD_B','')}
- Next gate: {row.get('NEXT_GATE','')}
"""
    write_text(journal, old.rstrip() + "\n\n" + entry.lstrip())
    print("[MSG-022AE.6.5.10DD-B] Side-branch savepoint appended.")
