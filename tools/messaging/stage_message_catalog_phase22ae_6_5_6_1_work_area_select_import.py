...
import argparse, csv, hashlib, json, shutil, struct
from datetime import datetime, timezone
from pathlib import Path
from typing import Any

STATUS_GREEN = "MESSAGE_CATALOG_PHASE22AE_6_5_6_1_WORK_AREA_SELECT_IMPORT_PROOF_STAGED_SOURCE_HELD"
STATUS_BLOCKED = "MESSAGE_CATALOG_PHASE22AE_6_5_6_1_WORK_AREA_SELECT_IMPORT_PROOF_STAGING_BLOCKED"
NEXT_GATE = "RUN_PHASE22AE_6_5_6_1_WORK_AREA_SELECT_RUNTIME_THEN_VALIDATE"

REPORT_DIR = Path("docs/messaging/reports")
SANDBOX_ROOT = Path("docs/messaging/sandbox/phase22ae_6_5_6_1_work_area_select_import_v1")
ABS_SCRIPT_PATH = Path("docs/messaging/scripts/MESSAGE_CATALOG_PHASE22AE_6_5_6_1_WORK_AREA_SELECT_IMPORT_ABSOLUTE_PATH_PROOF.dts")
PATH_SETUP_SCRIPT = Path("docs/messaging/scripts/MESSAGE_CATALOG_PHASE22AE_6_5_6_1_DBF_PATH_LOCATION_SETUP.dts")
...
UNIQUE_MESSAGE_DBF = "MSG6561_MESSAGES_NATIVE_IMPORT.dbf"
UNIQUE_TEXT_DBF = "MSG6561_TEXT_NATIVE_IMPORT.dbf"
    ...
    for source_row in source_rows:
        ...
        for f in field_names(info):
            ...
            if f == sym_field:
                ...
                val = pick(src, f, SOURCE_ALIASES) or "22AE_6_5_6_1"
            elif f == kind_field:
...
def main() -> int:
    ...
    val656 = first_row(reports / "message_catalog_phase22ae_6_5_6_validate_status_summary_v1.csv")
    stage656 = first_row(reports / "message_catalog_phase22ae_6_5_6_stage_status_summary_v1.csv")
    ...
    # Accept either a saved blocked 6.5.6 validation or just green 6.5.6 staging.
    gate("PHASE22AE_6_5_6_EVIDENCE_PRESENT",
         stage656.get("STATUS") == "MESSAGE_CATALOG_PHASE22AE_6_5_6_NATIVE_IMPORT_CSV_WRITE_PROOF_STAGED_SOURCE_HELD",
         f"stage={stage656.get('STATUS','missing')}; validate={val656.get('STATUS','not-run')}")
    ...
    write_csv(reports / "message_catalog_phase22ae_6_5_6_1_active_fingerprint_before_v1.csv",
    ...
    if failures == 0:
        try:
            ...
            msg_import_csv = import_dir / "MSG6561_MESSAGES_NATIVE_IMPORT.csv"
            txt_import_csv = import_dir / "MSG6561_TEXT_NATIVE_IMPORT.csv"
            ...
            abs_script.write_text("\n".join([
                "* MESSAGE_CATALOG_PHASE22AE_6_5_6_1_WORK_AREA_SELECT_IMPORT_ABSOLUTE_PATH_PROOF.dts",
                "* Native DotTalk++ IMPORT proof with explicit work-area discipline.",
                "* Absolute DBF and CSV paths are authoritative.",
                "SELECT 1",
                f"USE {unique_msg.resolve().as_posix()}",
                f"IMPORT {msg_import_csv.resolve().as_posix()}",
                "SELECT MSG6561_MESSAGES_NATIVE_IMPORT",
                "COUNT",
                "SELECT 2",
                f"USE {unique_txt.resolve().as_posix()}",
                f"IMPORT {txt_import_csv.resolve().as_posix()}",
                "SELECT MSG6561_TEXT_NATIVE_IMPORT",
                "COUNT",
                "",
            ]), encoding="utf-8")
            ...
            path_script.write_text("\n".join([
                "* MESSAGE_CATALOG_PHASE22AE_6_5_6_1_DBF_PATH_LOCATION_SETUP.dts",
                "* Convenience helper for DBF path-location workflows.",
                "* Absolute path proof remains authoritative.",
    ...
    write_csv(reports / "message_catalog_phase22ae_6_5_6_1_stage_gate_check_v1.csv",
              gates, ["GATE", "STATUS", "DETAIL"])
    write_csv(reports / "message_catalog_phase22ae_6_5_6_1_sandbox_copy_inventory_v1.csv",
              copy_rows, ["ROLE", "SOURCE", "TARGET", "BYTES", "SHA256"])
    write_csv(reports / "message_catalog_phase22ae_6_5_6_1_import_field_mapping_v1.csv",
              mapping_rows, ["TABLE", "TARGET_FIELD", "SAMPLE_VALUE"])
    write_csv(reports / "message_catalog_phase22ae_6_5_6_1_expected_message_keys_v1.csv",
              expected_msg_rows, ["SYMBOL_FIELD", "SYMBOL"])
    write_csv(reports / "message_catalog_phase22ae_6_5_6_1_expected_text_keys_v1.csv",
    ...
    boundary = [
        ...
        {"PROTECTED_SYSTEM": "ACTIVE_MESSAGING_DBF_CATALOG", "MUTATION_ALLOWED": 0, "OBSERVED_MUTATION": 0, "DETAIL": "Fresh sandbox DBF copies only."},
    ...
    write_csv(reports / "message_catalog_phase22ae_6_5_6_1_stage_boundary_ledger_v1.csv",
              boundary, ["PROTECTED_SYSTEM", "MUTATION_ALLOWED", "OBSERVED_MUTATION", "DETAIL"])

    write_csv(reports / "message_catalog_phase22ae_6_5_6_1_stage_status_summary_v1.csv", [{
        "STATUS": status,
        "VALIDATION_ISSUES": validation_issues,
        "PHASE22AE_6_5_6_EVIDENCE_PRESENT": 1 if stage656.get("STATUS") == "MESSAGE_CATALOG_PHASE22AE_6_5_6_NATIVE_IMPORT_CSV_WRITE_PROOF_STAGED_SOURCE_HELD" else 0,
        ...
        "TEXT_FIELD_TYPE": text_field_type,
        "WORK_AREA_DISCIPLINE_INCLUDED": 1,
        "SELECT_TABLE_PROOF_INCLUDED": 1,
    ...
    }], ["STATUS", "VALIDATION_ISSUES", "PHASE22AE_6_5_6_EVIDENCE_PRESENT",
    ...
    print(f"  Phase 22AE.6.5.6 evidence present: {1 if stage656.get('STATUS') == 'MESSAGE_CATALOG_PHASE22AE_6_5_6_NATIVE_IMPORT_CSV_WRITE_PROOF_STAGED_SOURCE_HELD' else 0}")
    ...
    print(f"  work-area discipline included: 1")
    print(f"  select table proof included: 1")
    print(f"  absolute path names supported by proof: 1")
