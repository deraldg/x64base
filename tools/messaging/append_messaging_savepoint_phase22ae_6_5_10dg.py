#!/usr/bin/env python3
"""Append MSG-022AE.6.5.10DG messaging savepoint with duplicate guard."""
...
SUMMARY_REL = "docs/messaging/apply/phase22ae_6_5_10dg_native_writer_reuse_decision_review_and_runtime_proof_plan_v1/phase22ae_6_5_10dg_summary_v1.json"
...
def load_summary(repo: Path) -> Dict[str, object]:
    p = repo / SUMMARY_REL
    if not p.exists():
        raise SystemExit(f"10DG summary not found: {p}")
    data = json.loads(p.read_text(encoding="utf-8"))
    if data.get("status") != STATUS:
        raise SystemExit(f"10DG summary is not green: {data.get('status')}")
    return data
...
def update_index(index: Path, entry: Dict[str, str]) -> None:
    ...
    for f in preferred:
        if f not in fields:
            fields.append(f)
    # Fill likely alternate column names too, without losing existing schema.
...
def main() -> int:
    ...
    entry_sha = hashlib.sha256(entry_body.encode("utf-8")).hexdigest()
