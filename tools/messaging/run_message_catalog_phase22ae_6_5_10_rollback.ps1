...
$script = Join-Path $RepoRoot "tools\messaging\rollback_message_catalog_phase22ae_6_5_10_active_promotion.py"
$argsList = @("--repo-root", $RepoRoot)
if ($BackupRoot) { $argsList += @("--backup-root", $BackupRoot) }
if ($AllowActiveCatalogRollback) { $argsList += "--allow-active-catalog-rollback" }
