...
def main() -> int:
    ap = argparse.ArgumentParser(description="DD-073 guarded DDICT FIELDS implementation")
    ap.add_argument("--repo-root", required=True)
    ap.add_argument("--out-dir", required=True)
    ap.add_argument("--run-id", default="DD073-guarded-ddict-fields-implementation-v0")
    ap.add_argument("--dd072-dir", default="docs/datadict/reports/DD072-ddict-fields-tags-schema-plan-v0")
    ...
    dd072_dir = (repo / args.dd072_dir).resolve()
    dd072_manifest = read_json(dd072_dir / "dd072_ddict_fields_tags_schema_plan_manifest.json")
    ...
    (out / "dd073_cmd_ddict_fields_patch_preview.diff").write_text(preview, encoding="utf-8")

    dd072_green = int(dd072_manifest.get("status") == EXPECTED_DD072_STATUS)
    source_exists = int(source.exists())
    existing_has_status_tables = int("print_status" in old and "print_tables" in old)
    generated_has_fields = int("void print_fields" in DDICT_CPP and 'sub == "FIELDS"' in DDICT_CPP)
    generated_preserves_tags_pending = int('sub == "TAGS"' in DDICT_CPP and "print_pending" in DDICT_CPP)
    generated_readonly = int("READ-ONLY" in DDICT_CPP and "BUILDLMDB" not in DDICT_CPP and "REPLACE" not in DDICT_CPP)

    review_rows: List[Dict[str, Any]] = []
    if not dd072_green:
        review_rows.append({"issue": "DD072_NOT_READY", "detail": dd072_manifest.get("status", "")})
    if not source_exists:
        review_rows.append({"issue": "SOURCE_MISSING", "detail": str(source)})
    if not existing_has_status_tables:
        review_rows.append({"issue": "STATUS_TABLES_BASELINE_NOT_DETECTED", "detail": "existing cmd_ddict.cpp does not appear to contain DD-070 status/tables baseline"})
    ...
    if args.apply_source_patch and patched and failures == 0:
        status = "DDICT_FIELDS_SOURCE_PATCH_APPLIED_BUILD_REQUIRED"
    elif failures == 0:
        status = "DDICT_FIELDS_SOURCE_PATCH_READY"
    else:
        status = "DDICT_FIELDS_SOURCE_PATCH_REVIEW"

    gate_rows = [
        {"gate": "dd072_schema_plan_ready", "expected": EXPECTED_DD072_STATUS, "observed": dd072_manifest.get("status", ""), "pass": dd072_green},
        {"gate": "cmd_ddict_source_exists", "expected": 1, "observed": source_exists, "pass": source_exists},
        {"gate": "status_tables_baseline_detected", "expected": 1, "observed": existing_has_status_tables, "pass": existing_has_status_tables},
        {"gate": "generated_fields_surface", "expected": 1, "observed": generated_has_fields, "pass": generated_has_fields},
        {"gate": "generated_tags_pending_preserved", "expected": 1, "observed": generated_preserves_tags_pending, "pass": generated_preserves_tags_pending},
    ...
    boundary_rows = [
        {"boundary": "guarded_fields_source_patch", "observed": 1, "required": 1, "pass": 1},
    ...
    write_csv(out / "dd073_gate_ledger.csv", gate_rows, ["gate", "expected", "observed", "pass"])
    write_csv(out / "dd073_review_rows.csv", review_rows, ["issue", "detail"])
    write_csv(out / "dd073_no_mutation_boundary_ledger.csv", boundary_rows, ["boundary", "observed", "required", "pass"])

    report = f"""# DD-073 Guarded DDICT FIELDS Implementation
...
DD-073 implements only:

```text
DDICT FIELDS <table>
```

It preserves `DDICT STATUS` and `DDICT TABLES`, and keeps `DDICT TAGS` pending for DD-074.

## Target

- Source: `{rel(repo, source)}`
- Generated candidate: `{rel(repo, generated_source)}`
- Patch preview: `{rel(repo, out / 'dd073_cmd_ddict_fields_patch_preview.diff')}`
...
DD-073 edits only `cmd_ddict.cpp` when `--apply-source-patch` is supplied.
...
"""
    (out / "DD073_GUARDED_DDICT_FIELDS_IMPLEMENTATION_REPORT.md").write_text(report, encoding="utf-8")

    manifest = {
        "contract": "dd073_guarded_ddict_fields_impl_v0",
        "run_id": args.run_id,
        "created_utc": utc_now(),
        "status": status,
        "repo_root": str(repo),
        "profiles": args.profile,
        "dd072_status": dd072_manifest.get("status", ""),
        ...
        "next_recommended_action": "Build DotTalk++ and run DDICT FIELDS DDOBJECT/DDATTR/DDEDGE smoke; then DD-073 runtime closure.",
    }
    write_json(out / "dd073_guarded_ddict_fields_impl_manifest.json", manifest)

    print(f"DD-073 guarded DDICT FIELDS manifest: {out / 'dd073_guarded_ddict_fields_impl_manifest.json'}")
