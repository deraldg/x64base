...
def main() -> int:
    ap = argparse.ArgumentParser(description="DD096F-R root-aware staged-row review and simulated apply validation")
    ap.add_argument("--repo-root", required=True)
    ap.add_argument("--out-dir", required=True)
    ap.add_argument("--run-id", default="DD096F-R-root-aware-staged-row-simulated-apply-v0")
    ...
    for r in staged_attr:
        ...
        ref_rows.append({"family": "DDATTR", "row_id": r.get("attrid", ""), "reference_field": "objid", "reference_value": r.get("objid", ""), "reference_ok": int(ok)})
    missing_edge_refs = 0
    for r in staged_edge:
        for field in ["from_objid", "to_objid"]:
            ok = r.get(field, "") in allowed_objids
            missing_edge_refs += 0 if ok else 1
            ref_rows.append({"family": "DDEDGE", "row_id": r.get("edgeid", ""), "reference_field": field, "reference_value": r.get(field, ""), "reference_ok": int(ok)})
    ...
    simulated_sequence = [
        {"step": 1, "phase": "verify", "description": "Verify root-aware staged files and green prerequisites.", "simulated_only": 1, "apply_now": 0},
        ...
        {"step": 3, "phase": "validate_refs", "description": "Validate DDATTR and DDEDGE references against active reused OBJIDs plus 9 staged new OBJIDs.", "simulated_only": 1, "apply_now": 0},
        {"step": 4, "phase": "duplicate_check", "description": "Check 9 staged DDOBJECT rows for active duplicate risk.", "simulated_only": 1, "apply_now": 0},
    ...
    validation_rows = [
        ...
        {"validation": "staged_index_count_root_aware", "expected": 158, "observed": staged_total, "pass": int(staged_total == 158)},
        {"validation": "simulated_insert_total_root_aware", "expected": 158, "observed": simulated_insert_total, "pass": int(simulated_insert_total == 158)},
        {"validation": "suppressed_existing_objects", "expected": 11, "observed": len(suppressed_obj), "pass": int(len(suppressed_obj) == 11)},
        {"validation": "new_objects_root_aware", "expected": 9, "observed": len(staged_obj), "pass": int(len(staged_obj) == 9)},
        {"validation": "staged_attrs_root_aware", "expected": 126, "observed": len(staged_attr), "pass": int(len(staged_attr) == 126)},
    ...
    boundary_rows = [
        {"boundary": "root_aware_staged_row_review_simulation_only", "observed": 1, "required": 1, "pass": 1},
    ...
    write_csv(out / "dd096f_next_lane_recommendations.csv", [
        {"next_id": "DD096G", "title": "final guarded apply package design", "allowed_scope": "design only until explicit authorization"},
        {"next_id": "DD096H", "title": "apply authorization record / go-no-go checkpoint", "allowed_scope": "authorization record only"},
    ...
    report = f"""# DD096F-R Root-Aware Staged-Row Review and Simulated Apply Validation
...
DD096F-R validates the DD096E-R root-aware staged rows as if they were going to be applied, but only in memory/report form.
...
- Staged DDATTR rows: **{len(staged_attr)}**
...
DD096F-R is root-aware staged-row-review/simulation-only. It does not edit C++ source, edit build files,
...
"""
    write_text(out / "DD096F_ROOT_AWARE_STAGED_ROW_SIMULATED_APPLY_REPORT.md", report)

    manifest = {
        "contract": "dd096fr_root_aware_staged_row_simulated_apply_v0",
        "run_id": args.run_id,
        "created_utc": utc_now(),
        "status": status,
        "repo_root": str(repo),
    ...
    write_json(out / "dd096fr_root_aware_staged_row_simulated_apply_manifest.json", manifest)

    print(f"DD096F-R root-aware staged-row simulated apply manifest: {out / 'dd096fr_root_aware_staged_row_simulated_apply_manifest.json'}")
