...
def main() -> int:
    ap = argparse.ArgumentParser(description="DD-078 guarded DDICT REL implementation")
    ap.add_argument("--repo-root", required=True)
    ap.add_argument("--out-dir", required=True)
    ap.add_argument("--run-id", default="DD078-guarded-ddict-rel-implementation-v0")
    ap.add_argument("--dd077-dir", default="docs/datadict/reports/DD077-ddict-rel-representation-plan-v0")
    ...
    dd077_dir = (repo / args.dd077_dir).resolve()
    dd077_manifest = read_json(dd077_dir / "dd077_ddict_rel_representation_plan_manifest.json")
    ...
    (out / "dd078_cmd_ddict_rel_patch_preview.diff").write_text(preview, encoding="utf-8")

    dd077_green = int(dd077_manifest.get("status") == EXPECTED_DD077_STATUS)
    source_exists = int(source.exists())
    existing_has_tags = int("print_tags" in old and 'sub == "TAGS"' in old)
    generated_has_rel = int("void print_rel" in DDICT_CPP and 'sub == "REL"' in DDICT_CPP)
    generated_preserves_tags = int("void print_tags" in DDICT_CPP and 'sub == "TAGS"' in DDICT_CPP)
    generated_readonly = int("READ-ONLY" in DDICT_CPP and "BUILDLMDB" not in DDICT_CPP and "CDX ADDTAG" not in DDICT_CPP and "REPLACE" not in DDICT_CPP)

    review_rows: List[Dict[str, Any]] = []
    if not dd077_green:
        review_rows.append({"issue": "DD077_NOT_READY", "detail": dd077_manifest.get("status", "")})
    if not source_exists:
        review_rows.append({"issue": "SOURCE_MISSING", "detail": str(source)})
    if not existing_has_tags:
        review_rows.append({"issue": "TAGS_BASELINE_NOT_DETECTED", "detail": "existing cmd_ddict.cpp does not appear to contain DD-075 TAGS baseline"})
    ...
    if args.apply_source_patch and patched and failures == 0:
        status = "DDICT_REL_SOURCE_PATCH_APPLIED_BUILD_REQUIRED"
    elif failures == 0:
        status = "DDICT_REL_SOURCE_PATCH_READY"
    else:
        status = "DDICT_REL_SOURCE_PATCH_REVIEW"

    gate_rows = [
        {"gate": "dd077_representation_plan_ready", "expected": EXPECTED_DD077_STATUS, "observed": dd077_manifest.get("status", ""), "pass": dd077_green},
        {"gate": "cmd_ddict_source_exists", "expected": 1, "observed": source_exists, "pass": source_exists},
        {"gate": "tags_baseline_detected", "expected": 1, "observed": existing_has_tags, "pass": existing_has_tags},
        {"gate": "generated_rel_surface", "expected": 1, "observed": generated_has_rel, "pass": generated_has_rel},
        {"gate": "generated_tags_preserved", "expected": 1, "observed": generated_preserves_tags, "pass": generated_preserves_tags},
    ...
    boundary_rows = [
        {"boundary": "guarded_rel_source_patch", "observed": 1, "required": 1, "pass": 1},
    ...
    write_csv(out / "dd078_gate_ledger.csv", gate_rows, ["gate", "expected", "observed", "pass"])
    write_csv(out / "dd078_review_rows.csv", review_rows, ["issue", "detail"])
    write_csv(out / "dd078_no_mutation_boundary_ledger.csv", boundary_rows, ["boundary", "observed", "required", "pass"])

    report = f"""# DD-078 Guarded DDICT REL Implementation
...
The implementation is read-only. It resolves a catalog object and reports bounded incoming/outgoing DDEDGE rows decorated with DDOBJECT metadata.

## Target

- Source: `{rel(repo, source)}`
- Generated candidate: `{rel(repo, generated_source)}`
- Patch preview: `{rel(repo, out / 'dd078_cmd_ddict_rel_patch_preview.diff')}`
...
DD-078 edits only `cmd_ddict.cpp` when `--apply-source-patch` is supplied.
...
"""
    (out / "DD078_GUARDED_DDICT_REL_IMPLEMENTATION_REPORT.md").write_text(report, encoding="utf-8")

    manifest = {
        "contract": "dd078_guarded_ddict_rel_impl_v0",
        "run_id": args.run_id,
        "created_utc": utc_now(),
        "status": status,
        "repo_root": str(repo),
        "profiles": args.profile,
        "dd077_status": dd077_manifest.get("status", ""),
        ...
        "next_recommended_action": "Build DotTalk++ and run DDICT REL DDOBJECT/DDATTR/DDEDGE smoke; then DD-079 closure.",
    }
    write_json(out / "dd078_guarded_ddict_rel_impl_manifest.json", manifest)

    print(f"DD-078 guarded DDICT REL manifest: {out / 'dd078_guarded_ddict_rel_impl_manifest.json'}")
