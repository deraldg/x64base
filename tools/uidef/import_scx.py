...
KINDMAP = {
    ...
    'image':'image',
    # R67. `grid` was refused in v1 until R66 gave it a KIND whose columns are
    # DECLARED by BINDING rather than generated by ColumnCount -- which answers R6
    # instead of overruling it. Measured over the corpus before mapping it: 33 grid
    # objects, 17 of which state every bound column as `alias.field` and can produce
    # a tuple spec; the other 16 are refused BY NAME below.
    'grid':'grid',
}
...
COMPOSITE = {
    ...
    # R67: `grid` is NOT here any more. Its columns become the tuple spec in
    # BINDING (contract 10c), not child rows -- a grid that carried both would hold
    # two copies of its own column list, which is the drift 4b(a) forbids for
    # `tree` and `summary` for the same reason.
}
...
LAST_GRID_REFUSED = []      # R67: [(objname, reason)] -- named, never dropped in silence
...
def grid_spec(p):
    """A VFP grid's columns as a UIDEF tuple spec, or a named refusal. R67.

    Contract 10c. Measured over the 195-file corpus, 33 grid objects:

        Column<n>.ControlSource shapes : 99 alias.field, 42 absent/empty,
                                         3 expression, 0 bare field
        grids whose bound columns are all alias.field : 17
        of those, satisfying 10c's Relation rule      : 17 of 17
        grids declaring ReadOnly = .F.                : 0 of 33

    That last number is why 4b(b) costs nothing: not one document in the corpus
    would be refused by the read-only rule. Seven state `ReadOnly = .T.` and the
    rest are silent.

    Returns (spec, None) or (None, reason).
    """
    ...
    if cc in ('-1', '-1.0'):
        # VFP's -1 means "build the columns at runtime from the RecordSource".
        return None, ('ColumnCount = -1: the columns are generated at runtime from '
                      'the RecordSource, which is R6\'s implicit children exactly '
                      'and is still out of v1')
    if str(p.get('recordsourcetype', '')).strip() in ('3', '4'):
        # 3 = query (.qpr), 4 = SQL statement. Neither is a work area, and SOURCE
        # can only name work areas (R62.2, contract section 13).
        return None, ('RecordSourceType %s: the source is a query or a SQL '
                      'statement, not a work area, and SOURCE can only name work '
                      'areas (contract section 13)'
                      % str(p.get('recordsourcetype')).strip())
    cols = sorted({int(m.group(1)) for k in p
                   for m in [re.match(r'column(\d+)\.', k.strip().lower())] if m})
    srcs = []
    for i in cols:
        v = (p.get('column%d.controlsource' % i) or '').strip().strip('"').strip("'")
        if not v:
            continue
        head = v.split('.', 1)[0].strip().lower()
        if v.count('.') != 1 or head in ('this', 'thisform', 'thisformset'):
            return None, ('column %d\'s ControlSource is %s, not alias.field'
                          % (i, 'an object reference'
                             if head in ('this', 'thisform', 'thisformset')
                             else 'a computed expression'))
        srcs.append(v)
    if not srcs:
        return None, ('no column carries a ControlSource, so the columns are not '
                      'declared anywhere the document can be read from')
    return ','.join(srcs), None
...
def convert(scx_path, out_stem):
    ...
    for r in objs:
        ...
        if b == 'grid':
            # R67. The columns ARE the binding (contract 10c). A grid that cannot
            # state its columns is refused BY NAME rather than emitted bound to
            # nothing -- R28.1's rule: v1 may decline to carry something, but it
            # may not lose it in silence.
            spec, why = grid_spec(p)
            if why:
                grid_refused.append(((r.get('OBJNAME') or '').strip() or oid, why))
                continue
            binding = spec
            # A column's DECLARED WIDTH is measured design evidence -- the same
            # evidence R17 and R25 use to check a bound control -- and the first
            # version of this branch threw it away with the rest of the dotted
            # keys. `Column<n>.Width` is present on 23 of 33 corpus grids. Kept as
            # one ordinal-aligned list rather than as child rows, so the grid still
            # holds exactly one copy of its column list (4b(a)'s reasoning).
            wid = []
            for i in sorted({int(m.group(1)) for k in p
                             for m in [re.match(r'column(\d+)\.', k.strip().lower())]
                             if m}):
                if (p.get('column%d.controlsource' % i) or '').strip().strip('"'):
                    wid.append(str(p.get('column%d.width' % i, '')).strip())
            for dk in [k for k in keep
                       if re.match(r'column\d+\.', k.strip().lower())]:
                keep.pop(dk, None)
            if any(wid):
                keep['ColumnWidths'] = ','.join(wid)
            # Contract 4b(b); measured, 0 of 33 corpus grids say otherwise. Drop
            # whatever case the source used first -- writing 'ReadOnly' beside a
            # surviving 'readonly' put the property in twice, which a reader would
            # have to break a tie on.
            for dk in [k for k in keep if k.strip().lower() == 'readonly']:
                keep.pop(dk, None)
            keep['ReadOnly'] = '.T.'
    ...
    global LAST_IMPLIED, LAST_MATERIALISED, LAST_COUNT_MISMATCH, LAST_GRID_REFUSED
    LAST_GRID_REFUSED = grid_refused
