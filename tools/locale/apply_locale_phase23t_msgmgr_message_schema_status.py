...
STATUS_APPLIED = "LOCALE_PHASE23T_MSGMGR_MESSAGE_SCHEMA_STATUS_SOURCE_PATCH_APPLIED_BUILD_HELD"
STATUS_BLOCKED = "LOCALE_PHASE23T_MSGMGR_MESSAGE_SCHEMA_STATUS_SOURCE_PATCH_BLOCKED"
NEXT_GATE = "BUILD_AND_RUN_MSGMGR_MESSAGE_SCHEMA_STATUS_SMOKE_THEN_VALIDATE"
REPORT_DIR = Path("docs/locale/reports")
BACKUP_ROOT = Path("docs/locale/backups")

CMD_MSGMGR = Path("src/cli/cmd_msgmgr.cpp")
SMOKE = Path("docs/locale/scripts/LOCALE_PHASE23T_MSGMGR_MESSAGE_SCHEMA_STATUS_SMOKE.dts")
ACTIVE_MESSAGE_SCHEMA = Path("dottalkpp/data/schemas/messaging/message_catalog.dtschema")

OLD_LINE = '        << "  messaging schema     : held; field/tag reconciliation pending\\n"'
NEW_LINE = '        << "  messaging schema     : dottalkpp/data/schemas/messaging/message_catalog.dtschema\\n"'

SMOKE_TEXT = """* LOCALE_PHASE23T_MSGMGR_MESSAGE_SCHEMA_STATUS_SMOKE.dts
* MSGMGR active message-schema status reporting smoke.
...
def patch_cmd_msgmgr(text: str) -> tuple[str, str]:
    if "messaging schema     : dottalkpp/data/schemas/messaging/message_catalog.dtschema" in text:
        return text, "already_present"
    if OLD_LINE not in text:
        return text, "anchor_missing"
    return text.replace(OLD_LINE, NEW_LINE, 1), "replaced_held_with_active_message_schema_path"
...
def main() -> int:
    ...
    phase23s = first_row(reports / "locale_phase23s_message_schema_validation_status_summary_v1.csv")
    phase23p = first_row(reports / "locale_phase23p_msgmgr_schema_status_validation_summary_v1.csv")

    cmd = repo / CMD_MSGMGR
    smoke = repo / SMOKE
    active_schema = repo / ACTIVE_MESSAGE_SCHEMA
    ...
    gate("PHASE23S_MESSAGE_SCHEMA_VALIDATED_GREEN",
         phase23s.get("STATUS") == "LOCALE_PHASE23S_MESSAGE_CATALOG_ACTIVE_SCHEMA_VALIDATED_GREEN",
         phase23s.get("STATUS", ""))
    gate("PHASE23S_VALIDATION_ZERO",
         phase23s.get("VALIDATION_ISSUES", "") == "0",
         f"validation_issues={phase23s.get('VALIDATION_ISSUES', '')}")
    gate("ACTIVE_MESSAGE_SCHEMA_PRESENT",
         active_schema.exists(),
         rel(active_schema, repo))
    gate("PHASE23P_SCHEMA_STATUS_GREEN",
         phase23p.get("STATUS") == "LOCALE_PHASE23P_MSGMGR_SCHEMA_STATUS_BUILD_SMOKE_GREEN",
         phase23p.get("STATUS", ""))
    gate("CMD_MSGMGR_PRESENT",
         cmd.exists(),
         rel(cmd, repo))
    ...
    backup_root = repo / BACKUP_ROOT / f"LOC-023T_MSGMGR_MESSAGE_SCHEMA_STATUS_BACKUP_{timestamp}"
    ...
    if failures == 0:
        ...
        if patch_status == "anchor_missing":
            ...
            mutation_rows.append({
                ...
                "ACTION": "CREATE_OR_REPLACE_MESSAGE_SCHEMA_STATUS_SMOKE",
    ...
    boundary = [
        ...
        {"PROTECTED_SYSTEM": "ACTIVE_SCHEMA_CONTRACTS", "MUTATION_ALLOWED": 0, "OBSERVED_MUTATION": 0, "DETAIL": "No active schema mutation; report existing promoted Messaging schema path only."},
    ...
    write_csv(reports / "locale_phase23t_msgmgr_message_schema_status_apply_summary_v1.csv", [{
        "STATUS": status,
        ...
        "REPORT_TIMESTAMP_UTC": datetime.now(timezone.utc).replace(microsecond=0).isoformat().replace("+00:00", "Z"),
         ...
         "SOURCE_FILES_MUTATED", "DOCS_LOCALE_FILES_MUTATED", "BACKUP_ROWS",
         "PATCH_STATUS", "BUILD_EXECUTED", "RUNTIME_SMOKE_EXECUTED", "NEXT_GATE", "REPORT_TIMESTAMP_UTC"])

    write_csv(reports / "locale_phase23t_msgmgr_message_schema_status_gate_check_v1.csv", gates, ["GATE", "STATUS", "DETAIL"])
    write_csv(reports / "locale_phase23t_msgmgr_message_schema_status_mutation_inventory_v1.csv", mutation_rows,
              ["TARGET_PATH", "ACTION", "BYTES", "SHA256"])
    write_csv(reports / "locale_phase23t_msgmgr_message_schema_status_backup_inventory_v1.csv", backup_rows,
              ["SOURCE_PATH", "BACKUP_PATH", "BYTES", "SHA256", "ACTION"])
    write_csv(reports / "locale_phase23t_msgmgr_message_schema_status_boundary_ledger_v1.csv", boundary,
              ["PROTECTED_SYSTEM", "MUTATION_ALLOWED", "OBSERVED_MUTATION", "DETAIL"])

    doc = repo / "docs/locale/LOCALE_PHASE23T_MSGMGR_MESSAGE_SCHEMA_STATUS.md"
    doc.parent.mkdir(parents=True, exist_ok=True)
    doc.write_text(f"""# Locale Phase 23T — MSGMGR Message Schema Status

Status: `{status}`

This phase updates `MSGMGR STATUS` text to report the promoted active Messaging
