...
STATUS_GREEN = "LOCALE_PHASE23T_MSGMGR_MESSAGE_SCHEMA_STATUS_BUILD_SMOKE_GREEN"
STATUS_BLOCKED = "LOCALE_PHASE23T_MSGMGR_MESSAGE_SCHEMA_STATUS_BUILD_SMOKE_BLOCKED"
NEXT_GATE = "HOLD_OR_AUTHORIZE_MSGMGR_PROVIDER_SCHEMA_STATUS_WIRING_OR_CLOSE_PHASE23"
REPORT_DIR = Path("docs/locale/reports")
RUNLOG = Path("docs/locale/runlog/LOC-023T_MSGMGR_MESSAGE_SCHEMA_STATUS_BUILD_AND_SMOKE_PROOF.md")
...
def main() -> int:
    ...
    apply_row = first_row(reports / "locale_phase23t_msgmgr_message_schema_status_apply_summary_v1.csv")
    runlog = repo / RUNLOG
    ...
    gate("PHASE23T_APPLY_STATUS_GREEN",
         apply_row.get("STATUS") == "LOCALE_PHASE23T_MSGMGR_MESSAGE_SCHEMA_STATUS_SOURCE_PATCH_APPLIED_BUILD_HELD",
    ...
    gate("LOCALE_SCHEMA_PATH_PROOF",
         ...
         "MSGMGR STATUS should still report active locale schema path")
    gate("MESSAGE_SCHEMA_ACTIVE_PATH_PROOF",
         "MESSAGING SCHEMA" in upper and "DOTTALKPP/DATA/SCHEMAS/MESSAGING/MESSAGE_CATALOG.DTSCHEMA" in upper,
         "MSGMGR STATUS should report active Messaging schema path")
    gate("MESSAGING_SCHEMA_HELD_REMOVED",
         "MESSAGING SCHEMA" in upper and "FIELD/TAG RECONCILIATION PENDING" not in upper,
         "MSGMGR STATUS should no longer report Messaging schema held")
    ...
    write_csv(reports / "locale_phase23t_msgmgr_message_schema_status_validation_summary_v1.csv", [{
        "STATUS": status,
        ...
        "MESSAGE_SCHEMA_ACTIVE_PATH_PROOF": 1 if ("MESSAGING SCHEMA" in upper and "DOTTALKPP/DATA/SCHEMAS/MESSAGING/MESSAGE_CATALOG.DTSCHEMA" in upper) else 0,
        "MESSAGING_SCHEMA_HELD_REMOVED": 1 if ("MESSAGING SCHEMA" in upper and "FIELD/TAG RECONCILIATION PENDING" not in upper) else 0,
        "NEXT_GATE": NEXT_GATE,
        "REPORT_TIMESTAMP_UTC": datetime.now(timezone.utc).replace(microsecond=0).isoformat().replace("+00:00", "Z"),
    }], ["STATUS", "VALIDATION_ISSUES", "BUILD_PROOF", "MSGMGR_STATUS_PROOF",
         "SCHEMA_ROOT_PROOF", "LOCALE_SCHEMA_PATH_PROOF", "MESSAGE_SCHEMA_ACTIVE_PATH_PROOF",
         "MESSAGING_SCHEMA_HELD_REMOVED", "NEXT_GATE", "REPORT_TIMESTAMP_UTC"])
    write_csv(reports / "locale_phase23t_msgmgr_message_schema_status_validation_gate_check_v1.csv", gates, ["GATE", "STATUS", "DETAIL"])
    write_csv(reports / "locale_phase23t_msgmgr_message_schema_status_validation_boundary_v1.csv", boundary,
    ...
    print(f"  message schema active path proof: {1 if ('MESSAGING SCHEMA' in upper and 'DOTTALKPP/DATA/SCHEMAS/MESSAGING/MESSAGE_CATALOG.DTSCHEMA' in upper) else 0}")
    print(f"  messaging schema held removed: {1 if ('MESSAGING SCHEMA' in upper and 'FIELD/TAG RECONCILIATION PENDING' not in upper) else 0}")
