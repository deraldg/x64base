...
class ReferenceCandidateTests(unittest.TestCase):
    ...
    def test_review_policy_entries_are_explicit_and_actionable(self) -> None:
        self.assertGreater(len(REVIEW_DISPOSITIONS), 0)
        allowed = {
            "DEFER_NO_RUNTIME_IDENTITY",
            "INCLUDE_PARTIAL_HELP_REFERENCE",
            "INCLUDE_WITH_RUNTIME_EVIDENCE",
            "MERGE_ALIAS_TO_CANONICAL",
            "ROUTE_DEVELOPER_DIAGNOSTIC_APPENDIX",
            "ROUTE_SOURCE_FACT_APPENDIX",
        }
        for topic_key, policy in REVIEW_DISPOSITIONS.items():
            self.assertTrue("|" in topic_key or topic_key.startswith("MINER:"))
            self.assertIn(policy["disposition"], allowed)
            self.assertTrue(policy["rationale"])
            if policy["disposition"] == "MERGE_ALIAS_TO_CANONICAL":
                self.assertTrue(policy["targets"])
    ...
    def test_structural_review_policy_is_explicit_and_targeted(self) -> None:
        self.assertGreater(len(STRUCTURAL_REVIEW_DISPOSITIONS), 0)
        for topic_key, policy in STRUCTURAL_REVIEW_DISPOSITIONS.items():
            self.assertIn("|", topic_key)
            self.assertTrue(policy[0])
            self.assertIn(policy[1], {"HIGH", "MEDIUM"})
