...
After that structure has been accepted, the command is idempotent: a balanced
reader with no unresolved review statuses produces an
`ALREADY_NORMALIZED_NOOP` candidate, an empty status ledger, and no reader-byte
drift. Historical first-acceptance counts are evidence, not permanent topology
requirements.
...
For a later vertical refresh, an already-normalized structure may bind a
zero-row `NO_CHANGES_ALREADY_ACCEPTED` decision. The plan then omits byte-
identical reader and section targets and derives command-page and lineage
counts from the bound candidate instead of requiring the first-publication
shape.

`apply-gate4-acceptance` requires that exact plan run plus a durable JSON
authorization bound to both package hashes and the plan's exact target count.
It rechecks every before and staged hash, backs up every existing target,
materializes a complete staged-after manifest, applies command pages before
any included reader/section targets and the acceptance records, and verifies
every final hash. Any partial failure triggers automatic rollback. The retained
backup also includes a guarded Python 3.12 rollback command. Apply-time
finalization changes only the three acceptance records named by the
authorization; reader pointer, HELP/META, product source, and website remain
