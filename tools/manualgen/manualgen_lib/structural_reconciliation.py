...
STRUCTURAL_REVIEW_DISPOSITIONS: dict[str, tuple[str, str, str]] = {
    ...
    "DOT|BBS": ("ai_portal_and_pseudo_chat", "HIGH", "Source and HELP define BBS as the AI-BBS / pseudo-chat board surface (boards, threads, posts) that projects the USER SYSGRANT request/approve loop; it belongs with the AI portal and pseudo-chat identity surface alongside USER."),
    "DOT|BUILDVECTORS": ("getting_started_and_session_basics", "HIGH", "Source and DOTREF define BUILDVECTORS as read-only build and session identity reporting alongside ABOUT and VERSION."),
    "DOT|DEFCMD": ("command_surface_dispatch_and_entry_variants", "HIGH", "Source and HELP define session-only command-surface creation with protected built-in boundaries."),
    "DOT|DEFFN": ("functions_and_expression_helpers", "HIGH", "Source and HELP define session-only expression-function creation on the function registry seam."),
    ...
    "DOT|NET": ("system_shell_and_files", "HIGH", "Source and HELP define NET EGRESS as a permissioned, audited host-shell capability that shells an elevated PowerShell to toggle WSL/AFB network egress; it belongs on the host-shell and external-process surface alongside BANG and PRN."),
    ...
    "DOT|STOP_ON_ERROR": ("scripting_and_control_flow", "HIGH", "Source and HELP define the script abort threshold as execution control keyed to messaging severity."),
    "DOT|UNDEFCMD": ("command_surface_dispatch_and_entry_variants", "HIGH", "Source and HELP restrict removal to session-only scratch commands created by DEFCMD."),
    "DOT|UNDEFFN": ("functions_and_expression_helpers", "HIGH", "Source and HELP restrict removal to session-only custom functions created by DEFFN."),
    "DOT|USER": ("ai_portal_and_pseudo_chat", "HIGH", "Source and HELP define the local human and AI identity, authentication, RBAC, authorization, and session-administration surface."),
    "DOT|VDISK": ("import_export_and_storage_bridges", "HIGH", "Source and HELP define the ephemeral in-memory table store and relocatable storage-path bridge."),
}
...
def build_structural_reconciliation(paths: ManualgenPaths, logger: RunLogger | None = None) -> tuple[dict[str, Any], dict[str, int]]:
    ...
    disposition_state = json.loads(disposition_manifest.read_text(encoding="utf-8")) if disposition_dir else {}
    ...
    topology_ok = (
        bool(primary)
        and bool(appendices)
        and len(controlled) == len(primary)
        and len(media_slugs) == len(primary) + 1
        and len(union_slugs) == len(primary) + 2
    )
    status = "PASS" if disposition_dir and disposition_state.get("status") == "PASS" and approved and len(approved) == len(mapping_rows) and topology_ok and len(resolved_review_rows) == len(STRUCTURAL_REVIEW_DISPOSITIONS) and not review_fallbacks and not unplaced_fallbacks and not invalid_review_targets and not missing_review_topics and not duplicate_keys and not missing_targets else "FAIL"
    ...
    proposal.extend(["", "## Gate result", "", f"- Approved topics mapped: {len(mapping_rows)}/{len(approved)}.", f"- Topics already named in an existing section command list: {exact_mentions}.", f"- Topics also named in one of three review appendices: {appendix_mentions}.", f"- Structural review dispositions: {len(resolved_review_rows)}/{len(STRUCTURAL_REVIEW_DISPOSITIONS)}.", f"- Remaining review-confidence topics: {review_fallbacks}.", f"- Remaining unplaced fallbacks: {unplaced_fallbacks}.", f"- Missing structural targets: {len(missing_targets)}.", f"- Duplicate topic keys: {duplicate_keys}.", "- Canonical manual mutation: 0.", "- Reader-pointer mutation: 0.", "- Publication authority claimed: 0.", "", "Next gate: generate per-section additive delta drafts from the accepted mapping; section replacement and publication remain separately gated."])
