...
def validate_gate4_authorization(
    ...
    expected = {
        ...
        "mutation_rows_authorized": mutation_rows,
...
def finalize_gate4_record(
    ...
    if target == PRIMARY_RECORD:
        ...
        payload["status"] = (
            f"ACCEPTED_COMMAND_REFERENCE_{payload.get('page_count')}_PAGES"
        )
    return _json_bytes(payload)


def _validate_target_set(
    rows: list[dict[str, str]], counts: dict[str, Any]
) -> list[str]:
    findings: list[str] = []
    targets = [row.get("target", "") for row in rows]
    expected_rows = counts.get("planned_mutation_rows")
    if len(rows) != expected_rows or len(set(targets)) != len(rows):
    ...
    if READER in targets:
        expected_other.add(READER)
    observed_other = {
        target
        for target in targets
        if not target.startswith(page_prefix) and not target.startswith(section_prefix)
    }
    if len(pages) != counts.get("command_pages") or any(
        not name.endswith(".md") for name in pages
    ):
        findings.append(f"COMMAND_PAGE_SET:{len(pages)}")
    if len(sections) != counts.get("section_status_files") or any(
        not name.endswith(".md") for name in sections
    ):
    ...
    if (
        creates != counts.get("planned_create_rows")
        or replaces != counts.get("planned_replace_rows")
    ):
...
def apply_gate4_acceptance(
    ...
    observed_creates = sum(row.get("operation") == "CREATE" for row in rows)
    observed_replaces = sum(row.get("operation") == "REPLACE" for row in rows)
    relational_counts = {
        "planned_mutation_rows": len(rows),
        "planned_create_rows": observed_creates,
        "planned_replace_rows": observed_replaces,
        "command_indexes": 1,
        "reader_review_statuses": 0,
        "findings": 0,
    }
    for key, expected in relational_counts.items():
        if counts.get(key) != expected:
            findings.append(f"PLAN_COUNT:{key}:{counts.get(key)}:{expected}")
    if counts.get("reader_begin_markers") != counts.get("reader_end_markers"):
        findings.append(
            "PLAN_COUNT:reader_marker_balance:"
            f"{counts.get('reader_begin_markers')}:{counts.get('reader_end_markers')}"
        )
    if counts.get("reader_command_destinations") != counts.get("command_pages"):
        findings.append(
            "PLAN_COUNT:reader_command_destinations:"
            f"{counts.get('reader_command_destinations')}:{counts.get('command_pages')}"
        )
    findings.extend(_validate_target_set(rows, counts))
    ...
    findings.extend(
        validate_gate4_authorization(
            authorization, plan_run, manifest_hash, ledger_hash, len(rows)
        )
    )
    ...
    reader_bytes = (
        after_bytes[READER]
        if READER in after_bytes
        else (paths.repo_root / READER).read_bytes()
    )
    reader_sha = _sha_bytes(reader_bytes)
    ...
    if logger:
        logger.artifact("gate4_acceptance_execution", execution_path, f"Authorized {len(rows)}-target Gate 4 apply result.")
