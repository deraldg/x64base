...
from .prose_review import COMMAND_SLUG, PARTIAL_HELP_SLUG, PROSE_REVIEW_POLICY, RUNTIME_SLUG
from .runlog import RunLogger, utc_now_iso
from .util import relpath_posix, sha256_file, write_csv, write_json


REVIEW_DECISION = "docs/maintenance/lanes/full_stack_documentation/MANUALGEN_PROSE_REVIEW_DECISION_2026-07-28.md"
...
def _latest_prose_review_dir(paths: ManualgenPaths) -> Path | None:
    ...
    for directory in root.glob("MANRUN-*") if root.exists() else []:
        ...
        if (
            ...
            and counts.get("input_topics") == len(PROSE_REVIEW_POLICY)
