...
def _line_inclusion(row: dict[str, str]) -> tuple[bool, str]:
    ...
    if "Mined from command-local usage/help output" in text or " pattern=" in text:
        return False, "EXCLUDE_SOURCE_EVIDENCE_FROM_PUBLIC_BODY"
...
def _render_page(topic: dict[str, str], label: str, selected: list[dict[str, str]], reference_run: str, disposition_run: str) -> str:
    ...
    curated_summary_rows = [
        row
        for row in summary_rows
        if row.get("SOURCE", "").upper() == "USAGE_CONTRACT"
    ]
...
def build_command_reference_candidate(paths, reference_run: str, disposition_run: str, logger=None):
    ...
    findings: list[str] = []
    attention_pages = 0
    # Page sources must be disposition-approved. Matching only approved topics keeps
    # non-approved source-mined duplicates (e.g. spaced aliases merged to a supported
    # canonical) from tying with, or outranking by catalog, the canonical that owns the
    # reader-linked page slug.
    approved_topics = [row for row in topics if row.get("TOPICKEY", "") in approved]
    for link in links:
        slug, label = str(link["slug"]), str(link["label"])
        topic, rule, match_count = _resolve_topic(label, slug, approved_topics)
        if topic is None:
