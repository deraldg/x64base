...
def build_publication_structure_candidate(paths, logger=None):
    ...
    source_begin_count = sum(1 for line in source_text.splitlines() if BEGIN_RE.match(line))
    source_end_count = sum(1 for line in source_text.splitlines() if END_RE.match(line))
    source_review_statuses = sum(
        1 for line in source_text.splitlines() if REVIEW_STATUS_RE.match(line)
    )
    source_historical_statuses = source_text.count("<!-- HISTORICAL STATUS:")
    structure_mode = (
        "ALREADY_NORMALIZED_NOOP"
        if not marker_rows and not status_rows
        else "NORMALIZATION_CANDIDATE"
    )
    ...
    if preserved_statuses != source_historical_statuses + len(status_rows):
        findings.append(
            "HISTORICAL_STATUS_COVERAGE:"
            f"{source_historical_statuses}:{len(status_rows)}:{preserved_statuses}"
        )
    if structure_mode == "ALREADY_NORMALIZED_NOOP":
        if source_begin_count != source_end_count:
            findings.append(
                f"SOURCE_MARKER_BALANCE:{source_begin_count}:{source_end_count}"
            )
        if source_review_statuses:
            findings.append(
                f"SOURCE_REVIEW_STATUSES_WITHOUT_DISPOSITION:{source_review_statuses}"
            )
        if candidate_text.encode("utf-8") != source_bytes:
            findings.append("NOOP_CANDIDATE_BYTE_DRIFT")
    ...
    review_lines = [
        ...
        "",
        f"- Status: `{status}`",
        f"- Structure mode: `{structure_mode}`",
