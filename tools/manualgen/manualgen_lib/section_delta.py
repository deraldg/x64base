...
def build_section_delta_candidates(paths: ManualgenPaths, logger: RunLogger | None = None) -> tuple[dict[str, Any], dict[str, int]]:
    ...
    status = "PASS" if mapping_rows and packet_rows and len(mapping_rows) == len(blocks) == len(used_keys) and not missing_blocks and not duplicate_blocks and not duplicate_used and not unused_blocks else "FAIL"
