...
def command_reference_totals(
    existing: dict[str, Any], reader_pages: int, reader_lineage_rows: int
) -> dict[str, int]:
    supplemental_pages = int(
        existing.get("supplemental_standalone_pages")
        or existing.get("supplemental_standalone_command_reference_pages")
        or 0
    )
    supplemental_lineage_rows = int(existing.get("supplemental_lineage_rows") or 0)
    return {
        "reader_linked_pages": reader_pages,
        "reader_linked_lineage_rows": reader_lineage_rows,
        "supplemental_pages": supplemental_pages,
        "supplemental_lineage_rows": supplemental_lineage_rows,
        "total_pages": reader_pages + supplemental_pages,
        "total_lineage_rows": reader_lineage_rows + supplemental_lineage_rows,
    }
...
def build_gate4_acceptance_plan(
    ...
    if command_manifest:
        ...
        command_counts = command_manifest.get("counts", {})
        candidate_pages = command_counts.get("candidate_pages")
        reader_link_destinations = command_counts.get("reader_link_destinations")
        if not isinstance(candidate_pages, int) or candidate_pages <= 0:
            findings.append(f"COMMAND_COUNT:candidate_pages:{candidate_pages}")
        if reader_link_destinations != candidate_pages:
            findings.append(
                "COMMAND_COUNT:reader_link_destinations:"
                f"{reader_link_destinations}:{candidate_pages}"
            )
        if not isinstance(command_counts.get("lineage_rows"), int) or command_counts.get("lineage_rows", 0) <= 0:
            findings.append(f"COMMAND_COUNT:lineage_rows:{command_counts.get('lineage_rows')}")
        for key in ("findings", "local_path_hits"):
            if command_counts.get(key) != 0:
                findings.append(f"COMMAND_COUNT:{key}:{command_counts.get(key)}")
        for key in (
            "candidate_only",
    ...
    if structure_manifest:
        ...
        structure_counts = structure_manifest.get("counts", {})
        if structure_counts.get("candidate_begin_markers") != structure_counts.get("candidate_end_markers"):
            findings.append(
                "STRUCTURE_COUNT:marker_balance:"
                f"{structure_counts.get('candidate_begin_markers')}:"
                f"{structure_counts.get('candidate_end_markers')}"
            )
        if structure_counts.get("remaining_review_statuses") != 0:
            findings.append(
                "STRUCTURE_COUNT:remaining_review_statuses:"
                f"{structure_counts.get('remaining_review_statuses')}"
            )
        if structure_counts.get("headings_before") != structure_counts.get("headings_after"):
            findings.append(
                "STRUCTURE_COUNT:heading_drift:"
                f"{structure_counts.get('headings_before')}:"
                f"{structure_counts.get('headings_after')}"
            )
        if structure_counts.get("findings") != 0:
            findings.append(f"STRUCTURE_COUNT:findings:{structure_counts.get('findings')}")
    ...
    if not status_ledger_path.is_file():
        ...
        status_rows = [
            row for row in status_rows if row.get("status", "").upper() != "EMPTY"
        ]
    expected_status_rows = structure_manifest.get("counts", {}).get(
        "status_dispositions", -1
    )
    if len(status_rows) != expected_status_rows:
        findings.append(f"STATUS_ROW_COUNT:{len(status_rows)}:{expected_status_rows}")
    ...
    if approval:
        ...
        expected_decision = (
            "NO_CHANGES_ALREADY_ACCEPTED"
            if expected_status_rows == 0
            else "APPROVE_ALL_ROWS"
        )
        if approval.get("decision") != expected_decision:
            findings.append(
                f"STATUS_APPROVAL_DECISION:{approval.get('decision')}:{expected_decision}"
            )
        if approval.get("approved_rows") != expected_status_rows or approval.get("held_rows") != 0:
    ...
    expected_command_rows = command_manifest.get("counts", {}).get(
        "candidate_pages", -1
    )
    if len(command_rows) != expected_command_rows:
        findings.append(f"COMMAND_ROW_COUNT:{len(command_rows)}:{expected_command_rows}")
    ...
    if structure_candidate_path.is_file():
        ...
        if reader_mutation["before_sha256"] != reader_mutation["staged_sha256"]:
            mutation_rows.append(reader_mutation)
    else:
        planned_reader = ""
        link_rewrites = 0
    ...
    existing_command_record = _load_json(
        paths.repo_root / command_record_rel, findings, "COMMAND_REFERENCE_RECORD"
    )
    reference_totals = command_reference_totals(
        existing_command_record,
        expected_command_rows,
        command_manifest.get("counts", {}).get("lineage_rows", 0),
    )
        ...
        primary_record.update(
            {
                ...
                "command_reference_pages": reference_totals["total_pages"],
                "reader_linked_command_reference_pages": reference_totals["reader_linked_pages"],
                "supplemental_standalone_command_reference_pages": reference_totals["supplemental_pages"],
                "section_statuses_reviewed": structure_manifest.get("counts", {}).get("historical_status_comments", 0),
                "section_marker_balance": {
                    "begin": len(BEGIN_RE.findall(planned_reader)),
                    "end": len(END_RE.findall(planned_reader)),
                },
                "structure_mode": structure_manifest.get("structure_mode", ""),
        ...
        canonical_record.update(
            {
                ...
                "command_reference_pages_projected": reference_totals["total_pages"],
                "reader_linked_command_reference_pages": reference_totals["reader_linked_pages"],
                "supplemental_standalone_command_reference_pages": reference_totals["supplemental_pages"],
                "section_statuses_reviewed": structure_manifest.get("counts", {}).get("historical_status_comments", 0),
                "section_markers_balanced": len(BEGIN_RE.findall(planned_reader)),
                "structure_mode": structure_manifest.get("structure_mode", ""),
    ...
    command_record = dict(existing_command_record)
    command_record.update({
        ...
        "page_count": reference_totals["total_pages"],
        "reader_linked_pages": reference_totals["reader_linked_pages"],
        "reader_linked_lineage_rows": reference_totals["reader_linked_lineage_rows"],
        "supplemental_standalone_pages": reference_totals["supplemental_pages"],
        "supplemental_lineage_rows": reference_totals["supplemental_lineage_rows"],
        "attention_labelled_pages": command_manifest.get("counts", {}).get("attention_labelled_pages", 0),
        "lineage_rows": reference_totals["total_lineage_rows"],
    ...
    if len(BEGIN_RE.findall(planned_reader)) != len(END_RE.findall(planned_reader)):
    ...
    expected_historical_statuses = structure_manifest.get("counts", {}).get(
        "historical_status_comments", -1
    )
    if planned_reader.count("<!-- HISTORICAL STATUS:") != expected_historical_statuses:
        findings.append(
            "READER_HISTORICAL_STATUS_COUNT:"
            f"{planned_reader.count('<!-- HISTORICAL STATUS:')}:"
            f"{expected_historical_statuses}"
        )
    ...
    report_lines = [
        ...
        "## Approval binding",
        "",
        f"The `{expected_status_rows}`-row structure decision is bound to status ledger `{expected_status_sha}` and approval record `{approval_rel}`.",
        "",
        ...
        "",
        f"The accepted reader's complete {len(reader_destinations)}-destination set resolves in the staged publication. `{len(section_link_gap_rows)}` pre-existing links found only in richer standalone section sources are recorded in `standalone_section_link_gap_ledger.csv`; Gate 4 does not introduce them or invent unsupported pages for them.",
        "",
        "## Apply boundary",
        "",
