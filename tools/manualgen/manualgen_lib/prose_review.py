...
PROSE_REVIEW_POLICY: dict[str, dict[str, str]] = {
    ...
    "DOT|DEFCMD": {
        ...
        "review_disposition": "ADDITIVE_PROSE",
        "anchor_after": "## Command surface",
        "candidate_action": "ADD_SESSION_DEFINITION_NOTE",
        "rationale": "Document session-only command-surface creation with protected built-in boundaries without implying a persistent public command.",
    },
    "DOT|UNDEFCMD": {
        "target_slug": COMMAND_SLUG,
        "review_disposition": "ADDITIVE_PROSE",
        "anchor_after": "## Command surface",
        "candidate_action": "ADD_SESSION_DEFINITION_NOTE",
        "rationale": "Document removal restricted to session-only scratch commands created by DEFCMD without implying built-in removal.",
    },
    "UI|RECORDVIEW": {
        "target_slug": COMMAND_SLUG,
        "review_disposition": "ADDITIVE_PROSE",
        "anchor_after": "## Aliases and entry variants",
        "candidate_action": "ADD_CONDITIONAL_UI_ENTRY_NOTE",
        "rationale": "Describe the record-view UI entry point with a build-availability caveat rather than presenting it as an always-present command.",
    },
    "UI|RECORD": {
        "target_slug": COMMAND_SLUG,
        "review_disposition": "ADDITIVE_PROSE",
...
def _latest_section_delta_dir(paths: ManualgenPaths) -> Path | None:
    ...
    for directory in root.glob("MANRUN-*") if root.exists() else []:
        ...
        if (
            ...
            and counts.get("approved_topics", 0) > 0
            and counts.get("approved_topics") == counts.get("mapped_topics")
            and counts.get("mapped_topics") == counts.get("used_topic_blocks")
            and counts.get("target_packets", 0) > 0
        ):
...
def build_prose_review_batch(paths: ManualgenPaths, logger: RunLogger | None = None) -> tuple[dict[str, Any], dict[str, int]]:
    ...
    status = "PASS" if (
        len(PROSE_REVIEW_POLICY) == 16
        and len(all_input_keys) == len(PROSE_REVIEW_POLICY)
