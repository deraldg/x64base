# DotTalk++ Reader Manual Draft v1

Status: reader-facing draft  
Audience: new DotTalk++ user, human developer, AI development agent  
Source root: `D:\code\ccode`  
Generated from curated harvest material on 2026-06-28
...
Current classification for this draft:
...
1. Ran the canonical x64 walkthrough through `DOTSCRIPT ... OUT ...`.
2. Ran a dedicated CDX `SET INDEX` plus `SET ORDER TO TAG <tag>` proof.
3. Added transcript paths beside the x64 and order examples.
4. Filled navigation/search from current HELP and runtime proof.
5. Filled CSV import/export after checking current command behavior.
6. Classified xBase/FoxPro/VFP compatibility claims as proven, source-supported, lineage-only, or experimental.
7. Added and proved schema-driven `DDL CREATE DBF ... FROM schema.json`.
8. Proved CNX order for v32/schema-created tables.
9. Proved non-interactive `BROWSE` listing forms.
10. Retired `DO X64` from the reader quickstart while preserving it as legacy/profile script vocabulary.
11. Moved manualgen/MAN*/drift/process language into the developer appendix.
12. Added manual generation anchors that join HELP, META, SelfDoc, cmdhelpchk, manualgen, runtime proofs, and data dictionary targets.
13. Added a prose guide that explains how anchors become orientation, tutorial, reference, concept, caveat, handoff, and data dictionary prose.
14. Added professional manual blueprint and first advanced database prose for cursor control, locking, table buffering, commit, and rollback.
15. Added maturity levels, maturity targets, and operational safety checklist.
16. Added a direct source usage-contract command reference harvest and readable command guide.

Remaining before promotion to `DOTTALKPP_READER_MANUAL_V1.md`:

1. Generate the command/function reference skeleton from HELP plus META.
2. Run a final wording polish and remove draft-only phrasing.
3. Decide whether the promoted manual should include interactive `BROWSER`/`BROWSETUI`; this draft only promotes non-interactive `BROWSE`.
