...
inline bool parse_integer(std::string_view in, long long &out) {
    ...
    // Require full-string consumption. std::from_chars accepts a valid prefix,
    // so without checking result.ptr, values like "123ABC" would parse as 123.
    std::string cleaned;
    ...
    return result.ec == std::errc() && result.ptr == last && ptr != last;
}
