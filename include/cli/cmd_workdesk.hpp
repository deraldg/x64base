...
// subsystem: cli
// layer: header
// owns: the WORKDESK verb and the desk it reports -- one session's open
//       workspaces as one value
// project: project.x64base.runtime
// lane: 
// owner: member.derald
// status: experimental

// File: include/cli/cmd_workdesk.hpp
...
// Render the desk as the WORKSPACES section of a report.
//
// SEPARATE FROM observe() ON PURPOSE, and the split is the point of the file.
// observe() answers WHAT IS TRUE; render() answers HOW IT READS. A caller that
// wants the facts takes the Desk and never calls this; a caller that wants the
// section calls both and adds nothing of its own. What must not happen again is
// a third caller doing its own walk because rendering and observing were welded
// together and it only wanted one of them -- which is how this file came to
// exist.
//
// Writes only to the stream it is handed. It does not reach for std::cout, and
// it does not know whether that stream is a console, an alternate capture, or a
// string. Deciding that is the command's job.
