...
This page separates public clone/build examples from Derald's local development paths. Local evidence pages may still cite `implementation checkout` because that is the active development tree, but a new reader should be able to start from a normal checkout.
...
The staging/publishing workflow currently mirrors active development into `local staging mirror`, but outside contributors should treat the GitHub checkout as the portable starting point.
