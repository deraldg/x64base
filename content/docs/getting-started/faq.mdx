...
> **FAQ reviewed 2026-09-02.** Each full-stack documentation reconciliation must
...
> authority.
>
> **Disposition: `UPDATED`.** No engine source changed between 2026-08-26 and this
> review, so the product, format, workspace and interface answers were re-read and
> left standing. Three things did change, and two were defects rather than
> staleness. **A contradiction was corrected:** the status answer cited the
> application UI DSL as the example of a planned lane, while the UIDEF (UI DEFINITION -- a screen layout stored as a DBF design table) answer 150
> lines above already described it as an Alpha implementation lane -- two answers
> in one FAQ disagreeing. **A gap was filled:** the FAQ mentioned SQL zero times
> across 44 questions, while the product ships `SQLSEL`, `SQL`, `SQLHELP`,
> `SQLITE`, `SQLVER` and `SQLERASE` and the site carries three SQL pages; a reader
> asking whether this does SQL got no answer, and the several commands sharing the
> name are easy to confuse. **A caveat was added:** what `supported` means on a
> command page, because 33 commands publish it while their own contract says
> something narrower.
...
subqueries and single-table transactional DML -- each verified against an
...
answer called it absent -- while cross-table atomic commit, stored NULL in DML
and subqueries over a joined outer scope remain absent. A command's page may also show a catalog status
