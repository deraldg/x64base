...
in-process SQLite oracle -- while the bare `SELECT` alias without the `SQLSEL`
prefix, cross-table atomic commit, stored NULL in DML and subqueries over a
joined outer scope remain absent. A command's page may also show a catalog status
