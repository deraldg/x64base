...
- **RelTalk** names relation-facing workflows such as relation trees, descendant
  summaries, and relation-aware browse output -- and it is where joining lives:
  `REL JOIN` walks a parent and its children and emits one tuple per
  combination, which is a different output from the positioned projection above.
...
developed, and that is not the same as a claim of completeness. Joins and
`GROUP BY` are not implemented in the SELECT slice, and a command's page may show
a catalog status broader than its own contract claims (see below).
