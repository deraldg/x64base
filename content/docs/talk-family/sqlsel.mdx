...
- **Explicit transactions require SQL mode.** Corrected 2026-09-12: this bullet
  also said they accept ONE target table and that cross-table atomic commit was
  refused. That stopped being true on 2026-09-11. A transaction now enlists
  every table it touches, each member prepares durably, and ONE group-log row
  decides all of them, so a crash leaves ALL members applied or NONE. A
  multi-table commit reports
