...
| Identity allocation | **Runtime-proven.** `max(id)+1` taken under the writer's own lock -- `bbs_store::next_id` for `SYSTHREAD` / `SYSPOST`, and `WORKSPACE NEW` for `WS_ID` under a `FLOCK`. Ids are never reused, the scan counts deleted rows on purpose, and it self-heals after a hand-edited row |
| Header sequence slot | `autoq_next` (64-bit) is present in the x64 header and hydrated at open, and is **reserved and deliberately unwired** -- steward ruling R119, 2026-08-24. Not a pending lane: the question it would answer is already answered by the allocator above, and making the header authoritative as well would give one question two ladders. `autoQNext64()` has zero callers by design, and the ruling ships the three-part recipe for wiring it should that change |
| No `PACK` on the workspace catalog | A consequence of the row above, stated where it is decided: `max(WS_ID)+1` over a scan that counts deleted rows is the only thing preventing a reissued identity. Packing would renumber silently, so the verb does not exist |
...
Classic 32-bit support is **deliberately limited in scope**, and the scope is
worth stating precisely so nobody plans around a promise that was never made.
What is supported is **tables, memos, and indexes** -- and specifically the
**house memos and house indexes we provide for v32**:
...
compatibility platform, does not aim to reproduce a legacy application
runtime, and does not chase every historical index or memo variant produced
by three decades of vendors. The x32 lane exists because teaching a database
engine honestly means showing the classic generation beside the new one -- not
because this is a migration target.

**If you need broader classic coverage, other platforms exist and are better
suited.** Harbour, xHarbour, Alaska Xbase++, XSharp, and the commercial
dBASE/DBF tool families all carry far deeper legacy compatibility, and the
[ecosystem comparison](/docs/engine/ecosystem-feature-comparison) names them
with links. Choosing one of those over this project for legacy work is the
correct decision, and saying so costs this project nothing.
...
| SELECT | `SQLSEL`: WHERE, ORDER BY, LIMIT, DISTINCT, COUNT(*); INNER / LEFT / RIGHT / FULL / CROSS join; aggregates with GROUP BY and HAVING; UNION / INTERSECT / EXCEPT; IN and EXISTS subqueries; expression projection; INSERT / UPDATE / DELETE with single-table transactions. Every result set verified against an in-process SQLite oracle over identical data in the same run. 13 specs are graded against that oracle: 6 in the default suite, 7 explicit-run. The bare `SELECT` alias ships behind `SET MODE SQL` (corrected 2026-09-09; this row called it absent). Still absent, stated plainly: cross-table atomic commit, stored NULL and memo-field DML, and subqueries over a joined outer scope |
...
| Item | Value |
| --- | --- |
| Snapshot formats | `DTSHEMA 2` (the canonical schema -- portable, diffable, no absolute paths) and `DTSHEMA 3` (the session -- measured flavor, own roots, cursor + selection state). Both are now spellable as `V2` / `V3`; v2 remains the default. `MINIDB 1` is the container -- posture plus table and index bytes -- and implies V3, so `MINIDB V2` is refused rather than silently upgraded |
...
| Simultaneity | several workspaces resident at once since 2026-08-26; `OPEN` and `LOAD` are both additive (R128, R130). Containment is declared, never inferred (R137): a workspace records its parent's durable id and its depth at birth. Open: cross-workspace name resolution, per-workspace RAM residency |
...
| Item | Value |
| --- | --- |
| Curated regressions | 64 registered specs, 21 in the default suite (measured 2026-09-02); the registry stores each spec's doctrine, and the public list is generated from that same table. A spec joins the default suite on two green runs plus a measured coverage hole, not on judgement |
