...
A superscript on an x64base cell points to a numbered note directly beneath that
table. The cell carries the verdict; the note carries the evidence, the scope,
and whatever the verdict does *not* cover. Nothing was shortened by dropping a
caveat -- the caveats moved, they did not go.
...
| 64-bit indexing architecture | Partial: CDX/LMDB carry 64-bit recnos; CNX and legacy `.inx` stay 32-bit by design ¹ | Classic RDD/index ecosystem | Classic index ecosystem | DBF/NTX/CDX engines | Runtime dependent | Classic indexes/tools | Usually no persistent index support |
| 64-bit record navigation and locking | Yes, in the public build ² | Classic xBase record model | Classic xBase record model | Runtime/DBE dependent | Runtime dependent | Classic | Library/dialect dependent |
...
| Vectored long table/field names with fallback mangling | Yes, up to 256-byte names with classic 10-byte fallback tokens ³ | Not same claim | Not same claim | Product dependent | .NET/runtime dependent | Product dependent | No |
| Custom field types | Yes — register-once codec registry ⁴ | Not same claim | Not same claim | Product/runtime dependent | .NET type system | Product dependent | Conversion hooks only |

¹ **64-bit indexing architecture.** Partial: the active CDX/LMDB path carries 64-bit record numbers (`RecNo = uint64_t`, 8-byte little-endian storage; index-hook and keyed-seek decoders widened, public build 2026-07); classic CNX and legacy `.inx` are kept 32-bit by design and report their ceiling. Boundary proof past `2^31` is green (unit + sparse-file end-to-end tests)

² **64-bit record navigation and locking.** Yes, in the public build: `GO`/`GOTO`/`SKIP`/`RECNO`/`TOP`/`BOTTOM`/`FIRST`/`LAST`/`NEXT`/`PRIOR`, the table-buffer change key, and the record-lock API were widened to 64-bit (CURSOR and INDEX_X64 regression-checked), and the classic `recno()`/`recLength()`/`recCount()`/`cpr()` accessors now return `-1` past their 32-bit range instead of saturating. Addressing past `2^31` is proven end-to-end — the engine reads distinct records at recno `2^31+1`/`+2` off a sparse x64 table

³ **Vectored long table/field names with fallback mangling.** Yes, x64 metadata supports up to 256-byte table/field names (ceiling raised from 128 in 2026-07; default 128) with classic 10-byte fallback descriptor tokens

⁴ **Custom field types.** Yes — register-once codec registry (type code + codec + on-disk width + flavor eligibility); the built-in `I`/`B`/`Y`/`T` binary types use the same path, and a worked custom type ships as a demo
...
| Workspace stored *inside* a database table | Yes: `WORKSPACE SAVE <name> MEMO` ⁵ | Not same claim | Not same claim | Not same claim | Not same claim | Not same claim | No |
...
| Multiple workspaces simultaneously resident | Yes, since 2026-08-26 — `OPEN` and `LOAD` are both additive ⁶ | Not same claim | Not same claim | Product/project dependent | Runtime dependent | Product dependent | No |
...
| Full SQL server/RDBMS | **No** — and the missing half is the integrity model, not the query surface ⁸ | No, but can bridge | No, but can bridge | Can bridge to server DBs | Uses .NET DBs | Product dependent | No |
| **Primary key enforced on write** | **Yes**, since 2026-09-07 — real schema rather than a convention ⁹ | App/RDD dependent | App/RDD dependent | Product/database dependent | Database dependent | Product dependent | No native |
| Referential integrity enforced on write | **No**, and this is the sharp one ¹⁰ | App/RDD dependent | App/RDD dependent | Product/database dependent | Database dependent | Product dependent | No native |
| Database triggers | **No** — a reserved design slot, not a feature ¹¹ | App/RDD/SQL dependent | App/RDD/SQL dependent | Product/database dependent | Database dependent | Product dependent | No native |
| **Domain integrity enforced on every write** | **Yes**, and more strictly than the lineage ¹² | App/RDD dependent | App/RDD dependent | Product/database dependent | Database dependent | Product dependent | Library dependent; many write what they are handed |
| Declared constraints (`CHECK`-style) | **Partial**, opt-in rather than automatic ¹³ | App/RDD dependent | App/RDD dependent | Yes/product dependent | Database dependent | Product dependent | Python code only |
...
⁵ **Workspace stored *inside* a database table.** Yes: `WORKSPACE SAVE <name> MEMO` writes the snapshot into a memo field of a self-creating catalog, attributed, versioned by supersede, byte-verified on write

⁶ **Multiple workspaces simultaneously resident.** Yes, since 2026-08-26: `OPEN` and `LOAD` are both ADDITIVE (R128, R130) -- a load closes nothing and a posture's area numbers are treated as keys, not addresses. Relation scope, work-area budget and environment ownership are closed; cross-workspace name resolution (`CREATE`, `SELECT`) and per-workspace RAM residency remain open

⁷ **SQL `SELECT` over native DBF tables.** **Yes**: `SQLSEL` runs the relational algebra over open work areas -- selection, projection, `ORDER BY`, `LIMIT`, `COUNT(*)`, the full join family (`INNER` / `LEFT` / `RIGHT` / `FULL` / `CROSS`), `DISTINCT`, the set operations, `GROUP BY` / `HAVING` with aggregates, and subqueries. Every shipped operator is verified against an in-process SQLite oracle, and the joins additionally assert their reported access path separately from their answer

⁸ **Full SQL server/RDBMS.** **No, and the missing half is not the one people expect.** The *query* surface is relational algebra, oracle-verified. What is not relational is the *integrity model*, and as of 2026-09-07 that is one gap rather than two. **A declared primary key is now enforced on write** -- `SET UNIQUE FIELD <field> PRIMARY` designates the column, the engine mints `max+1` into a blank key on `APPEND` under the writer's table lock, and a write that would change a minted key is *refused* rather than silently accepted. The designation is stamped into the x64 field metadata (`X64_FIELD_FLAG_PRIMARY`), so it **survives a restart** -- a fresh process reopens the table and still refuses. Read the scope honestly: enforcement is proven on the five write paths the `PKPOLICY` regression asserts -- native `REPLACE`, `SQLSEL INSERT`, `SQLSEL UPDATE`, and the two legacy bare `INSERT`/`UPDATE` verbs, which are different files reached by a different registration. It is *not* yet universal: a static gate counts 13 direct field-write call sites in 11 files outside the funnel, and `CALCWRITE`, browse/record-view editing, `COPY`, `SORT` and `IMPORTSQL` are unasserted. Like stored `NULL`, it is also flavor-scoped -- the flag lives in x64 metadata, and a reader that does not know it reads zero and enforces nothing, in silence. **Referential integrity remains genuinely absent** and is the row below. Stored `NULL` is no longer on that list -- VFP-flavor tables carry a real `_NullFlags` bit, asserted every run by the `NULLASSERT` regression -- but it is flavor-scoped rather than universal, and SQLsel's own DML refuses to write one. There is also no server -- SQLsel runs in-process over open work areas. Physical transparency is refused **by design**, not by omission: the glass-box thesis exposes headers, geometry and index containers that an RDBMS would hide

⁹ **Primary key enforced on write.** **Yes, since 2026-09-07, and it is real schema rather than a convention.** `SET UNIQUE FIELD <field> PRIMARY` designates the column. The engine **mints** `max+1` into a blank key on `APPEND` inside the writer's table lock, **reserves** a deleted row's key until `PACK` because `RECALL` can bring that row back, and **refuses** a write that would change a minted key. The designation is stamped into the x64 field metadata (`X64_FIELD_FLAG_PRIMARY`), so it survives a restart: a cold process reopens the table and still refuses, which `PKDURABLE` proves across two processes because no single script can. Scope stated rather than implied: enforcement is proven on the five write paths `PKPOLICY` asserts, a static gate counts 13 direct field-write sites in 11 files outside the funnel, and the flag is x64-flavour-scoped because a classic or VFP descriptor has nowhere to carry it

¹⁰ **Referential integrity enforced on write.** **No, and this is the sharp one.** Relations are declared for *traversal* -- `SET RELATION`, `REL ADD ... ON` -- and drive navigation and joins. Nothing constrains a write: no orphan is refused, no delete cascades or restricts. A declared relation here is a path, not a constraint

¹¹ **Database triggers.** **No.** Corrected 2026-09-03: this row read "Yes" and it was wrong. `src/cli/cmd_trigger.cpp` is a reserved design slot -- *"Declares the TRIGGER name; no handler exists yet"*, proof-state `idea`, deliberately not harvested into `SYSCMD`/HELP. `RULE` and `VALIDATE` are real but are not triggers. Below the language there IS an engine-level hook -- a per-work-area C++ callback that fires after a successful unbuffered field replace, and as of AIF-087 M1 a second registration slot that can refuse a write -- but no DotTalk statement can register one, and the refusal slot has no caller yet. Nothing fires at all on the buffered SQL write path. Chartered and partly built; not usable

¹² **Domain integrity enforced on every write.** **Yes, and more strictly than the lineage.** `REPLACE` validates each value against the field's declared **type, length and decimals** and either canonicalizes it into the domain or **refuses the write** -- `D` rejects an invalid date, `N`/`F` an out-of-domain numeric, `I` an int32 out of range, `L`, `Y` and `T` likewise. `REPLACE DOB WITH 02/14/1956` stores `19560214`, *not* the slashed text truncated to the field width. Silent truncation to width is the classic xBase failure mode; this refuses instead

¹³ **Declared constraints (`CHECK`-style).** **Partial, and opt-in rather than automatic.** `RULE` carries a real constraint model, and `VALIDATE` checks it -- but the write path does not consult it, so a rule is enforced when you run it, not when you write. That is a separate layer from the domain integrity above, which *is* automatic
