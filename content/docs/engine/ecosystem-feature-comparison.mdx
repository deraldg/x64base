...
| Server process / wire protocol | **No** — `SQLSEL` runs in-process; no daemon, no port ⁸ | No, but can bridge | No, but can bridge | Can bridge to server DBs | Uses .NET DBs | Product dependent | No |
...
⁸ **Server process / wire protocol.** **No, and it is the narrow question it looks like.** `SQLSEL` runs in-process over open work areas: no daemon, no port, no wire protocol, no second machine.

    This row was called *Full SQL server/RDBMS* until 2026-09-09, and it answered **four questions at once** -- which is why one flat refusal sat over a system whose actual profile is mixed. Those questions have their own rows now and they disagree with each other, which is the point. The query surface is **Yes** (relational algebra, every operator oracle-verified, three rows above). A declared primary key on write is **Yes** since 2026-09-07. Domain integrity on every write is **Yes**, and stricter than the lineage. Declared constraints are **Partial**. Referential integrity and triggers are **No**. Read the seven rows, not the one.

    **And one of the absences is refused rather than missing.** Physical opacity -- an engine that hides its headers, geometry and index containers -- is something an RDBMS provides and this project deliberately does not. The glass-box thesis is *why* the files stay inspectable, so in that respect "not an RDBMS" is a design statement and not a gap.

    Stored `NULL` is no longer among the gaps either: VFP-flavour tables carry a real `_NullFlags` bit, asserted every run by the `NULLASSERT` regression -- though it is flavour-scoped rather than universal, and SQLsel's own DML refuses to write one. **That capability has no row of its own on this page and is described only here**, which is a gap in the table rather than in the engine.
