...
|---|---|
| `JOIN` **as SQL syntax inside a `SQLSEL` statement** | Planned -- `sqlsel_statement.cpp` says "v1 reads a single table; joins arrive with the join phase". This is a limit of the SQL surface, **not of the engine**: three join forms ship and are proven, listed below |
...
| Today | Supported -- **three join forms ship**: `SET RELATION` positional navigation, `REL JOIN` / `REL ENUM` enumeration (one tuple per combination, with `DISTINCT` / `ALL` / `LIMIT`), and `REL JOIN ONE` from the current pointers | `SELECT` supported; joining from `SELECT` syntax planned |
...
- `JOIN` - **equivalent, different surface.** Not available as SQL syntax inside
  `SQLSEL`; available three ways through RelTalk (see the shipped forms above).
  The engine's own reference is careful about which is meant, and so is this
  line: what is missing is reaching a join *from a `SELECT`*, not joining
