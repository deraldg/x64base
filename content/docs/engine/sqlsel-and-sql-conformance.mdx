...
| Surface | Status |
|---|---|
| `JOIN` **as SQL syntax inside a `SQLSEL` statement** | **Ships.** `INNER`, `LEFT`, `RIGHT`, `FULL` and `CROSS`, with `DISTINCT`, the set operations, `GROUP BY` / `HAVING` and aggregates, subqueries, and workspace-scoped name resolution -- pushed to `development` as `e7d6ef130`. Result sets are compared against an in-process SQLite oracle by executable validators, which also assert the reported access path **separately from the answer** -- **13 `SQLSEL_*` specifications, 7 of them in the default suite** (buffer visibility, `SELECT` v1, inner join, join edges, left join, the full family, and the `SET PARALLEL` differential) and 6 explicit-run -- corrected 2026-09-09 from "fourteen ... the four", which was wrong in both halves, grown to thirteen on 2026-09-22 when `SQLSEL_PARALLEL` registered the `SET PARALLEL` differential gate, and promoted to the default suite the same day after a green-red-green mutation proof and a two-run soak on one build. Counting specs graded against the oracle rather than specs named `SQLSEL_*` also gives **13 oracle-graded specs**, but they are not the same thirteen: `SQLMODE_SMOKE` is oracle-graded without the `SQLSEL_` name, and `SQLSEL_PARALLEL` carries the name without the oracle -- its serial leg is its own oracle. Outer-join `WHERE` is refused, naming the reason: three-valued predicate support does not exist yet. The frontier moves faster than this page; [the SQLsel manual](https://github.com/deraldg/x64base/blob/development/docs/manuals/user/sqlsel.md) is the authority |
...
| `INSERT` / `UPDATE` / `DELETE` as SQL syntax | **Ships, transactional, single- or multi-table** -- `SQLSEL INSERT INTO ... VALUES`, `SQLSEL UPDATE ... SET ... WHERE`, `SQLSEL DELETE FROM ... WHERE`, built on the engine's own `APPEND` / `REPLACE` / `DELETE`, cursor, lock, TableBuffer and WAL facilities. **Cross-table atomic commit ships since 2026-09-11**: each enlisted table prepares durably and ONE group-log row decides all of them. **No memo-field DML.** `COMMIT ALL` is deliberately unchanged and still partial across DBF, memo and index storage; `GROUPCOMMIT` is its atomic twin |
...
- `INSERT` - **supported as SQL syntax** (`SQLSEL INSERT INTO ... VALUES`), transactional and enlistable in a multi-table transaction; the two-step `APPEND` then `REPLACE` form remains the native path
...
**Derived from engine commit `2e0f636ee` (2026-09-23).**
