...
> Concurrent and nested multi-workspace management is **Alpha** -- proven, and
> still moving. Several workspaces are resident at once (R128, R130, additive
> `OPEN` and `LOAD`), containment is declared with depth and cycle rules (R137),
> and seven default-suite regressions cover the lane. What Alpha means here is
> that the surface is still changing, not that the behaviour is unproven. See
...
> inspection and RAM hydration are runtime-proven in development. So are named
> concurrent workspaces and nesting: ownership, depth, cycle and close-order all
> have answers, and a 2026-08-21 descent opened a depth-2 container into 13
> tables and 11 index containers. What remains directional is **deeper
> recursion** -- a container whose payload holds another container is chartered
> and untried, measured at at-rest depth of exactly one.
...
inside a memo today; several hydrated instances can already be named, nested and
navigated at once, and the open Alpha question is **isolation** -- cross-workspace
name resolution, and a per-workspace RAM subroot.
