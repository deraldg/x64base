...
| V64 VFP/x64 | `<table>.cdx` | `.cdx` or `.cnx` | CDX is the command-facing container. Its physical backend depends on **residence**: LMDB on disk, native CDX-V64 in RAM. `.cnx` is accepted on v64 and auto-resolves only when a `.cnx` exists and no `.cdx` does; the default is untouched. |
...
`INX` and `IDX` are different things and are not interchangeable. `INX` is a single-tag on-disk container, listed above for V32. `IDX` is a memory-only educational index lab and is not a storage format for real work.
