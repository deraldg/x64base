...
That means SQLite, LMDB, and libsodium are dynamically loaded support libraries.
Their code is not statically folded into the DotTalk++ executable.
...
> The PE-section and largest-object breakdowns below are from the July 7 baseline
> build (`6,675,456` bytes) and predate the growth to `7,707,648` bytes (identity /
> AI-BBS, expression functions, and related code); a future re-measure will
> refresh them.
...
- Do not blame the size on SQLite, LMDB, or libsodium; they are dynamically imported.
