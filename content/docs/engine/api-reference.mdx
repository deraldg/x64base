...
This is not a frozen Python ABI and it is not the DotTalk++ command shell.
Workspace, relation, complete index/order semantics, memo mutation, and general
command execution must not be assumed unless their Python contracts and proof
are added.
...
pycrud](/docs/engine/python-integration) for the current boundary and possible
development path.
...
- native Python binding through the bounded `pydottalk` surface.
- local API and GUI experiments through `pydottalk_api` and pycrud.
