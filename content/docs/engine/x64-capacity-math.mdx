...
The record-number widening lane (internal name RECNO64) removed the 32-bit choke
points listed above. The table records the current state in the public build;
prior states are preserved in this site's version history.
...
represent larger, and these numbers are deliberately round.

**The configuration layer this page used to call a future direction now exists.**
AIF-044 (the BUILD_VECTORS lane) moved them out of hand-maintained constants:
`config/build_vectors.cmake` declares each as a CMake `CACHE STRING`, feeds it
through `configure_file()` into `generated/dottalk/build_vectors.hpp`, and
`include/xbase.hpp` now *derives* `MAX_AREA`, `MAX_FIELDS`, `MAX_INDEX` and the
record-size ceilings from it rather than declaring them. Configuration validates
itself — a `MAX_AREAS` below 1, or a name default above its own maximum, fails
`cmake` rather than the build.

So the honest statement about every number below is: **it is a selected build
input with a named authority, not a limit the format imposes.** What remains
future is the *runtime* half — setting a ceiling without a rebuild, via profile
parameters or an `xbase.meta` descriptor.

One number is worth stating plainly because it is the one that answers "is this
really 64-bit": `DOTTALK_MAX_ROWS` is **9,223,372,036,854,775,807** — `INT64_MAX`,
the engine's row-count policy. It is not a round compatibility number, and it is
not narrowed anywhere in the vector file.

The values below are the current defaults in the **public build** (2026-07);
earlier builds carried the prior values, and any of them can be set on the
`cmake` line.

| Capacity item | Current x64base evidence | Documentation status |
| --- | --- | --- |
| Work areas / open table slots | `DOTTALK_MAX_AREAS = 512` in `config/build_vectors.cmake`, reaching the engine as `xbase::MAX_AREA`. **No architectural or format bound sits behind it.** The cost that sets the default is measured rather than assumed: areas are allocated eagerly as `std::array<std::unique_ptr<DbArea>, MAX_AREA>`, at roughly 1.3 KB per slot. | Public build default; selectable at configure time; raised from 256 (2026-07). |
...
This is the exact reason the project says **x64base implements an x64 DBF-style
architecture with 64-bit record addressing proven end to end**, while staying
candid that a fully populated multi-billion-row table is a volume question — not
something these boundary proofs establish.
...
| Open table/work areas | 512 work areas — **a configured default, not a ceiling.** `DOTTALK_MAX_AREAS` is a `cmake` cache variable; the number is chosen against a measured ~1.3 KB per eagerly-allocated slot, and nothing in the format or the engine bounds it. The figures in the columns to the right are mostly format or product ceilings, so this row does not compare like with like | 65,535 tables open at one time, memory/file-handle limited | 256-512 open dBASE tables per system depending BDE generation | Implementation/runtime dependent | Runtime/DBE dependent | Not a work-area engine |
...
| Fields per table | 256 (`DOTTALK_MAX_FIELDS`, a configure-time cache variable; raised from 128 in 2026-07) — again a selected default rather than a ceiling | 255, or 254 when nullable-field overhead applies | 255 dBASE 4; 1024 dBASE for Windows | Backend/dialect dependent | Listed as not limited by DBFDBE table, but practical DBF/runtime limits apply | Dialect/library dependent |
