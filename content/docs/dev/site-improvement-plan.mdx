...
| Application UI DSL text syntax is not implemented | The LANE is Alpha and chartered -- design-table contract, importers, validators and four target backends exist. Only the text commands are absent. | Keep CREATE MENU/WINDOW/DIALOG under planned syntax; do not describe the lane itself as unstarted. See [Application UI DSL Lane](/docs/dev/application-ui-dsl-lane). |
