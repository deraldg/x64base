...
Use `implementation checkout` as the authoritative local source tree for DotTalk++ / x64base work.

Use `website source tree` as the website publication tree.

Use `local staging mirror` as the local runtime mirror for the
...
- Website publication should flow through `website source tree` to
  `local staging mirror`, then to GitHub Pages and
  `x64base.com`.
- If a technical claim diverges between the two trees, fix `implementation checkout` first and then republish the site.
