...
**Error handling inside a loop.** `SET ERRORSTOP` decides what an errored line
does. With it on, the line does not merely report -- the running construct
**bails out**, and `SCAN`, `LOOP`, `WHILE` and `UNTIL` all honour it (owner
ruling, 2026-09-01). With it off, execution continues past a recorded error.

**Iteration ceiling.** `LOOP` stops at 10,000 iterations and says so, rather
than running away quietly.

**Buffering in script files.** Before 2026-09-01, `SCAN`, `LOOP`, `WHILE` and
`UNTIL` buffered only in the interactive shell: run from a `DOTSCRIPT` file the
body was captured by the wrong reader and the construct executed over zero
lines. It reported success. All four now buffer identically from a file.
...
### AUTODBF: create a DBF from a CSV

`AUTODBF` builds a new X64 DBF from a CSV and imports its rows in one step, for when there is no table to `IMPORT` into yet:

```text
AUTODBF students FROM students.csv           && AUTO header, INFER types, X64, comma
AUTODBF students FROM students.csv HEADER     && force first row = field names
AUTODBF grades   FROM grades.csv NOHEADER     && generate deterministic FIELDnnn names
AUTODBF students FROM students.csv OVERWRITE  && replace an existing table
```

Defaults are the X64 flavor, `AUTO` header detection, `INFER` types, and comma-separated records (records are parsed as records, so quoted fields may span lines). With a header, CSV column names become field names normalized to command-safe x64 logical names and uniquified; with no header, deterministic `FIELDnnn` names are generated. Flags: `HEADER` / `NOHEADER` / `AUTO`, `INFER` / `TEXTONLY`, and `OVERWRITE`. Current limit: long text is not auto-promoted to a memo (`M`) field yet, so values must fit the current fixed-field widths.

Proof label: active beta (experimental command). Generated field inference and mutation safety still warrant review before use on non-disposable data; source is `src/cli/cmd_autodbf.cpp`.
