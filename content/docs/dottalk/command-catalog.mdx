...
Source extraction snapshot: `241` registered command keys and `241` parsed usage contract blocks.
...
| `AREA51` | diagnostics | developer | Report the current work-area slot and its cursor/order state WITHOUT invoking the full AREA command and WITHOUT triggering relation refresh. | `src\cli\cmd_area51.cpp` |
...
| `DBAREA` | workspace | supported | Report the current DbArea/work-area state, including file identity, the workspace that OWNS the area and the session's CURRENT workspace, logical name, record counts, current record, active order/index status, and field structure. | `src\cli\cmd_dbarea.cpp` |
...
| `GPS` | report | supported | Report current work-area position, including area slot, table label, physical record number, computed logical row, and the workspace that OWNS the area beside the session's CURRENT workspace. | `src\cli\cmd_gps.cpp` |
| `GROUPCOMMIT` | data | supported | Commit every buffered area as ONE atomic decision, so a crash leaves either ALL members applied or NONE. COMMIT ALL's atomic twin. | `src\cli\cmd_commit.cpp` |
...
| `SQL` | sql | reserved | Reserved verb. Reports where the scanning and statement surfaces now live. | `src\cli\cmd_sql.cpp` |
...
| `SQLSEL` | sql | supported | Typed set-oriented SELECT and DML over open x64base work areas. | `src\cli\cmd_sql_select.cpp` |
...
| `WORKDESK` | diagnostics | experimental | Report the desk -- every open workspace in this session with its areas, identity, lineage and roots. | `src\cli\cmd_workdesk.cpp` |
| `WORKSPACE` | workspace | supported | Report and manage live work-area/session layout, and the path environment each workspace carries (R131). | `src\cli\cmd_workspace.cpp` |
| `WSREPORT` | diagnostics | supported | Print a session status report: open workspaces and their areas, the order/LMDB summary, table-buffer state, and per-area index detail. | `src\cli\cmd_wsreport.cpp` |
