...
Source extraction snapshot: `237` registered command keys and `236` parsed usage contract blocks.
...
| `AVERAGE` | aggregate | supported | Average a numeric expression over the visible record set. (alias of AVG) | `src\cli\cmd_aggs.cpp` |
...
| `BUILD INFO` | diagnostics | supported | Report the compiled engine capacity "build vectors" (AIF-044) — the selected maximums this binary was configured with, plus a fingerprint. (alias of BUILD VECTORS) | `src\cli\cmd_buildvectors.cpp` |
...
| `COMMANDSHELP` | help | supported | Build or report the current HELP DATA catalogs and their legacy compatibility views. (alias of CMDHELP) | `src\cli\cmdhelp.cpp` |
...
| `EXIT` | shell | supported | Request DotTalk++ shell shutdown without mutating open table data, HELP data, metadata catalogs, manualgen artifacts, or source files. (alias of QUIT) | `src\cli\cmd_quit.cpp` |
...
| `EXPFUNCs` | help | supported | Export the expression/function catalog to Markdown documentation. (alias of EXPORTFUNCTIONS) | `src\cli\cmd_export_functions.cpp` |
...
| `FOXTALK` | turbo-vision-ui | supported-conditional | Launch the ArcticTalk Turbo Vision shell when TV support is available. (alias of ARCTICTALK) | `src\tv\cmd_foxtalk.cpp` |
...
| `REL_LIST` | relation | supported | Inspect and manage active relation definitions, relation files, and relation enumeration helpers. (alias of RELATIONS) | `src\cli\cmd_relations.cpp` |
...
| `STRCAT` | string | supported | Concatenate one or more expressions into a single string and print the result. (alias of CONCAT) | `src\cli\cmd_concat.cpp` |
...
| `UNDELETE` | table-mutation | supported | Clear deleted flags on the current record or selected deleted records. (alias of RECALL) | `src\cli\cmd_recall.cpp` |
