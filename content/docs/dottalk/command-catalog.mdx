...
Source extraction snapshot: `236` registered command keys and `215` parsed usage contract blocks.
...
| `BBS` | diagnostics | supported | AI-BBS / pseudo-chat board (M1): boards, threads, and posts persisted as x64base DBF tables under data/metadata/bbs/. The read-only board.governance projects the identity SYSGRANT request/approve loop (the agent&lt;-&gt;owner "pseudo chat") as posts. | `src\cli\cmd_bbs.cpp` |
...
| `SET RELATION` | registered | registered | Registered in the central shell command registry; no parsed @dottalk.usage block was matched by this website extraction pass. | `src/cli/shell_commands.cpp` |
| `SET UNIQUE` | registered | registered | Registered in the central shell command registry; no parsed @dottalk.usage block was matched by this website extraction pass. | `src/cli/shell_commands.cpp` |
