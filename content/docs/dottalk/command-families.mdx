...
| `SMARTBROWSER` / `SM` / `SMART` | Smart browser surface over table/tuple/workspace data. The shortcut resolver maps `SM` and `SMART` to `SMARTBROWSER`. |
| `SIMPLEBROWSER` / `SB` | Simple browser surface. The shortcut resolver maps `SB` to `SIMPLEBROWSER`. |
...
Use `SMARTLIST` / `SL`, `SMARTBROWSER` / `SM` / `SMART`, and `SIMPLEBROWSER` / `SB` for the current smart/list/browser surfaces.
...
## Education and Laboratory Campus

The Laboratory Campus should expose education commands as runnable labs:
`ASCII`, `BBOX`, `RETRO`, `STUDENTHELLO`, `SHELLO`, `STUDENTECHO`, `SECHO`,
`IDX`, `BOOLEAN`, `CASE`, `CHRISTMAS`, `COBOL`, `CODASYL`, `NORMALIZE`, and
`SIX`.
