...
//
// The lane's published cap table was measured under g++/libstdc++ x86-64:
//     sizeof(DbArea) = 1088   sizeof(AreaState) = 176   ~1296 B/slot
// MSVC lays out std::string, std::multimap and std::fstream differently, so
// those figures are runtime-evidenced FOR THE WRONG COMPILER. That is what G0
// exists to close. "Measured on the wrong target is still not measured"
// (COST_BENEFIT_GATE_DOCTRINE_V1.md).
//
// Build (from D:\code\ccode, Release-equivalent flags; adjust /I if the
// generated build-vectors header lives under a different preset dir):
//
//   cl /std:c++20 /EHsc /O2 /nologo ^
//      /I include /I build\generated ^
//      src\AIPortal\sessions\2026-07-30_cowork_workspace_qualifier\g0_size_probe.cpp ^
//      /Fe:build\g0_size_probe.exe
//
// Then run it and tee the output to a committed transcript under
// labtalk\proofs\runs\ -- a proof row must cite a tracked artifact
// (AI_ENGINEERING_STANDARDS_SEED_V1.md sec 5c).
...
int main() {
    ...
    std::printf("AIF-078 G0 -- per-slot cost, shipping toolchain\n");
#if defined(_MSC_VER)
