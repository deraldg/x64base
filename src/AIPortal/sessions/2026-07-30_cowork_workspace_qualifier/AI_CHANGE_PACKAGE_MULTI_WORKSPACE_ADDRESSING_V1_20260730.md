...
**Status:** `review-needed` -- **SUPERSEDED IN PART.** Read `AI_COST_BENEFIT_WORKSPACE_QUALIFIER_VS_SQLSEL_P41_V1_20260730.md` first. That analysis reframes this package: SQLSEL phase P4.1 already owns table-reference qualification (`SQLSEL_PLDC_LANE_V1.md:112`), so the live decision is the *namespace depth* of a qualifier being authored now, not a standalone multi-workspace lane. §4.1's registry and §4.2's grammar remain the reference design **if and when** the feature is built; §9 Q8 is withdrawn as wrong. The recommendation there is to buy the option, not the feature.
