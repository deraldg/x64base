# AI Change Package — LMDB-Lane In-Transaction Index Key Maintenance

**File:** `AI_CHANGE_PACKAGE_LMDB_INDEX_TXN_MAINTENANCE_V1_20260721.md`
**Supersedes:** `AI_CHANGE_PACKAGE_NATIVE_INDEX_TXN_MAINTENANCE_V1_20260721.md` (terminology corrected — see glossary)
...
**Scope:** Wire the existing `IndexManager` snapshot/bulk API into the COMMIT path so **LMDB-lane** index keys are maintained incrementally inside one LMDB transaction, replacing full rebuild on key mutation. Native (own-format CDX) and CNX unchanged.

> **Terminology (corrected).**
> - **Native indexing** = CDX's *own* self-contained on-disk index format (`include/cnx/cdx.hpp`, `src/cdx/cdx_file.*`) — CDX-native, independent of LMDB. **This package does not touch native indexing.**
> - **LMDB indexing** = the CDX front-end organizer running over the LMDB backend (`src/xindex/cdx_backend.cpp` `CdxBackend` + `LmdbBackend`; opens an LMDB env dir, all `mdb_*`). This is the **default index lane** and the **subject of this package**. (Note: the Engine Feature Crosswalk phrasing "V64 defaults to `.cdx`" refers to the container/extension naming; the active default backend is LMDB — confirm crosswalk wording in Q7.)
> - **CNX** = the self-contained `.cnx` file lane (V32), no mutable tree — rebuild-only.
>
> *(Follow-up flagged: standardize native vs LMDB vs CNX naming across HELP/manualgen and the Engine Feature Crosswalk — Q7.)*

---

## 1. Intent

**Because LMDB is the default index lane, this is the primary write path, not an edge case** — every indexed key mutation on a default-configured table currently forces a rebuild. Today an indexed key `REPLACE` on the LMDB lane does not update the index; correctness is currently recovered by a full rebuild. The engine already has:
...
This package connects those to `COMMIT`, aligned to the existing record-lock + write-ahead-journal critical section, so LMDB-lane index and data commit/roll back together.
...
|---|---|
| `src/cli/cmd_commit.cpp` | `commit_one_area` locks each recno via `xbase::locks::try_lock_record`, applies buffered field writes, then `auto_reindex_if_needed`; write-ahead `journal_begin_commit` runs before DBF apply; buffer restored (`tb.changes = pending_before`) on finalize failure. `auto_reindex_if_needed` **skips the LMDB lane** by contract. |
...
| `include/xindex/index_manager.hpp` | `capture_delete_snapshot_for_current_record` (LMDB lane walks all field-backed tags), `apply_replace_snapshot(before, after, rec)` (delete-all-before then insert-all-after), bulk API is LMDB-lane-only, no-op/false otherwise. |
| `src/xindex/cdx_backend.cpp` | Opens an LMDB env (`<container>.d`), all `mdb_*`. While `bulk_txn_` is open, `upsert`/`erase`/`setTag` route onto the shared txn. `seek`/`scan`/`seekRecnoUserKey`/`stepOrdered` open a **separate** `MDB_RDONLY` txn (do **not** see bulk-pending writes). DBI storage mode derived per-call from live flags (composite key `base‖recno8` vs DUPSORT dup value). |
...
2. `COMMIT` → applies field writes under per-record lock; `auto_reindex_if_needed` prints `CdxSkipped` and returns `true` for the LMDB lane → **index never updated at commit**.
3. Net: the LMDB index is stale after any key mutation until an out-of-band rebuild/`BUILDLMDB`. This is the behavior being replaced.

---

## 4. Proposed call sequence

### 4.1 COMMIT (buffered path) — LMDB lane only

Insert an index-maintenance wrapper into `commit_one_area`, gated on `orderstate::isCdx(A)` (the LMDB-backed `CdxBackend`):
...
commit_one_area(A, area0):
    ...
    lmdb = isCdx(A)                                           # NEW (LMDB-backed lane)
    if lmdb:                                                  # NEW
    ...
    for each recno in tb.changes:
        ...
        # --- NEW: pre-image key snapshot (old keys), LMDB lane only ---
        before = lmdb ? im.capture_delete_snapshot_for_current_record() : {}
    ...
    if applied_fail != 0:
        if lmdb: im.abortBulkWrite()                           # NEW: drop all index edits
        set_dirty(area0,true); return PartialRecordFailure     # buffer retained (unchanged)

    if memo flush fails:
        if lmdb: im.abortBulkWrite()                           # NEW
        tb.changes = pending_before; return FinalizeFailure    # unchanged otherwise

    # auto_reindex_if_needed stays a SKIP for the LMDB lane (now truthfully
    ...
    if lmdb:                                                   # NEW: commit index with data
...
- `src/cli/cmd_commit.cpp` — add bulk wrap + per-recno pre/post snapshot calls in `commit_one_area`; adjust `auto_reindex_if_needed` LMDB branch message from "skipped" to "maintained".
- `src/cli/cmd_replace.cpp` — `TABLE OFF` path: add `replace_active_field_value` after successful `replaceFieldStored` (LMDB lane only).
...
- No change: `cmd_rollback.cpp`, `cnx_backend.cpp`, `lmdb_backend.cpp`, and the native `.cdx` format sources (`cnx/cdx.hpp`, `cdx_file.*`).
...
- Indexed key `REPLACE` + `COMMIT` on the LMDB lane updates the index in O(log n) per key instead of O(n) rebuild.
- LMDB index and data become atomic within the LMDB txn boundary; `COMMIT` failure paths abort the index txn and retain the buffer for retry (matches existing semantics).
- `ROLLBACK` unchanged and correct.
- CNX still rebuilds at commit; INX/IDX still reindex; native `.cdx` format lane untouched.
...
1. **Correctness:** `USE`; `SET ORDER TO <keytag>` (LMDB lane); `TABLE ON`; `REPLACE key WITH <newval>`; `COMMIT`; `SEEK <newval>` → lands on the record; `SEEK <oldval>` → not found. No `REBUILD`/`BUILDLMDB` between.
...
- **Q7.** *(Terminology follow-up)* Standardize **native** (own-format `.cdx`) vs **LMDB** (CDX-over-LMDB) vs **CNX** across HELP, manualgen, and the Engine Feature Crosswalk. Also decide whether native `.cdx` is an active lane, legacy, or planned — and whether it should ever get the same in-transaction maintenance (it would need its own mutable-tree write path, unlike the LMDB lane which gets it for free from `mdb_put`/`mdb_del`).

---

## 9. Fallback

If any exit condition fails, revert to current behavior: leave `auto_reindex_if_needed` LMDB-skip as-is and mark the area stale so an explicit `BUILDLMDB`/rebuild restores correctness. The change is additive and gated on `isCdx`; disabling the wrap restores today's semantics.
