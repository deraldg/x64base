...
// future Dewey Decimal educational material, exports no shell command,
// registers nothing, and defines nothing.
//
...
// our systems design". This translation unit reserves Dewey Decimal's place in
// the education subsystem, stated in the vocabulary the build reads: a file
// that exists. Stronger than a TODO -- it survives refactors, appears in the
// census, and must be deliberately deleted to disappear.
//
...
//
// ONE OF THREE IDENTICAL PLACEHOLDERS -- edu_boyce_codd.cpp,
// edu_dewey_decimal.cpp and edu_snx.cpp: all 35 lines, all zero definitions,
// all declaring `command: none`. Fixing the first surfaced the second, which
// surfaced the third. CONTRACT_QA reports one INVALID_IDENTITY per NAME, and
// all three shared the name `none`, so a defect repeated three times presented
// as a single finding. Worth remembering when reading any grouped report: the
// count is of distinct values, not of occurrences.
//
// IF A DEWEY COMMAND EVER BECOMES USER-FACING: add the runtime handler and the
