...
TStatusLine* buildStatusLine(TRect bounds)
{
    ...
    return new TStatusLine(
        ...
        *new TStatusDef(0, 0xFFFF)
            ...
            + *new TStatusItem("~F3~ Record",           kbF3,     cmRecordView)
            ...
            + *new TStatusItem("~Up/Dn~ History",       kbDown,   0)
            + *new TStatusItem("~Esc/Ctrl-U/K~ Edit",   kbEsc,    0)
            + *new TStatusItem("~Alt-X~ Close",         kbAltX,   cmTalkExit)
    );
}
