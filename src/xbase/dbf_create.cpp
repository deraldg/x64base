...
static bool validate_lengths_for_flavor(const std::vector<FieldSpec>& fields,
                                        Flavor flavor,
                                        std::string& err)
{
    for (const auto& f : fields) {
        if (f.len == 0) {
            err = "field '" + f.name + "' has zero length";
            return false;
        }
        if (flavor != Flavor::X64 && f.len > 255u) {
            err = "field '" + f.name + "' exceeds classic/VFP 255-byte field length limit";
            return false;
        }
        if (flavor == Flavor::X64 && f.type != 'C' && f.len > 255u) {
...
static std::uint8_t descriptor_length_for(const FieldSpec& f, bool x64_context)
{
    if (x64_context && f.type == 'C' && f.len > 255u) {
        return 0xFF;
    }
    return static_cast<std::uint8_t>(f.len);
}
                              ...
                              std::string& err)
{
    ...
    for (const auto& f : fields)
    {
        ...
        out.put((char)f.type);
        put_u32_le(out, 0u);
        out.put(static_cast<char>(descriptor_length_for(f, false)));
        out.put((char)f.dec);
                          ...
                          std::string& err)
{
    ...
    {
        if (f.len == 0) {
            err = "X64 field length cannot be zero";
            return false;
        }
        if (f.type != 'C' && f.len > 255u) {
            err = "Only X64 character fields may exceed 255 bytes in current CREATE";
            return false;
        }
        recLenWide += f.len;
