...
- Indexes.
  ...
  DOTTALK++ -- current, contract-backed.
    ...
    Build them with the container command for the table's flavor. INDEX ON
    builds an INX file, which is a separate lane:

      x64 / v128 -- CDX. Runtime tag mutation IS supported.
        USE <table>
        CDX CREATE
        CDX ADDTAG <field>
        BUILDLMDB CLEAN YES

      x32 / MS-DOS -- CNX, a sorted index by tag. BATCH REBUILD ONLY;
      runtime tag mutation is NOT supported on this lane.
        USE <table>
        CNX CREATE
        CNX ADDTAG <field>
        REINDEX CNX

    Per USE: "x64/v128 tables prefer CDX. x32 tables prefer CNX, then INX."

    Reproducible build scripts: dottalkpp/data/scripts/mcc/

  Corrected 2026-07-14. See
