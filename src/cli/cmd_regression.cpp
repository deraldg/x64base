...
constexpr std::array<RegressionSpec, 49> kRegressionSpecs{{
    {
        "NONDESTRUCTIVE",
    ...
    },
    {
        ...
        "NAME AMBIGUITY: one resolver, and a duplicate logical name now says so (AIF-120 I1.3a, 2026-08-22). Two functions answered 'which open area is called X' and disagreed: find_open_area_by_name_ci (workarea_util.cpp, 21 call sites) returned the FIRST match -- the lowest engine slot -- while build_area_by_up_name (set_relations.cpp, the recursive REL LIST tree builder) built its own map with an unconditional out[key] = a, making it the LAST match, the highest slot. Same question, two answers, neither saying which area it picked. The local map is deleted, not replaced in kind: both resolvers are now built on one primitive (find_open_areas_by_name_ci, every match ascending by slot), so they agree BY CONSTRUCTION rather than by inspection. R112 sec 3 measured the reach -- twelve basenames exist in all three of dbf/x64, dbf/x32 and dbf/vfp (students, teachers, classes, enroll, courses, dept, majors, rooms, stud_maj, tassign, terms, building), the MCC demo corpus -- and it needs no second workspace, because USE guards duplicates by PATH only and USE ... ALIAS assigns the logical name with no uniqueness check, which is how this script builds its fixture in one workspace. WHAT THIS SPEC CANNOT SEE, stated rather than implied: the divergence itself is NOT field-observable -- the tree builder's map fed record counts that get PRINTED and never moved a cursor -- so NO MARKER HERE GOES RED ON THE OLD BUILD and this is not a discriminator for the consolidation. The compiler is: the function is gone and cannot be called. Saying so is the point, because a spec that cannot fail on the defect it is named after is exactly the false green this house keeps finding. What the markers DO pin is the ruled CHOICE. R112 sec 6a ruled first-wins-plus-instrumented-warning as the migration step toward requiring qualification, so N_T1/N_T2 assert that choice by field value and MOVE the day the policy flips to a hard refusal -- the flip becomes a red marker and its own ruling, which is what 'time-boxed by measurement' has to mean. N_T2 IS THE DISCRIMINATOR FOR DIRECTION: NAMLO and NAMHI are both open as NAMDUP in areas 1 and 2, the parent moves, and exactly one child follows; a green N_T2 says the HIGHER slot stayed put, so anyone 'fixing' ambiguity by making the last match win reds both lines at once. THE LEDGER IS TRANSCRIPT EVIDENCE, not a marker: WORKSPACE REGISTRY prints 'name ambiguity : N resolution(s)' plus one row per distinct (name, call site), and R112 sec 6a is explicit that a printed warning nobody counts is the thing it rejected -- the counter has to reach a MEASURED ZERO across this corpus before first-wins becomes a refusal. The NAME-AMBIGUITY-LEDGER block MUST therefore contain a non-zero count and a NAMDUP row; the same field printed as 0 elsewhere is the gate, which is why it prints even when it is zero -- 'absent' and 'not instrumented' must not look alike (AIF-118). Eleven REL call sites are tagged; every other caller records as 'unattributed', printed as such rather than as no site, because an unattributed hit is a call site nobody has labelled yet. Self-bootstrapping NAMPAR/NAMLO/NAMHI in SANDBOX, self-erasing. Explicit-run until soaked, then promote to default.",
        false
    }
}};
