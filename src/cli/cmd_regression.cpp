...
constexpr std::array<RegressionSpec, 84> kRegressionSpecs{{
    ...
    },
    {
        ...
        "SQLsel G2 (AIF-074): the sqlsel library target owns statement execution and session mode state; SQL mode routes SELECT to SQLSEL, blocks REL/SET RELATION and relation auto-refresh, intercepts native expression fallback, and marks the prompt. NATIVE/OTHER retain work-area SELECT, canonical SQLSEL is mode-invariant, and mode is process-local. Two SELECT/SQLSEL row sets are compared with SQLite; four behavioral mode/cursor markers and three exact refusals are required. Self-erasing SANDBOX fixtures. Explicit-run pending mutation proof, independent review, and soak. PROMOTED TO THE DEFAULT SUITE 2026-09-23, and the explicit-run sentence above is LEFT STANDING because it was the condition at the time; all three preconditions are now discharged. MUTATION PROOF 2026-09-23, one targeted single-defect script mutation per grading class, no rebuilds (the .dts is data): oracle divergence (the SELECT-alias arm queried ID=1 while SQLite O1 kept ID=2 -> FAIL naming the pair, rows 1|ALPHA vs 2|BETA); exact-block shortfall (SET RELATION line removed -> FAIL 'SQLMODE-REFUSALS expected 2 line(s), got 1'); forced-false cursor marker (C1 asserting the wrong row -> FAIL naming the missing fragment, the .F. visible in transcript). Baseline green FIRST, tracked-bytes restore via git checkout after each -- and the baseline is itself a finding: the spec, authored in the AIF-074 era, survived the 2026-09-04 scanner retirement and the 2026-09-10 predicate retirement without rot. SOAK: two PASS runs on ONE build, both banners 'dottalk++ beta 1.b, 648a7a3e  (Sep 23 2026 04:38:55)' transcribed directly from the operator's console, back-to-back with nothing changed between them; rows in coordination/SOAK_EVIDENCE.md. THE REASON IS A MEASURED COVERAGE HOLE: no default spec asserts ANY SQL-mode ROUTING behavior. The suite's only other SQL-mode exercise is the GRPMINT arm, which covers BEGIN/COMMIT transactions and nothing else; the SELECT alias, the REL/SET RELATION block, the expression-fallback interception and the mode-restoration cursors live ONLY in this spec -- a regression quietly returning SQL-mode SELECT to native work-area SELECT (the pre-AIF-074 behavior) would leave REGRESSION ALL green. The cost supports it: self-bootstrapping self-erasing SANDBOX fixtures, no catalog rows. INDEPENDENT REVIEW is the flip commit itself: the operator reads this package before committing it, and the review that matters is the one attached to the act. The flip needs a REBUILD (the registry is compiled in); read REGRESSION LIST for the [default] tag BEFORE believing any run; the ALL headline moves 33 -> 34. VERIFIED IN-SUITE 2026-09-23 -- the first post-flip REGRESSION ALL: specs run 34, passed 34, FAILED 0, exactly as predicted. The position bought a measurement standalone never could: this entry declares mid-registry, so in-suite it ran after DEF_FAMILY and BEFORE the entire SQLSEL family, inheriting RELJOIN's still-open x64 schema -- its USE into area 1 landed on an OPEN CLASSES table and replaced it -- and graded 8/8 identically to standalone anyway, while every SQLSEL spec downstream of its SET MODE SQL/OTHER/NATIVE round-trip and its REL clear stayed green. That is an ORDER claim for THIS registry, not an isolation proof (the 8fba98c5c ruling); a re-ordering re-opens it and the next post-reorder ALL is its measurement.",
        true,   // PROMOTED 2026-09-23 -- soak rows in coordination/SOAK_EVIDENCE.md
        false,
    ...
    },
    {
        ...
        "SQLSEL statement surface, gate G3 (AIF-074 P3): SELECT <cols|*> FROM <table> with typed expression projection, WHERE, ORDER BY [ASC|DESC], LIMIT and COUNT(*), each row set compared against an in-process SQLite oracle over identical data in the same run. Asserts cursor neutrality by data, corrective errors for an unopened table / bad LIMIT / unknown ORDER BY field / ORDER BY on COUNT(*), and that ORDER BY sorts the full match set BEFORE LIMIT applies. LEGACY PREDICATE FORM RETIRED 2026-09-09 by owner ruling (a) on AIF-074 (1a96dac9b: cmd_sql_select.cpp 618 lines to 225, the private DelMode and the raw walk deleted, a corrective error naming COUNT left in their place). THIS ENTRY SAID THE FORM WAS PRESERVED FOR THREE DAYS AFTER THE DELETION AND NOTHING WENT RED, because this spec never exercised that form: a spec summary is PROSE, and prose is not graded by the spec it describes. It was found by a person reading the catalog, which is the same way the four stale site sentences were found. Self-bootstrapping throwaway SQLSTU table in SANDBOX; self-erasing. Promoted 2026-09-03 because the prior default suite invoked SQLSEL HELP but executed no SQLSEL statement, leaving the complete statement path unasserted.",
        true,
        false,
    ...
    },
    {
        ...
        "SET PARALLEL differential (AIF-168 PERF-2, the registered G-P2 gate): a 2400-row self-bootstrapping SANDBOX fixture (ID == RECNO by construction) shaped so DELETED rows sit in BOTH 2-worker partitions and in a band that STRADDLES the partition seam (recno 1200/1201). Five OFF/ON pairs are pinned as EXACT blocks -- the OFF leg IS the oracle -- covering bare COUNT, char WHERE, function WHERE, a seam-straddling numeric band, and ORDER BY+LIMIT row order. The validator also pins the clamp (SET PARALLEL 6 over 2400 rows must RUN and REPORT workers=2), both declines with their exact serial answers (5-row table too small; subquery predicate, whose 5-row inner select declines again), exact occurrence counts for the workers=2 path line (6) and each decline, the fixture shape (one 100-row UPDATE, three 10-row DELETEs), two cursor restorations, and that the spec leaves PARALLEL OFF for whatever runs next. R21 is exercised structurally: every ON leg runs workers that opened PRIVATE DbAreas. The deleted-rows-per-partition probe this spec carries is the one the pinocchio diff script could not (read-only fixtures carry no deleted rows). PROMOTED TO THE DEFAULT SUITE 2026-09-22 on the NULLASSERT precedent: first red graded a real fixture defect (2370 vs 2400, RECNO-through-REPLACE), mutation proof green-red-green on build e88408bb with the arm doubling as an OFF=191/ON=190 divergence witness (0b458adab), then TWO PASS runs on ONE build -- banner 'dottalk++ beta 1.b, c559013d  (Sep 22 2026 17:46:44)' both times, rows in coordination/SOAK_EVIDENCE.md. The REASON is a measured coverage hole, not the soak alone: before this spec, REGRESSION ALL never ran a single SET PARALLEL statement, so a partitioning defect that lost or double-counted rows at the seam would have shipped green. The flag flip needs a REBUILD (the registry is compiled in); read REGRESSION LIST for the [default] tag BEFORE believing the run -- NAV_NATURAL's entry records the wasted REGRESSION ALL that rule exists to prevent.",
        true,
        false,
    ...
    },
    {
        ...
        RegressionValidator::WorkspaceMarkersV1
    },
    {
        "USE_ARGS",
    ...
    },
    {
        ...
        "THE REPORTS THAT NAME A CONTAINER, AN ENV OR AN ORDER MUST NAME THE ONE THE ENGINE ACTUALLY USES (2026-09-10, from the metadata scan of the same day). Four verified falsehoods shipped as ab9905e56 and NOT ONE OF THEM HAD AN ARM: REGRESSION ALL was green through the whole defect and would have stayed green through its return. This spec is that missing instrument. IT IS GRADED BY A VALIDATOR AND NOT BY MARKERS, AND THAT IS FORCED RATHER THAN CHOSEN. Every claim here is CONSOLE TEXT -- a path, an envdir, a parenthetical -- and a marker in this language is a FIELD-VALUE comparison, the limit already recorded against LIST, COUNT and RECNO (the IDXDIFF precedent). MetadataReportV1 reads the routed capture and grades EIGHT FENCED BLOCKS, MR-P1 through MR-P8. A MISSING BLOCK IS A FAILURE, never a skipped check: that is the house COUNT THE MARKERS rule applied to fences, because a spec that quietly stops printing one of its eight would otherwise pass while asserting seven things. NOT ONE ASSERTION IS A LITERAL PATH. Every path claim is either an EQUALITY BETWEEN TWO VERBS' ANSWERS or a claim about an EXTENSION, so a clone on another drive letter grades identically and renaming the fixture touches nothing in the validator. Comparisons fold case, because these are Windows paths and a case-only difference is not a defect; the ORIGINALS are printed on failure. THE DISCRIMINATOR IS A SLOT THAT MOVES, AND THAT CORRECTS THIS TREE'S OWN WRITTEN ASSUMPTION. Both the scan and ab9905e56's commit message said proving the CDX INFO repair needed a two-workspace fixture with doubled basenames and divergent INDEXES/LMDB roots, and that no such fixture existed. IT NEEDS NO FIXTURE AT ALL. Attach a container, then re-point the INDEXES slot somewhere the container is not: a build whose reuse branch fires answers from the ATTACHMENT, a build that falls through to resolve_cdx_token(stem) answers from the SLOT. One SET PATH line replaced a fixture nobody had built. MR-P1 IS A CONTROL AND IS NOT EVIDENCE, stated so it is never read as one: while the slot still agrees with the attachment BOTH builds answer the same thing, and P1's only job is to supply the value P2 is measured against. If P1 names no container the validator reports UNPROVEN rather than FAIL, because an arm with no reference value has not failed, it has not run. MR-P3 IS THE MIRROR AND THE HALF THAT BIT HARDEST, and it must be READ BY THE EXTENSION AND NOT BY THE VERDICT. With a .cnx attached the old predicate FIRED and handed the .cnx path to a CDX verb, which then reported not-found on a file that exists. In this block 'file not found' is the CORRECT answer -- the x32 tree has no .cdx -- so what is graded is that the path ends .cdx and that no .cnx appears anywhere in the block. An arm that graded the verdict would have read this backwards. MR-P4 IS AN ORACLE FROM CODE NOBODY CHANGED. SET INDEX prints the env the backend opens; P5's echoed envdir must equal it. That is what makes the claim assertable without a literal path, and it is the same shape as the SQLSEL specs' use of SQLite: the reference comes from an instrument the change under test cannot have moved. The two slots are DIFFERENT ROOTS in this run on purpose, so gluing '.d' onto the container path lands under INDEXES while the backend opens what resolve_lmdb_env_for_cdx() returns, rooted at LMDB. MR-P8 IS THE COUNTERWEIGHT AND IS THE REASON THE FIX CANNOT OVERSHOOT. A change that forced PHYSICAL unconditionally would pass P6 and P7 and be EXACTLY as wrong as the defect was, so a chosen tag must still bring a real direction back -- asserted in the SAME run, four lines below the PHYSICAL readings, so the two cannot both be accidents. EXPLICIT-RUN, AND IT MUST STAY THAT WAY UNTIL IT IS SOAKED. Promotion doctrine is the NULLASSERT precedent: two green runs on a build NOBODY CHANGED ANYTHING ON, then the flag moves, then a REBUILD, then read REGRESSION LIST for the [default] tag BEFORE believing the run -- NAV_NATURAL's entry records a whole REGRESSION ALL wasted on skipping that last step. The soak is now enforced by tools/staging/check_soak_evidence.py, which blocks a false->true flip that does not name two PASS runs on ONE build in coordination/SOAK_EVIDENCE.md. THE VALIDATOR WENT RED ON 2026-09-10 AND THE DEBT IS PAID -- but read what it MEASURED, not what it was predicted to. The three repaired .cpp files were restored from 0db8c64d3 into the working tree, rebuilt, run, restored to HEAD, rebuilt and run again: FAIL then PASS on the SAME fixtures one minute apart (builds Sep 10 2026 15:20:10 and 15:21:17), L3 six of six at both ends of both runs. A validator that has only ever passed has an unmeasured FAIL path -- DEF_FAMILY's recorded mistake -- and this one no longer does. MR-P2 WAS CONFIRMED AND THE PREDICTION UNDERSTATED IT. It was predicted to name a container under INDEXES\\SANDBOX. It did -- and there IS one there, carrying FOUR tags against the attached container's SIX. So the pre-fix verb did not error and did not come up empty: it confidently reported a FOUR-TAG DIRECTORY FOR A SIX-TAG TABLE. That is R131's defect in the flesh, a report about one workspace's tree for a table opened out of another's, delivered with nothing to suggest anything was wrong. A prediction of 'not found' would have been the comfortable answer and it was not the true one. MR-P3's EVIDENCE IS IN THE TRANSCRIPT AND WAS NOT GRADED, and that distinction is load-bearing rather than pedantic: THIS VALIDATOR FAILS FAST, so it returned at MR-P2 and never reached MR-P3. What the pre-fix transcript carries inside the MR-P3 block is `CDX INFO: invalid header.` -- NOT the predicted 'names the .cnx', and worse than it. The old predicate handed a perfectly good CNX to a CDX parser, which pronounced the file CORRUPT. A user meeting that line would believe their index was damaged. Corroborated BY TRANSCRIPT, not by the grader, and recorded that way rather than smoothed into a claim the run did not make. MR-P5, MR-P6 AND MR-P7 REMAIN PREDICTIONS, for the same fail-fast reason, and each needs its own targeted back-out to reach: restore cmd_setlmdb.cpp alone for MR-P5, cmd_setorder.cpp alone for MR-P7. MR-P6 sits behind MR-P5 in the same file and needs the report-only half backed out by itself. Stated as OWED rather than left implied by 'the validator went red'. MR-P1, MR-P4 AND MR-P8 STAYED GREEN ON BOTH BUILDS exactly as designed -- the first two because they are references, MR-P8 because it discriminates the OVERSHOOT and never the defect. The controls held, so the red is the fix's absence rather than a broken fixture, which is the reading the whole control/measurement split exists to make available. AND THE TWO BUILDS ARE INDISTINGUISHABLE BY BANNER. Both printed `5ccad4b7 dirty`: the commit did not move, and the tree carries unrelated modifications either way, so the dirty flag says nothing. Only the build TIMESTAMP differs. tools/staging/check_soak_evidence.py proves two runs used ONE build by comparing banners -- so A RESTORED-FILE BUILD IS INVISIBLE TO IT, and a soak taken either side of one would read as the same build when it was not. Harmless here, because nothing was promoted. It is not harmless in general, and this run is the evidence for it. NOT CLAIMED, stated rather than implied: CDX INFO'S PER-TAG COUNTS. Both readings print root_off=0 and recs=0 for EVERY tag of a container over a 200-row table, while the CNX reader in the same transcript prints entries=200 for the same six tag names. Two readings fit -- the fields are wrong, or CDX INFO does a header-only read and displays two unpopulated fields as though authoritative -- and this spec cannot separate them. It is the same species as the four repaired here and was found the same way, by reading a report instead of trusting it. RESOLVED BY SOURCE 2026-09-22, and it was the SECOND reading, sharpened: the fields are honestly zero ON DISK. Only the NATIVE rebuild (cdx_native_backend.cpp's RUN8 writer) sets root_page_off/stats_rec/updated_ts and writes the tag directory back; BUILDLMDB -- the x64 default flow -- reads the directory for tag NAMES only, builds every key into the LMDB env, and never writes the directory back, exactly as the M0 reconciliation recorded stats_rec 'available and unused'. The CNX reader prints entries=200 because ITS build path (the RUN1 writer) does populate the same fields. The repair went where the defect was, the PRESENTATION: CDX INFO now appends a pathless note when the container's LMDB env exists, naming which build path maintains those fields and pointing at LMDB INFO for the live keys. Pathless BY DESIGN so this validator holds unchanged: MR-P3 fails any line mentioning a .cnx and the note names no path at all; MR-P1/P2 read only the 'CDX file' line. Also not claimed: anything about a SECOND workspace, and anything about the ORDER a cursor actually walks (that is NAV_NATURAL's job; this spec asserts only what the reports SAY). COST AND FOOTPRINT: READ-ONLY. It opens shipped fixtures, creates no table, erases nothing, and mints NO catalog row -- none of the three minting verbs appear, so mints_catalog is false. It re-points the DBF, INDEXES and LMDB slots and the harness restores them. It sets ECHO OFF and STOP_ON_ERROR OFF and does not restore them, which is the DEF_FAMILY shape and harmless where it sits. FIXTURE DEPENDENCIES, so a red is diagnosable: DBF/x64 STUDENTS with INDEXES/x64/STUDENTS.cdx and its LMDB env, and DBF/x32 STUDENTS with INDEXES/x32/STUDENTS.cnx. All ship. P2 is only a discriminator because INDEXES/SANDBOX is a DIFFERENT root from INDEXES/x64; it does not matter whether a STUDENTS.cdx happens to sit there. PROMOTED TO THE DEFAULT SUITE 2026-09-23, and the EXPLICIT-RUN sentence above is LEFT STANDING because it was the condition at the time. The soak: two PASS runs on ONE build, banner 'dottalk++ beta 1.b, 632c737a  (Sep 22 2026 20:44:51)' -- run 2's banner transcribed directly from the operator's console; run 1's banner was cut from the paste but its identity is PINNED BY RUN 2'S OWN DATARUN HEADER, which staged the same exe ('built 405 min ago', newest candidate unchanged) seconds after run 1 completed -- a documented derivation, not a reconstruction, recorded the same way in coordination/SOAK_EVIDENCE.md. Both runs also verified the CDX INFO honesty note live: present in MR-P1/P2 where the env exists, absent in MR-P3, validator 8/8 untouched. THE REASON IS THE MEASURED COVERAGE HOLE THIS ENTRY OPENS WITH, the RELSCOPE2 precedent: four verified falsehoods about containers, envs and order shipped as ab9905e56 while REGRESSION ALL stayed green, because no default spec read a metadata report at all. The cost supports it: READ-ONLY, no catalog rows, shipped fixtures only. STILL OWED AND UNCHANGED BY PROMOTION: the MR-P5/P6/P7 targeted back-outs. IN-SUITE POSITION: this entry declares EARLY in the registry, so unlike NULLASSERT it does NOT run last; it sets ECHO OFF and STOP_ON_ERROR OFF without restoring them, which the DEF_FAMILY precedent treats as harmless -- and the first post-flip REGRESSION ALL is the measurement of that claim, not a proof offered in advance. The flip needs a REBUILD; read REGRESSION LIST for the [default] tag BEFORE believing the run. VERIFIED IN-SUITE 2026-09-23 -- the first post-flip REGRESSION ALL on the rebuilt registry: specs run 33, passed 33, FAILED 0, the headline arithmetic exactly as predicted (31 defaults plus the two arms), and this spec's own 8/8 mid-suite with the honesty note present in MR-P1/P2 and absent in MR-P3, same as standalone. CORRECTED 2026-09-23: the IN-SUITE POSITION sentence above said this entry declares EARLY and does not run last. MEASURED FALSE by that same run -- this entry is declared LAST in the registry array (after VARCHARRESET) and EXECUTED last among the defaults, immediately before the GRPMINT and L3 arms. The sentence is left standing because that is how corrections are filed here; the error was the author's reading of the array, not anything the engine did. What the position claim existed to guard still got measured, from the other end: the unrestored ECHO OFF / STOP_ON_ERROR OFF had only the two arms downstream, and both graded green (GRPMINT absent-before/present-after with GM_D0 credited, L3 6/6), so no NULLASSERT-shape leak was observed. That is a claim about THIS declaration order only, never a proof of isolation -- the 8fba98c5c ruling against NULLASSERT applies here verbatim: 'it runs last' is a statement about ORDER. A future re-ordering of the registry re-opens the question and the next post-reorder ALL is its measurement. MR-P5/P6/P7 BACK-OUTS: DEBT PAID 2026-09-23, all on commit 648a7a3e, each cycle FAIL then PASS on consecutive incremental builds. MR-P5 (whole pre-fix cmd_setlmdb.cpp from 0db8c64d3, clean because ab9905e56 is the ONLY commit to touch either file since): FAIL naming P5, envdir echoed under INDEXES against the P4 oracle's LMDB root, exactly as predicted -- and that transcript's P6 block shows TAG '' (ASC), the P6 defect present but ungraded behind the fail-fast, corroborated BY TRANSCRIPT the same way the 09-10 run corroborated P3. MR-P7 (whole pre-fix cmd_setorder.cpp): FAIL naming P7, bare SET ORDER claiming (ASC); P6 read PHYSICAL first, which also measures that P6 does not live in cmd_setorder.cpp. MR-P6 needed a VARIANT -- the pre-fix file with the envdir half of the fix put back, machine-diffed against 0db8c64d3 to prove it differs ONLY in that half -- and its red run shows P5 passing then P6 failing on TAG '' (ASC), the two halves separated at last. AND THE FIRST MR-P6 ATTEMPT WAS NOT A RUN, which is the finding worth the detour: the variant was installed with PowerShell copy, which preserves the SOURCE file's LastWriteTime; the copied file read older than its .obj, MSBuild compiled NOTHING, and the 'FAIL-expected' run printed PASS from the previous build's binary. The build TIMESTAMP in the banner caught it -- identical to the prior run's, Sep 23 2026 04:36:06 twice -- and the rerun with the mtime touched went red as predicted. That is check_soak_evidence's banner doctrine at single-build granularity: a run whose build stamp did not move is not a run, however plausible its verdict.",
        true,   // PROMOTED 2026-09-23 -- soak rows in coordination/SOAK_EVIDENCE.md
...
//
// Nineteen graded markers, derived not declared: nine guards PKP_G1..G9 and
// ten arms PKP_T1..T10, contiguous. COUNT THEM. An errored marker in this
// language PRINTS NOTHING rather than going red, so a transcript with
// eighteen is a spec that lost a claim, not a spec that passed. (This block
// read fifteen/seven/eight until 2026-09-23, when G8, T9 and T10 arrived with
// the IMPORTSQL FILE gating; G9 followed the same day from the extended
// spec's first, red run -- its CREATEs had closed T9's fixture and the
// ratchet caught the blind arm pre-commit.)
//
...
// that already worked.
//
// PART B -- PKP_T4..T10 -- is the ACCEPTANCE CRITERION for write-time refusal
// on the native REPLACE path, on SQLSEL INSERT and SQLSEL UPDATE, on the
// LEGACY bare INSERT and UPDATE verbs, and (2026-09-23) on IMPORTSQL FILE,
// whose pair also proves imported rows come out minted. Asserted as separate
// arms rather than one because each reaches the table by its own path and a
...
// BUMPED 5 -> 7 ON 2026-09-23, WITH THE ARMS IT COUNTS, IN THE SAME COMMIT.
// T9 and T10 are the IMPORTSQL FILE pair (owner ruling: identity is
// house-owned, meaning is data -- a source key is demoted to a data column,
// never imported into the house key, and the house key is minted at append).
// They are born green because the enforcement and the arms land together;
// the ratchet's job from here is to notice either of them going red.
constexpr int kPkAcceptanceExpected = 7;
...
{
    static constexpr std::array<const char*, 12> partA{{
        ...
        // AIF-156 2026-09-23: the IMPORTSQL verb guard, G6's reason a third
        // time -- T9 reads "refused" from rows not appearing, and a verb that
        // failed to parse (or a missing fixture file) also appends nothing.
        "PKP_G8_import_verb_appends_unkeyed:.T.",
        // AIF-156 2026-09-23, SAME DAY, FROM A RED RUN: T9's fixture-open
        // guard. The first cut had none; its CREATEs closed PKPOL out of
        // area 1 (CREATE closes the current area before writing), the import
        // refused for the WRONG reason, T9 read .F., and this ratchet fired
        // pre-commit. PKP_G5's lesson, seventh recorded arrival.
        "PKP_G9_t9_fixture_is_open:.T."
    }};
    static constexpr std::array<const char*, 7> partBNames{{
        ...
        // AIF-156 2026-09-23: the IMPORTSQL FILE pair. T9 -- an import whose
        // positional columns would land on the stamped key refuses BEFORE the
        // first row. T10 -- rows imported past the key come out MINTED, on a
        // key deliberately not named SID so the magic-name planner cannot
        // mint it by spelling alone.
        "PKP_T9_import_covering_key_refused:",
        "PKP_T10_import_rows_minted:"
    }};
    ...
    if (green != kPkAcceptanceExpected) {
        std::cout << "PK POLICY: FAIL -- acceptance count changed: "
                  << green << " of 7 Part B arms green, expected "
    ...
    const char* posture =
        (green == 7)
            ? "the six write paths THIS SPEC ASKS ABOUT refuse a write to a "
              "declared PRIMARY key -- native REPLACE, SQLSEL INSERT, SQLSEL "
              "UPDATE, the LEGACY bare INSERT and UPDATE verbs, and (as of "
              "2026-09-23) IMPORTSQL FILE, whose pair of arms also proves "
              "imported rows come out MINTED rather than blank. It still does "
              "not ask about COPY, SORT or the workspace writer, each of which "
              "reaches a field write by its own route (COPY and SORT carry the "
              "designation through carryPrimaryKey, asserted elsewhere); "
              "REPLACE_MULTI is gated as of 2026-09-07 and is UNASSERTED HERE "
              "because the multirep spec was retired on 2026-09-04 and nothing "
              "replaced it. Seven of seven is the arms that exist, NOT a key "
              "that is enforced; the static gate over the callers is what "
              "closes that gap."
        : (green == 0)
    ...
    std::cout << "PK POLICY: PASS -- Part A green (12 markers), acceptance "
              << green << " of 7 as expected. AIF-156: " << posture << "\n";
    return true;
}
                                   ...
                                   const std::array<const char*, N>& expected)
{
    ...
    while (std::getline(in, line)) {
        ...
        const bool truth_marker =
            line.compare(line.size() - 4, 4, ":.T.") == 0 ||
            line.compare(line.size() - 4, 4, ":.F.") == 0;
    ...
    for (std::size_t i = 0; i < expected.size(); ++i) {
        const std::string wanted = std::string(expected[i]) + ":.T.";
        if (actual[i] != wanted) {
            std::cout << label << ": FAIL -- marker " << (i + 1)
                      << " mismatch\n"
                      << "  expected: " << wanted << "\n"
                      << "  actual  : " << actual[i] << "\n";
            return false;
    ...
    std::cout << label << ": PASS -- " << expected.size()
              << " exact marker(s), all true and in order.\n";
    return true;
}

bool validate_workspace_markers(const RegressionSpec& spec,
                                const std::string& transcript)
{
    const std::string name(spec.name);

    if (name == "WORKSPACE_SCOPE") {
        static constexpr std::array<const char*, 8> kExpected = {{
            "WS_G0_default_sentinel_written",
            "WS_G1_parent_sentinel_written",
            "WS_G2_child_sentinel_written",
            "WS_T1_scoped_close_spared_default",
            "WS_G3_child_reopened",
            "WS_T2_recursion_off_kept_child_open",
            "WS_T3_default_untouched_under_off",
            "WS_T4_close_all_released_the_file"
        }};
        return validate_true_marker_contract(transcript, spec.name, "WS_", kExpected);
    }
    if (name == "WSMULTI") {
        static constexpr std::array<const char*, 11> kExpected = {{
            "WSM_G0_default_sentinel",
            "WSM_G1_alpha_sentinel",
            "WSM_G2_beta_sentinel",
            "WSM_G3_gamma_sentinel",
            "WSM_G4_share_open_in_alpha",
            "WSM_G5_share_open_in_beta",
            "WSM_T1_peer_survived_sibling_close",
            "WSM_T2_shared_file_survived_in_peer",
            "WSM_T3_default_untouched",
            "WSM_T4_child_file_released_and_reopenable",
            "WSM_T5_default_readable_after_switch_back"
        }};
        return validate_true_marker_contract(transcript, spec.name, "WSM_", kExpected);
    }
    if (name == "WSLADDER") {
        static constexpr std::array<const char*, 13> kExpected = {{
            "WSL_G4_default_sentinel_written",
            "WSL_G5_occupant_sentinel_written",
            "WSL_G0_live_row_located_by_name",
            "WSL_G1_located_row_is_live",
            "WSL_T1_birth_row_is_self_describing",
            "WSL_T2_birth_row_carries_no_payload",
            "WSL_T3_birth_row_is_the_chain_root",
            "WSL_G2_live_row_located_after_recreate",
            "WSL_T4_destroy_then_new_minted_a_fresh_id",
            "WSL_G3_retired_row_still_present",
            "WSL_T5_retired_row_is_superseded",
            "WSL_T6_default_survived_the_refusal",
            "WSL_T7_occupied_workspace_kept_its_area"
        }};
        return validate_true_marker_contract(transcript, spec.name, "WSL_", kExpected);
    }
    if (name == "RELSCOPE2") {
        static constexpr std::array<const char*, 5> kExpected = {{
            "RS_G0_child_fixture_written",
            "RS_G1_relation_live_before_the_act",
            "RS_T1_clear_all_spared_the_other_workspace",
            "RS_G2_other_workspace_relation_rebuilt",
            "RS_T2_scoped_close_spared_the_other_workspace"
        }};
        return validate_true_marker_contract(transcript, spec.name, "RS_", kExpected);
    }
    if (name == "MWXSHAKE") {
        static constexpr std::array<const char*, 48> kExpected = {{
            "MWX_G0a_fixture_A_parent_written",
            "MWX_G0b_fixture_A_shared_written",
            "MWX_G0c_fixture_B_parent_written",
            "MWX_G0d_fixture_B_shared_written",
            "MWX_G0e_child_table_written",
            "MWX_G1a_one_holds_its_area",
            "MWX_G1b_two_holds_its_area",
            "MWX_G1c_child_holds_its_area",
            "MWX_T1_open_as_named_the_workspace",
            "MWX_T2_reentry_left_the_open_table_alone",
            "MWX_T3_sibling_workspace_survived_a_scoped_close",
            "MWX_T4_recursion_off_left_the_nested_workspace_open",
            "MWX_T5_second_copy_opened_beside_the_first",
            "MWX_T6_destroy_refused_and_the_area_survived",
            "MWX_T7_default_refused_destruction_and_still_works",
            "MWX_T8_parent_with_a_child_refused_and_disk_intact",
            "MWX_G2_first_mint_wrote_a_birth_row",
            "MWX_T9_remint_after_destroy_is_a_fresh_identity",
            "MWX_T10_deleted_rows_are_still_on_disk",
            "MWX_T11_delete_did_not_lower_the_high_water_mark",
            "MWX_T12_purge_alias_still_reaches_the_verb",
            "MWX_T13_declared_workspace_kept_its_live_row",
            "MWX_G3_container_source_has_its_memo",
            "MWX_T14_session_survived_the_minidb_refusal",
            "MWX_T15_row_one_hydrated_from_the_memo",
            "MWX_T16_memo_value_survived_the_container",
            "MWX_T17_hydrated_table_is_writable",
            "MWX_T18_writeback_reached_the_real_disk",
            "MWX_T19_writeback_carried_the_post_hydration_write",
            "MWX_G4a_member_really_removed",
            "MWX_G4b_survivor_still_on_disk",
            "MWX_G5_healthy_session_standing",
            "MWX_T20_session_survived_the_shortfall_refusal",
            "MWX_T21_partial_restored_the_survivor",
            "MWX_T22_missing_index_did_not_refuse_the_load",
            "MWX_G6_order_is_live_before_the_save",
            "MWX_T25_set_order_survived_the_round_trip",
            "MWX_T26_and_the_far_end_of_the_order_too",
            "MWX_G7a_physical_first_is_ZULU_before_any_index",
            "MWX_G7b_physical_last_is_TANGO_before_any_index",
            "MWX_T27_self_built_cdx_orders_the_walk",
            "MWX_T28_and_the_far_end_of_the_built_order",
            "MWX_T31_addtag_refused_a_dead_tag_and_changed_nothing",
            "MWX_G7c_built_order_is_live_before_the_save",
            "MWX_T29_built_tag_survived_the_round_trip",
            "MWX_T30_far_end_after_the_built_round_trip",
            "MWX_T23_container_row_is_flagged_minidb",
            "MWX_T24_posture_row_is_flagged_dtshema"
        }};
        return validate_true_marker_contract(transcript, spec.name, "MWX_", kExpected);
    }
    if (name == "OPENJOIN") {
        static constexpr std::array<const char*, 7> kExpected = {{
            "OJC_G0_fixture_a_reads_AAA",
            "OJC_G1_fixture_b_reads_BBB",
            "OJC_G2_fixture_c_reads_CCC",
            "OJC_G3_the_open_landed_somewhere",
            "OJC_T1_opened_table_was_in_the_current_workspace",
            "OJC_T2_third_table_landed_past_both",
            "OJC_T3_AS_kept_the_open_out_of_the_current_workspace"
        }};
        return validate_true_marker_contract(transcript, spec.name, "OJC_", kExpected);
    }
    if (name == "WSENV") {
        static constexpr std::array<const char*, 9> kExpected = {{
            "RSE_G0_fixture_a_reads_AAA",
            "RSE_G1_fixture_b_reads_BBB",
            "RSE_G2_workspace_a_opened_its_own_table",
            "RSE_G3_workspace_b_opened_its_own_table",
            "RSE_T1_switch_restored_this_workspace_environment",
            "RSE_T2_switch_restores_the_other_way_too",
            "RSE_T3_in_clause_left_the_session_alone",
            "RSE_T4_in_clause_bound_the_named_workspace",
            "RSE_T5_explicit_binding_survived_the_switch"
        }};
        return validate_true_marker_contract(transcript, spec.name, "RSE_", kExpected);
    }

    std::cout << "WORKSPACE MARKERS " << spec.name
              << ": FAIL -- no marker contract registered for this spec.\n";
    return false;
}
                                    ...
                                    bool after_ok)
{
    ...
    if (not_graded) {
        std::cout << "\n  NOT GRADED -- ran, printed a transcript, and NOTHING checked\n"
                     "  it. These specs cannot go red. Their presence makes the suite\n"
    ...
    // AIF-165. PASS means the suite graded everything it ran. Transcript-only
    // specs are neither failures nor successes, so a clean but incomplete run
    // is PARTIAL and sets the engine command error context to non-success.
    const bool hard_fail = failed || unmeasured || !before_ok || !after_ok;
    const char* const verdict = hard_fail ? "FAIL" : (not_graded ? "PARTIAL" : "PASS");
    std::cout << "\n  VERDICT: " << verdict << "\n"
                 "============================================================\n";
}
...
void run_regression_default_suite(DbArea& area, const std::string& log_path)
{
    ...
    // CLEARS that global, a failure in the middle of the suite could not
    // survive to the end of it. AIF-165 also makes NOT GRADED non-success.
    std::size_t failed = 0, not_graded = 0, unmeasured = 0;
    for (const auto& r : results) {
        if (r.outcome == RegressionOutcome::Failed)     ++failed;
        if (r.outcome == RegressionOutcome::NotGraded)  ++not_graded;
        if (r.outcome == RegressionOutcome::Unmeasured) ++unmeasured;
    }

    if (!before_ok || !after_ok || failed || unmeasured || not_graded) {
        xbase::error::set_last_error(xbase::error::e_invalid_argument());
    } else {
        xbase::error::clear_last_error();
    }
