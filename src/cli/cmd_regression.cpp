...
// an expected 0, and this bump is the deliberate human acknowledgement it was
// asking for. PKP_T5 and PKP_T6 are green because SQLSEL INSERT and SQLSEL
// UPDATE both reach the table buffer through evaluate_store_expression, which
// calls validate_field_constraint_for_store, which now refuses any write to a
// declared PRIMARY field. PKP_T4 IS STILL RED AND STILL EXPECTED TO BE: native
// REPLACE does not route through that validator, so the third call site is
// unwired and the arm that would prove it is the one arm this constant does
// not credit. Raising this to 2 is NOT a claim that the primary key is
// enforced -- it is a claim that two of the three doors are now shut, and the
// ratchet still fails if either reopens.
constexpr int kPkAcceptanceExpected = 2;
