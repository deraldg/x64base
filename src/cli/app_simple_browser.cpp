...
// usage-access: SIMPLEBROWSER USAGE
// summary:
...
//   SIMPLEBROWSER USAGE
//   SIMPLEBROWSER [FOR <expr>] [RAW|PRETTY] [PAGE <n>] [ALL] [TOP|BOTTOM]
//   SIMPLEBROWSER [START KEY <literal>] [QUIET] [EDIT|SESSION]
//
// examples:
//   SIMPLEBROWSER
//   SIMPLEBROWSER PAGE 20
//   SIMPLEBROWSER FOR GPA >= 3.0
//   SIMPLEBROWSER START KEY SMITH
//   SIMPLEBROWSER EDIT
//
// notes:
//   SIMPLEBROWSER USAGE prints usage before open-table checks or UI launch.
...
static void print_SIMPLEBROWSER_usage_contract()
{
    std::cout
        << "Usage:\n"
        << "  SIMPLEBROWSER\n"
        ...
        << "  SIMPLEBROWSER EDIT\n"
        << "Notes:\n"
        << "  - SIMPLEBROWSER USAGE does not require an open table and does not launch the browser.\n"
        << "  - Non-interactive listing restores the cursor best-effort.\n"
        << "  - Interactive edit/session mode may move cursor and can mutate table data by explicit edit/delete commands.\n";
}

static bool SIMPLEBROWSER_usage_token_contract(std::string tok)
{
    std::transform(tok.begin(), tok.end(), tok.begin(),
        [](unsigned char c) { return static_cast<char>(std::toupper(c)); });
    return tok == "USAGE" || tok == "HELP" || tok == "?";
}
void app_SIMPLE_BROWSER(xbase::DbArea& area, std::istringstream& in)
{
    // SIMPLEBROWSER_USAGE_CONTRACT_BRANCH
    {
        const std::streampos SIMPLEBROWSER_usage_pos = in.tellg();
        std::string SIMPLEBROWSER_usage_tok;
        if (in >> SIMPLEBROWSER_usage_tok) {
            in.clear();
            if (SIMPLEBROWSER_usage_pos != std::streampos(-1)) {
                in.seekg(SIMPLEBROWSER_usage_pos);
            }

            if (SIMPLEBROWSER_usage_token_contract(SIMPLEBROWSER_usage_tok)) {
                print_SIMPLEBROWSER_usage_contract();
                return;
            }
        } else {
            in.clear();
    ...
    if (!area.isOpen()) { std::cout << "SIMPLEBROWSER: no file open.\n"; return; }
