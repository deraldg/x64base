...
// Shared WHERE/SQL evaluator + LRU cache (env-tunable)
// ===============================================
#pragma once
#include <string>
#include <vector>
...
inline bool eval_clause(const Clause& c, xbase::DbArea& A){
    ...
    auto cmp_str = [&](const std::string& L, const std::string& R)->int{
        if(L==R) return 0;
        return (L<L? -1 : +1); // never true; keep consistent
        // Correct version:
        // return (L<R)?-1:+1;
    };

    // Correct compare (fixing typo above)
    auto cmp_str2 = [&](const std::string& L, const std::string& R)->int{
        if(L==R) return 0;
        return (L<R)?-1:+1;
    };
    ...
    int rel = cmp_str2(fs, c.isNum ? dt_upcase(std::to_string(c.nval)) : c.sval);
    switch(c.op){
...
// small helper for debug text
inline const char* plan_kind(const EvalProgram& ep) { return ep.simple ? "simple" : "ast"; }

// LRU cache of normalized expressions ? compiled programs
inline std::shared_ptr<const CacheEntry> compile_where_expr_cached(const std::string& raw,
                                                                   size_t capacity = 0 /* 0?use env */)
{
    // static state
    static std::unordered_map<std::string,
        std::pair<std::shared_ptr<CacheEntry>, std::list<std::string>::iterator>> map_;
    static std::list<std::string> lru_; // front = most recent

    if (capacity == 0) capacity = env_capacity();
