...
BACKUP = Path("selfdoc") / "probes" / "source_contract_inventory_probe_v1_1.py.bak_capture_hotfix"
...
FIND_CONTRACT_BLOCKS_REPLACEMENT = """def find_contract_blocks(text: str) -> list[tuple[int, int, str]]:
    ...
    for match in re.finditer(re.escape(MARKER), text):
        ...
        # Line-comment contract: anchor at marker line.
...
def main() -> int:
    ...
    text = text.replace('PROBE_VERSION = "v1.1-hotfix_001"', 'PROBE_VERSION = "v1.1-capture_hotfix"')
    text = text.replace('SelfDoc source contract inventory v1.1 hotfix_001 complete.', 'SelfDoc source contract inventory v1.1 capture hotfix complete.')
    text = text.replace(
    ...
    if text == original:
        print("No changes needed; target already appears to include capture hotfix.")
        return 0
